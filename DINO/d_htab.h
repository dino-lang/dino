...
#else
static inline void dino_htab_assert_fail (const char *op, const char *var) {
  fprintf (stderr, "wrong %s for %s\n", op, var);
  assert (0);
}

#define HTAB_ASSERT(EXPR, OP, T) (void) ((EXPR) ? 0 : (dino_htab_assert_fail (OP, #T), 0))

#endif

#ifdef __GNUC__
#define DINO_HTAB_NO_RETURN __attribute__ ((noreturn))
#else
#define DINO_HTAB_NO_RETURN
#endif

static inline void DINO_HTAB_NO_RETURN dino_htab_error (const char *message) {
#ifdef DINO_HTAB_ERROR
  DINO_HTAB_ERROR (message);
  assert (FALSE);
#else
  fprintf (stderr, "%s\n", message);
#endif
...
#define DEF_HTAB(T)                                                                               \
  ...
  static inline void HTAB_OP (T, create) (HTAB (T) * *htab, htab_size_t min_size,                 \
    ...
    for (size = 2; min_size > size; size *= 2)                                                    \
      ;                                                                                           \
    ht = malloc (sizeof (*ht));                                                                   \
    if (ht == NULL) dino_htab_error ("htab: no memory");                                          \
