...
static ER_node_t create_class_stack (BC_node_t class_block, ER_node_t context, val_t *actuals_start,
                                     int actuals_num, int simple_p);

static void min_max_call (int pars_number, int min_flag) {
  ER_node_t val, r, v;
  int i;
  val_t res;
  ...
  for (i = 0; i < pars_number; i++) {
    ...
    if (ER_NODE_MODE (val) != ER_NM_int && ER_NODE_MODE (val) != ER_NM_long
      ...
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type,
                  (min_flag ? MIN_NAME : MAX_NAME));
    if (i == 0) {
      res = *(val_t *) val;
      continue;
    ...
    if (ER_NODE_MODE (val) == ER_NODE_MODE ((ER_node_t) &res)) {
      ...
      implicit_conversion_for_binary_arithmetic_op ((ER_node_t) &res, val, &r, &v);
      d_assert (ER_NODE_MODE (r) != ER_NM_int && ER_NODE_MODE (v) != ER_NM_int);
    }
    d_assert (ER_NODE_MODE (r) == ER_NODE_MODE (v));
    if (ER_NODE_MODE (v) == ER_NM_int) {
      if ((ER_i (v) < ER_i (r)) == min_flag) res = *(val_t *) val;
    } else if (ER_NODE_MODE (v) == ER_NM_long) {
      if ((mpz_cmp (*ER_mpz_ptr (ER_l (v)), *ER_mpz_ptr (ER_l (r))) < 0) == min_flag)
        res = *(val_t *) val;
...
void min_call (int pars_number) { min_max_call (pars_number, TRUE); }

void max_call (int pars_number) { min_max_call (pars_number, FALSE); }

static void to_lower_upper (int pars_number, int lower_flag) {
  ER_node_t vect;
  ...
  if (pars_number != 1) eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, name);
  to_vect_string_conversion (ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
...
void toupper_call (int pars_number) { to_lower_upper (pars_number, FALSE); }

void tolower_call (int pars_number) { to_lower_upper (pars_number, TRUE); }
...
static void initiate_subst_map (void) {
  subst_map_els = NULL;
...
static void free_subst_map (void) {
  if (subst_map_els != NULL) free (subst_map_els);
  if (subst_map_hash_index != NULL) free (subst_map_hash_index);
}

/* Finish all work with the map.  */
static void finish_subst_map (void) { free_subst_map (); }
...
static void start_subst_map (size_t els_num) {
...
static void set_map_subst (ucode_t from, ucode_t to) {
  unsigned int hash = from % subst_map_hash_index_len;
  size_t i, ind = subst_map_hash_index[hash];

  if (ind >= subst_map_els_bound) /* New hash: make a cycle.  */
    subst_map_els[subst_map_els_bound].next_subst_map_hash_index = subst_map_els_bound;
  else {
    for (i = ind;;)
...
/* Return mapping FROM.  If there is no corresponding element, return
   FROM.  */
static ucode_t get_map_subst (ucode_t from) {
  unsigned int hash = from % subst_map_hash_index_len;
  size_t i, ind = subst_map_hash_index[hash];
  ...
  if (ind >= subst_map_els_bound)
    ...
    for (i = ind;;)
      if (subst_map_els[i].from == from)
        ...
        i = subst_map_els[i].next_subst_map_hash_index;
        if (i == ind) return from; /* No element for FROM in the bucket.  */
      }
  }
}

void translit_call (int pars_number) {
  ER_node_t vect, v;
  ...
  if (pars_number != 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, TRANSLIT_NAME);
  ...
  if (ER_NODE_MODE (ctop) != ER_NM_vect
    ...
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, TRANSLIT_NAME);
  vect = ER_vect (vect);
  ucode_p = ER_pack_vect_el_mode (vect) == ER_NM_char;
  ...
  if (len != 0 && ER_els_number (ER_vect (ctop)) != 0) {
    ...
    for (i = 0, str = ER_pack_els (ER_vect (below_ctop)); i < ER_els_number (ER_vect (below_ctop));
         i++)
      if (from_ucode_p)
        set_map_subst ((unsigned) ((ucode_t *) str)[i],
                       subst_ucode_p ? ((ucode_t *) subst)[i] : ((byte_t *) subst)[i]);
      else
    ...
    for (str = ER_pack_els (vect), i = 0; i < len; i++)
      if (ucode_p)
        ((ucode_t *) str)[i] = get_map_subst ((unsigned) ((ucode_t *) str)[i]);
      else
        ((byte_t *) str)[i] = get_map_subst ((unsigned) ((byte_t *) str)[i]);
  }
  ER_SET_MODE (fun_result, ER_NM_vect);
  set_vect_dim (fun_result, vect, 0);
}

void eltype_call (int pars_number) {
  ER_node_t vect;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ELTYPE_NAME);
  if (ER_NODE_MODE (ctop) != ER_NM_vect)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, ELTYPE_NAME);
  vect = ER_vect (ctop);
  ...
  if (ER_NODE_MODE (vect) != ER_NM_heap_pack_vect)
    ...
    if (type == type_fun) {
      ...
      for (i = 0; i < ER_els_number (vect); i++) {
        memcpy ((char *) var_ref + displ, (char *) ER_pack_els (vect) + i * el_size, el_size);
...
void keys_call (int pars_number) {
  ER_node_t tab;
  ER_node_t vect;
  size_t i, index;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, KEYS_NAME);
  if (ER_NODE_MODE (ctop) != ER_NM_tab)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, KEYS_NAME);
  tab = ER_tab (ctop);
...
void closure_call (int pars_number) {
  ER_node_t val;
  BC_node_t block;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, CLOSURE_NAME);
  val = IVAL (ctop, -pars_number + 1);
  /* Place the result instead of the function. */
  if (ER_NODE_MODE (val) == ER_NM_stack && (block = ER_block_node (ER_stack (val))) != NULL
...
void context_call (int pars_number) {
  ER_node_t val;
  ER_node_t context;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, CONTEXT_NAME);
  val = IVAL (ctop, -pars_number + 1);
  if (ER_NODE_MODE (val) == ER_NM_stack)
    ...
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, CONTEXT_NAME);
  /* Place the result instead of the function. */
  if (context == NULL)
    ER_SET_MODE (fun_result, ER_NM_nil);
  else {
...
int internal_inside_call (const char **message_ptr, ER_node_t where, ER_node_t what,
                          int context_flag) {
  ...
  *message_ptr = NULL;
  ...
  result = (code == code_2 && (!context_flag || code_context == code_2_context));
  if (code != NULL && ER_NODE_MODE (what) == ER_NM_code) code = BC_scope (code);
  for (; !result && code != NULL; code = BC_scope (code), code_context = ER_context (code_context))
    if (code == code_2 && (!context_flag || ER_context (code_context) == code_2_context))
      result = 1;
  return result;
}

void inside_call (int pars_number) {
  const char *message;
  int result;
  int flag;

  if (pars_number != 2 && pars_number != 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, INSIDE_NAME);
  flag = 0;
  if (pars_number == 3) {
    implicit_int_conversion (ctop, NULL);
    if (!ER_IS_OF_TYPE (ctop, ER_NM_int))
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, INSIDE_NAME);
    flag = ER_i (ctop);
  }
  result = internal_inside_call (&message, IVAL (ctop, 2 - pars_number),
                                 IVAL (ctop, 1 - pars_number), flag);
  if (message != NULL) eval_error (partype_bc_decl, call_pos (), message, INSIDE_NAME);
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, result);
}

int code_use_p (BC_node_t code, BC_node_t where) {
  BC_node_t use;
  int result;

  d_assert (BC_IS_OF_TYPE (code, BC_NM_block) && BC_IS_OF_TYPE (where, BC_NM_block));
  for (result = code == where, use = BC_uses (code); !result && use != NULL;
       use = BC_next_use (use))
    if (BC_use (use) == where) result = 1;
  return result;
}

int internal_isa_call (const char **message_ptr, ER_node_t where, ER_node_t what) {
  ...
  if (ER_NODE_MODE (what) == ER_NM_stack) {
    ...
    if (message_ptr != NULL) *message_ptr = DERR_parameter_type;
    return 0;
  }
...
void isa_call (int pars_number) {
  const char *message;
  int result;

  if (pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ISA_NAME);
  result = internal_isa_call (&message, IVAL (ctop, 0), IVAL (ctop, -1));
  if (message != NULL) eval_error (partype_bc_decl, call_pos (), message, ISA_NAME);
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, result);
}
...
static void process_onig_errors (int code, OnigErrorInfo *einfo) {
...
DEF_VARR (regex_node_t);
DEF_HTAB (regex_node_t);

/* Temporary structure. */
static struct regex_node temp_regex_node;
...
/* Hash of the node. */
static htab_hash_t regex_node_hash (regex_node_t node) {
  if (!node->ucode_p) {
    return dino_hash (node->string, strlen (node->string), 8);
  } else {
    size_t i;
    ...
    for (i = 0; str[i] != '\0'; i++)
      ;
    return dino_hash (node->string, sizeof (ucode_t) * i, 7);
  }
}

/* Equality of nodes. */
static int regex_node_eq (regex_node_t node1, regex_node_t node2) {
  size_t i;
  int ch1, ch2;

  if (!node1->ucode_p && !node2->ucode_p) return strcmp (node1->string, node2->string) == 0;
  for (i = 0;; i++) {
    ch1 = (node1->ucode_p ? ((const ucode_t *) node1->string)[i]
                          : ((const byte_t *) node1->string)[i]);
    ch2 = (node2->ucode_p ? ((const ucode_t *) node2->string)[i]
...
/* Return size of ucode string STR (if UCODE_P) or byte string
   ...
   ZERO_CHAR_P.  */
static inline size_t get_str_size (const void *str, int ucode_p, int zero_char_p) {
  size_t tail_size = !zero_char_p ? 0 : ucode_p ? sizeof (ucode_t) : sizeof (byte_t);
  return (ucode_p ? ucodestrlen ((const ucode_t *) str) * sizeof (ucode_t)
                  : strlen ((const char *) str) * sizeof (byte_t))
         + tail_size;
}
...
static int find_regex (const void *pat, int pat_ucode_p, int str_ucode_p, regex_t **result,
                       OnigErrorInfo *einfo) {
  regex_node_t regn, tab_regn;
  regex_t *r;
  size_t len, size;
  int code, found_p;
  OnigCompileInfo ci;
  ...
  *result = NULL;
  /* We can remove const here, as find_hash_table does not change it.
     We always copy to temp_regex_node.string when put a node into the
     table.  */
  temp_regex_node.string = (void *) pat;
  temp_regex_node.ucode_p = pat_ucode_p;
  if ((found_p = HTAB_DO (regex_node_t, regex_tab, &temp_regex_node, HTAB_FIND, tab_regn))) {
    regn = tab_regn;
  ...
  code = onig_new_deluxe (&r, pat,
                          (OnigUChar *) ((char *) pat + get_str_size (pat, pat_ucode_p, FALSE)),
                          &ci, einfo);
  if (code != ONIG_NORMAL) {
    onig_free (r);
  ...
  if (!found_p) HTAB_DO (regex_node_t, regex_tab, regn, HTAB_INSERT, tab_regn);
  *result = r;
  return ONIG_NORMAL;
}
...
static void finish_regex_tab (void) {
  int i;
  regex_node_t regn;

  HTAB_DESTROY (regex_node_t, regex_tab);
  for (i = 0; i < VARR_LENGTH (regex_node_t, regex_varr); i++) {
    regn = VARR_GET (regex_node_t, regex_varr, i);
    if (regn->latin1_latin1_regex != NULL) onig_free (regn->latin1_latin1_regex);
    if (regn->latin1_ucode_regex != NULL) onig_free (regn->latin1_ucode_regex);
...
void internal_match_call (ER_node_t result_op, const char *regexp_string, int pat_ucode_p,
                          ER_node_t string_op, const char *additional_msg) {
  regex_t *reg;
  ER_node_t result;
  size_t els_number;
  size_t i;
  ...
  if (code != ONIG_NORMAL) process_onig_errors (code, &einfo);
  ...
  if (code == ONIG_MISMATCH)
    ...
    for (i = 0; i < els_number; i++) {
      ...
      ((rint_t *) ER_pack_els (result))[2 * i + 1] = region->end[i] / ch_size;
    }
  } else
...
void match_call (int pars_number) {
  if (pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, MATCH_NAME);
  to_vect_string_conversion (ctop, NULL, NULL);
  ...
  internal_match_call (fun_result, ER_pack_els (ER_vect (below_ctop)),
                       ER_pack_vect_el_mode (ER_vect (below_ctop)) == ER_NM_char, ctop, MATCH_NAME);
}

void gmatch_call (int pars_number) {
  regex_t *reg;
  ...
  if (pars_number != 2 && pars_number != 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GMATCH_NAME);
  flag = 0;
  if (pars_number == 3) {
    implicit_int_conversion (ctop, NULL);
    if (!ER_IS_OF_TYPE (ctop, ER_NM_int))
  ...
  if (ER_NODE_MODE (par2) != ER_NM_vect || ER_NODE_MODE (ER_vect (par2)) != ER_NM_heap_pack_vect
      || (ER_pack_vect_el_mode (ER_vect (par2)) != ER_NM_char
          && ER_pack_vect_el_mode (ER_vect (par2)) != ER_NM_byte)
      || ER_NODE_MODE (par1) != ER_NM_vect || ER_NODE_MODE (ER_vect (par1)) != ER_NM_heap_pack_vect
  ...
  code = find_regex (ER_pack_els (ER_vect (par1)), pat_ucode_p, str_ucode_p, &reg, &einfo);
  if (code != ONIG_NORMAL) process_onig_errors (code, &einfo);
  ...
  while (onig_search (reg, (const OnigUChar *) (start + disp), (const OnigUChar *) end,
                      (const OnigUChar *) (start + disp), (const OnigUChar *) end, region,
                      ONIG_OPTION_NONE)
         >= 0) {
    el = (region->beg[0] + disp) / ch_size;
    VARR_PUSH_ARR (char, temp_varr2, (char *) &el, sizeof (el));
    el = (region->end[0] + disp) / ch_size;
    VARR_PUSH_ARR (char, temp_varr2, (char *) &el, sizeof (el));
    if (flag || region->beg[0] >= region->end[0]) /* Empty string match here too.  */
      disp += ch_size;
    else
      disp += region->end[0];
    count++;
    if (disp >= ER_els_number (ER_vect (par2)) * ch_size) break;
  }
  if (count == 0)
    ER_SET_MODE (fun_result, ER_NM_nil);
  else {
    result = create_pack_vector (2 * count, ER_NM_int);
    memcpy (ER_pack_els (result), VARR_ADDR (char, temp_varr2), 2 * count * sizeof (el));
    ER_SET_MODE (fun_result, ER_NM_vect);
    set_vect_dim (fun_result, result, 0);
  }
}

static void generall_sub_call (int pars_number, int global_flag) {
  regex_t *reg;
  size_t len;
  ER_node_t result;
  ER_node_t vect;
  ER_node_t regex_val;
  size_t ch_size, els_num, bound;
  size_t disp;
  size_t i;
  ...
  code = find_regex (ER_pack_els (ER_vect (regex_val)), pat_ucode_p, str_ucode_p, &reg, &einfo);
  if (code != ONIG_NORMAL)
    process_onig_errors (code, &einfo);
  else {
    ...
    while ((disp < ER_els_number (vect) * ch_size || disp == 0)
           && onig_search (reg, (const OnigUChar *) (str + disp), (const OnigUChar *) end,
                           (const OnigUChar *) (str + disp), (const OnigUChar *) end, region,
                           ONIG_OPTION_NONE)
                >= 0) {
      bound = VARR_LENGTH (char, temp_varr2);
      VARR_GROW (char, temp_varr2, region->beg[0]);
      if (region->beg[0] != 0)
        memcpy (&VARR_ADDR (char, temp_varr2)[bound], str + disp, region->beg[0]);
      ...
      while (*src != '\0') {
        ...
        if (i >= 10) {
          ...
          if (str_ucode_p) {
            uc = c;
            VARR_PUSH_ARR (char, temp_varr2, (char *) &uc, sizeof (ucode_t));
          } else {
            ac = c;
            VARR_PUSH_ARR (char, temp_varr2, (char *) &ac, sizeof (byte_t));
          }
          els_num++;
          ...
          bound = VARR_LENGTH (char, temp_varr2);
          VARR_GROW (char, temp_varr2, len);
          memcpy (&VARR_ADDR (char, temp_varr2)[bound], str + disp + region->beg[i], len);
          els_num += len / ch_size;
        }
      }
    ...
    if (disp < ER_els_number (vect) * ch_size) {
      ...
      bound = VARR_LENGTH (char, temp_varr2);
      VARR_GROW (char, temp_varr2, len);
      memcpy (&VARR_ADDR (char, temp_varr2)[bound], str + disp, len);
      els_num += len / ch_size;
    }
    if (str_ucode_p) {
      uc = '\0';
    ...
    result = create_pack_vector (els_num + 1, str_ucode_p ? ER_NM_char : ER_NM_byte);
    memcpy (ER_pack_els (result), VARR_ADDR (char, temp_varr2), (els_num + 1) * ch_size);
...
void sub_call (int pars_number) { generall_sub_call (pars_number, FALSE); }

void gsub_call (int pars_number) { generall_sub_call (pars_number, TRUE); }

void split_call (int pars_number) {
  regex_t *reg;
  size_t len;
  ER_node_t result;
  ER_node_t vect;
  ER_node_t sub_vect;
  size_t i, els_number, bound;
  size_t chars_number;
  size_t disp, ch_size;
  char *split_regex;
  ER_node_t split_var;
  ...
  if (pars_number != 1 && pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SPLIT_NAME);
  if (pars_number != 1) to_vect_string_conversion (below_ctop, NULL, NULL);
  to_vect_string_conversion (ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
      || (ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_char
          && ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_byte)
  ...
  if (pars_number == 2) {
    ...
    str_ucode_p = ER_pack_vect_el_mode (ER_vect (ctop)) == ER_NM_char;
    split_var = IVAL (ER_stack_vars (re_stack), BC_var_num (split_regex_bc_decl));
    ...
    if (ER_NODE_MODE (split_var) == ER_NM_vect
      ...
      eval_error (invenvar_bc_decl, call_pos (), DERR_corrupted_environment_var, SPLIT_REGEX_NAME);
  }
  if (pat_ucode_p && !str_ucode_p) {
    /* Impossible ONIGURUMA combination: ucode pattern and ascii
       string.  */
  ...
  if (code != 0)
    ...
    disp = 0;
    for (;;) {
      ok = onig_search (reg, (const OnigUChar *) (str + disp), (const OnigUChar *) end,
                        (const OnigUChar *) (str + disp), (const OnigUChar *) end, region,
                        ONIG_OPTION_NONE)
           >= 0;
      if (ok) {
        if (region->beg[0] >= region->end[0]) /* Empty string matching. */
          region->beg[0] += ch_size;
        chars_number = region->beg[0] / ch_size;
      } else
        chars_number = ER_els_number (vect) - disp / ch_size;
      /* Create substring. */
      sub_vect = create_pack_vector (chars_number + 1, str_ucode_p ? ER_NM_char : ER_NM_byte);
      ER_set_els_number (sub_vect, chars_number);
      ER_set_immutable (sub_vect, TRUE);
      memcpy (ER_pack_els (sub_vect), ER_pack_els (vect) + disp, chars_number * ch_size);
      if (str_ucode_p)
        ((ucode_t *) ER_pack_els (sub_vect))[chars_number] = '\0';
      else
        ((byte_t *) ER_pack_els (sub_vect))[chars_number] = '\0';
      VARR_PUSH_ARR (char, temp_varr2, (char *) &sub_vect, sizeof (sub_vect));
      if (!ok) break;
      if (region->end[0] <= region->beg[0]) {
        /* Empty string matching. */
        disp += ch_size;
        if (disp >= ER_els_number (vect) * ch_size) break;
      } else {
        disp += region->end[0];
        if (disp > ER_els_number (vect) * ch_size) break;
      }
    }
    els_number = VARR_LENGTH (char, temp_varr2) / sizeof (ER_node_t);
    result = create_pack_vector (els_number, ER_NM_vect);
    ER_set_els_number (result, els_number);
    for (i = 0; i < els_number; i++)
      set_packed_vect_el (result, i, ((ER_node_t *) VARR_ADDR (char, temp_varr2))[i]);
  }
  if (result == NULL)
    ER_SET_MODE (fun_result, ER_NM_nil);
  else {
...
static int do_inline compare_elements (ER_node_mode_t el_type, const void *el1, const void *el2) {
  switch (el_type) {
  case ER_NM_char:
  ...
  case ER_NM_int:
    if (*(rint_t *) el1 < *(rint_t *) el2)
      ...
      return 1;
  case ER_NM_long: {
    int i = mpz_cmp (*ER_mpz_ptr (*(ER_node_t *) el1), *ER_mpz_ptr (*(ER_node_t *) el2));
...
static inline rint_t process_neg_index (rint_t index, size_t len) {
  if (index < 0) {
...
void subv_call (int pars_number) {
  ER_node_t vect;
  ER_node_t res;
  rint_t start;
  rint_t length;
  ...
  if (pars_number < 2 || pars_number > 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SUBV_NAME);
  if (pars_number == 2) {
    to_vect_string_conversion (below_ctop, NULL, NULL);
    implicit_int_conversion (ctop, NULL);
  } else {
  ...
  if (ER_NODE_MODE (IVAL (ctop, -pars_number + 1)) != ER_NM_vect || ER_NODE_MODE (ctop) != ER_NM_int
  ...
  if (start < vect_length && (length < 0 || start + length >= vect_length)) /* Get tail. */
  ...
  if (length == 0) {
    ...
    res = create_pack_vector (el_type == ER_NM_char || el_type == ER_NM_byte ? length + 1 : length,
                              el_type);
    ER_set_els_number (res, length);
    memcpy (ER_pack_els (res), ER_pack_els (vect) + start * el_size, el_size * length);
    ...
    memcpy (ER_unpack_els (res), (char *) ER_unpack_els (vect) + start * sizeof (val_t),
            length * sizeof (val_t));
  }
...
void cmpv_call (int pars_number) {
  ...
  if (pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, CMPV_NAME);
  to_vect_string_conversion (ctop, NULL, 0);
  to_vect_string_conversion (below_ctop, NULL, 0);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (below_ctop) != ER_NM_vect)
  ...
  for (i = 0; i < ER_els_number (vect1) && i < ER_els_number (vect2); i++) {
    if (ER_NODE_MODE (vect1) == ER_NM_heap_pack_vect) {
      ...
      addr1 = (char *) el + val_displ_table[ER_NODE_MODE ((ER_node_t) el)];
    }
    if (ER_NODE_MODE (vect2) == ER_NM_heap_pack_vect) {
      ...
      addr2 = (char *) el + val_displ_table[ER_NODE_MODE ((ER_node_t) el)];
    }
    ...
    if (el_type1 != el_type2
        || (el_type1 != ER_NM_float && el_type1 != ER_NM_int && el_type1 != ER_NM_char
            && el_type1 != ER_NM_byte && el_type1 != ER_NM_long))
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, CMPV_NAME);
...
void del_call (int pars_number) {
  ...
  val = IVAL (ctop, -pars_number + 1);
  if (pars_number < 2 || pars_number > 3 || (ER_NODE_MODE (val) == ER_NM_tab && pars_number != 2))
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, DEL_NAME);
  ...
  if (mode == ER_NM_vect) {
    ...
    if (ER_NODE_MODE (start_val) != ER_NM_int
      ...
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, DEL_NAME);
    ...
    GO_THROUGH_REDIR (vect);
    if (ER_immutable (vect))
      eval_error (immutable_bc_decl, call_pos (), DERR_immutable_vector_modification);
    vect_length = ER_els_number (vect);
    start = process_neg_index (start, vect_length);
    if (start < vect_length && (length < 0 || start + length >= vect_length)) {
      ...
      memmove ((char *) ER_unpack_els (vect) + start * sizeof (val_t),
               (char *) ER_unpack_els (vect) + (start + length) * sizeof (val_t),
    ...
    GO_THROUGH_REDIR (tab);
    if (ER_immutable (tab))
      eval_error (immutable_bc_decl, call_pos (), DERR_immutable_table_modification);
...
static void general_ins_call (int pars_number, int vector_flag) {
  ER_node_t vect_val;
  ER_node_t el_val;
  ER_node_t index_val;
  ER_node_t vect;
  ...
  if (pars_number == 3) {
    ...
    index_val = NULL;
  if (ER_NODE_MODE (vect_val) != ER_NM_vect || (vector_flag && ER_NODE_MODE (el_val) != ER_NM_vect)
      || (index_val != NULL && ER_NODE_MODE (index_val) != ER_NM_int))
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type,
                (vector_flag ? INSV_NAME : INS_NAME));
  if (index_val != NULL)
  ...
  if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect) {
    ...
    } else if (ER_pack_vect_el_mode (vect) == ER_NM_byte && ER_NODE_MODE (el_val) == ER_NM_char
  ...
  if (index < vect_length) {
    ...
    if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect) {
      ...
      memmove (ER_pack_els (vect) + (index + addition) * el_size,
               ER_pack_els (vect) + index * el_size, el_size * (vect_length - index));
    } else
      memmove ((char *) ER_unpack_els (vect) + (index + addition) * sizeof (val_t),
  ...
  if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect) {
    ...
    if (!vector_flag)
      memcpy (ER_pack_els (vect) + index * el_size,
              (char *) el_val + val_displ_table[ER_NODE_MODE (el_val)], el_size);
    else {
      d_assert (ER_NODE_MODE (el_vect) == ER_NM_heap_pack_vect
                && el_type == ER_pack_vect_el_mode (el_vect));
      memcpy (ER_pack_els (vect) + index * el_size, ER_pack_els (el_vect), el_size * addition);
    }
    if (el_type == ER_NM_byte)
    ...
    if (!vector_flag)
      *(val_t *) IVAL (ER_unpack_els (vect), index) = *(val_t *) el_val;
      ...
      memcpy (IVAL (ER_unpack_els (vect), index), ER_unpack_els (el_vect),
              addition * sizeof (val_t));
  }
...
void ins_call (int pars_number) { general_ins_call (pars_number, FALSE); }

void insv_call (int pars_number) { general_ins_call (pars_number, TRUE); }

void rev_call (int pars_number) {
  ER_node_t vect;
  ER_node_mode_t el_type;
  size_t vect_length;
  size_t el_size;
  ...
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, REV_NAME);
  if (ER_NODE_MODE (ctop) != ER_NM_vect)
  ...
  if (vect_length != 0) {
    if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect) {
      ...
      for (i = 0, j = vect_length - 1; i < j; i++, j--) {
        ...
        memcpy (ER_pack_els (vect) + i * el_size, ER_pack_els (vect) + j * el_size, el_size);
        memcpy (ER_pack_els (vect) + j * el_size, temp_el, el_size);
      }
    } else {
      for (i = 0, j = vect_length - 1; i < j; i++, j--) {
        temp_val = *(val_t *) IVAL (ER_unpack_els (vect), i);
        *(val_t *) IVAL (ER_unpack_els (vect), i) = *(val_t *) IVAL (ER_unpack_els (vect), j);
...
/* The function is comparison function for sorting homogeneous
   array. */
static int homogeneous_array_sort_compare_function (const void *el1, const void *el2) {
...
static int array_sort_compare_function (const void *el1, const void *el2) {
  int res;
  ...
  if (sorted_vect_el_type == ER_NM_byte) {
    ...
    memcpy ((char *) ctop + val_displ_table[sorted_vect_el_type], (char *) el1,
            type_size_table[sorted_vect_el_type]);
    if (sorted_vect_el_type == ER_NM_vect) ER_set_dim (ctop, 0);
  } else
  ...
  if (sorted_vect_el_type == ER_NM_byte) {
    ...
    ER_SET_MODE (ctop, sorted_vect_el_type);
    memcpy ((char *) ctop + val_displ_table[sorted_vect_el_type], (char *) el2,
            type_size_table[sorted_vect_el_type]);
    if (sorted_vect_el_type == ER_NM_vect) ER_set_dim (ctop, 0);
  } else
    *(val_t *) ctop = *(val_t *) el2;
  call_fun_class (dino_compare_fun_block, dino_compare_fun_block_context, 2, curr_from_c_code_p);
  TOP_UP;
  implicit_int_conversion (ctop, NULL);
...
void sort_call (int pars_number) {
  ER_node_t vect;
  ER_node_t var;
  ptrdiff_t offset = (char *) fun_result - (char *) cstack;

  if (pars_number != 1 && pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SORT_NAME);
  ...
  if (pars_number == 1) {
    if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
        || (ER_els_number (vect) != 0 && ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_char
            && ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_byte
      ...
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, SORT_NAME);
    vect = copy_vector (ER_vect (ctop));
    sorted_vect_el_type = ER_pack_vect_el_mode (vect);
    qsort (ER_pack_els (vect), ER_els_number (vect), type_size_table[sorted_vect_el_type],
    ...
    qsort ((sorted_vect_el_type == ER_NM_val ? (char *) ER_unpack_els (vect) : ER_pack_els (vect)),
           ER_els_number (vect),
           (sorted_vect_el_type != ER_NM_val ? type_size_table[sorted_vect_el_type]
                                             : sizeof (val_t)),
...
static inline void add_string_to_print (const char *str, int byte_p) {
  size_t i, len = strlen (str);
  ucode_t uc;

  if (byte_p)
    VARR_PUSH_ARR (char, temp_varr, str, len);
  else {
    for (i = 0; i < len; i++) {
      uc = str[i];
      VARR_PUSH_ARR (char, temp_varr, (char *) &uc, sizeof (ucode_t));
    }
  }
}

static int print_context (ER_node_t context, int byte_p) {
  BC_node_t block;
  string_t ident;
  char str[100];

  if (context == NULL || ER_context (context) == NULL) /* We ignore the uppest implicit block. */
    return FALSE;
  block = ER_block_node (context);
  ident = (BC_NODE_MODE (block) == BC_NM_block ? NULL : BC_ident (BC_fdecl (block)));
  if (print_context (ER_context (context), byte_p)) add_string_to_print (".", byte_p);
  if (ident == NULL)
...
static inline rint_t *get_file_unget_char_ptr (ER_node_t instance) {
  return (rint_t *) ((char *) IVAL (ER_stack_vars (instance), BC_var_num (unget_char_bc_decl))
                     + val_displ_table[ER_NM_int]);
}

static inline const char *get_file_encoding_name (ER_node_t instance) {
  return ER_pack_els (
    ER_vect (IVAL (ER_stack_vars (instance), BC_var_num (file_encoding_bc_decl))));
}

static inline encoding_type_t get_file_encoding_type (ER_node_t instance) {
  return (encoding_type_t) ER_hide (
    IVAL (ER_stack_vars (instance), BC_var_num (file_encoding_type_bc_decl)));
}

static FILE *get_file (int pars_number, ER_node_t *file_instance) {
  ER_node_t var;
  ER_node_t instance;

  var = IVAL (ctop, -pars_number + 1);
  if (!ER_IS_OF_TYPE (var, ER_NM_stack) || ER_stack_block (ER_stack (var)) != file_bc_decl)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, ifun_name);
  instance = ER_stack ((ER_node_t) IVAL (ctop, -pars_number + 1));
  if (file_instance != NULL) *file_instance = instance;
  return ER_hide (IVAL (ER_stack_vars (instance), BC_var_num (file_ptr_bc_decl)));
}

static conv_desc_t get_file_input_cd (ER_node_t file_instance) {
  ER_node_t var;
...
static void get_file_output_cds (ER_node_t file_instance, conv_desc_t *byte_cd,
                                 conv_desc_t *ucode_cd) {
  ER_node_t var;

  var = IVAL (ER_stack_vars (file_instance), BC_var_num (file_byte_ocd_bc_decl));
  *byte_cd = (conv_desc_t) (size_t) ER_hide (var);
  var = IVAL (ER_stack_vars (file_instance), BC_var_num (file_ucode_ocd_bc_decl));
  *ucode_cd = (conv_desc_t) (size_t) ER_hide (var);
}

static void set_file_encoding (ER_node_t file_vars, const char *encoding_name, conv_desc_t byte_cd,
                               conv_desc_t ucode_cd, conv_desc_t reverse_ucode_cd,
                               encoding_type_t tp) {
  ER_node_t var;
...
static void place_file_instance (FILE *f, ER_node_t result) {
  ER_node_t var;
  ER_node_t instance;
  ...
  instance
    = create_class_stack (file_bc_decl, get_obj_stack (io_bc_decl), (val_t *) result, 0, TRUE);
  ...
  ER_SET_MODE (var, ER_NM_hide);
  ER_set_hide (var, f);
  var = IVAL (ER_stack_vars (ER_stack (result)), BC_var_num (unget_char_bc_decl));
  ER_SET_MODE (var, ER_NM_int);
  ER_set_i (var, UCODE_BOUND);
  if (!set_conv_descs (curr_encoding_name, &byte_cd, &ucode_cd, &reverse_ucode_cd, &tp))
    eval_error (parvalue_bc_decl, call_pos (), DERR_parameter_value, ifun_name);
  set_file_encoding (ER_stack_vars (instance), curr_encoding_name, byte_cd, ucode_cd,
                     reverse_ucode_cd, tp);
}

static void two_strings_fun_start (int pars_number) {
  if (pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  to_vect_string_conversion (ctop, NULL, NULL);
  to_vect_string_conversion (below_ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
      || (ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_char
...
/* Return world representation of byte string (if BYTE_P) or unicode
   ...
   Use varr *VARR as container for the result if necessary.  */
static inline const char *general_str_to_world (void *str, VARR (char) * varr, int byte_p,
                                                conv_desc_t byte_cd, conv_desc_t unicode_cd,
                                                encoding_type_t tp, size_t *len) {
  const char *repr;

  if (byte_p)
    repr = encode_byte_str_varr ((byte_t *) str, byte_cd, tp, varr, len);
  else if (unicode_cd != NO_CONV_DESC)
    repr = encode_ucode_str_varr ((ucode_t *) str, unicode_cd, tp, varr, len);
  else
    repr = encode_ucode_str_to_raw_varr ((ucode_t *) str, varr);
  if (repr != NULL) return repr;
  eval_error (invencoding_bc_decl, call_pos (),
              byte_p || unicode_cd != NO_CONV_DESC ? DERR_in_ucode_encoding
                                                   : DERR_too_big_ucode_for_byte_representation,
              ifun_name);
}

/* Return world representation (according to current encoding) of
   vector VECT Use varr temp_varr (if FIRST_P) or temp_varr2 as
   container for the result if necessary.  */
static const char *strvect_to_world (ER_node_t vect, int first_p) {
  ER_node_mode_t el_type;
  size_t len;
  ...
  return general_str_to_world (ER_pack_els (vect), first_p ? temp_varr : temp_varr2,
                               el_type == ER_NM_byte, curr_byte_cd, curr_ucode_cd,
                               curr_encoding_type, &len);
}

void rename_call (int pars_number) {
...
static void string_fun_start (int pars_number) {
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  to_vect_string_conversion (ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
...
void remove_call (int pars_number) {
  string_fun_start (pars_number);
  if (remove (strvect_to_world (ER_vect (ctop), TRUE)) < 0) process_system_errors (REMOVE_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}
...
static int in_str_p (const char *str, int byte_p, int ch) {
  if (byte_p) {
    for (; *str; str++)
      if (*str == ch) return TRUE;
  } else {
    for (; *(const ucode_t *) str; str += sizeof (ucode_t))
      if (*(const ucode_t *) str == ch) return TRUE;
  }
  return FALSE;
}

void mkdir_call (int pars_number) {
  int mask;

  string_fun_start (pars_number);
  mask = (S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IWGRP | S_IXGRP | S_IROTH | S_IWOTH | S_IXOTH);
  if (mkdir (strvect_to_world (ER_vect (ctop), TRUE), mask) < 0) process_system_errors (MKDIR_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

void rmdir_call (int pars_number) {
  string_fun_start (pars_number);
  if (rmdir (strvect_to_world (ER_vect (ctop), TRUE)) < 0) process_system_errors (RMDIR_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

void getcwd_call (int pars_number) {
  ER_node_t vect;
  char buf[PATH_MAX + 1], *str;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GETCWD_NAME);
  str = getcwd (buf, PATH_MAX);
  if (str == NULL) process_system_errors (GETCWD_NAME);
...
void chdir_call (int pars_number) {
  string_fun_start (pars_number);
  if (chdir (strvect_to_world (ER_vect (ctop), TRUE)) < 0) process_system_errors (CHDIR_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

static void get_stat (ER_node_t var, struct stat *buf) {
  int result;

  if (ER_NODE_MODE (var) == ER_NM_vect && ER_NODE_MODE (ER_vect (var)) == ER_NM_heap_pack_vect
  ...
  else if (ER_IS_OF_TYPE (var, ER_NM_stack) && ER_stack_block (ER_stack (var)) == file_bc_decl)
    result = fstat (fileno ((FILE *) ER_hide (
                      IVAL (ER_stack_vars (ER_stack (var)), BC_var_num (file_ptr_bc_decl)))),
                    buf);
  else
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, ifun_name);
  if (result < 0) process_system_errors (ifun_name);
}

static void general_chmod (int pars_number, int clear_mask, int set_mask) {
  struct stat buf;
  int mask;
...
void chumod_call (int pars_number) {
  int mask = 0;
  const char *str;
  int byte_p;
  ...
  if (in_str_p (str, byte_p, 'r')) mask |= S_IRUSR;
  if (in_str_p (str, byte_p, 'w')) mask |= S_IWUSR;
  if (in_str_p (str, byte_p, 'x')) mask |= S_IXUSR;
  if (in_str_p (str, byte_p, 's')) mask |= S_ISVTX;
  general_chmod (pars_number, S_IRUSR | S_IWUSR | S_IXUSR | S_ISVTX, mask);
}

void chgmod_call (int pars_number) {
  int mask = 0;
  const char *str;
  int byte_p;
  ...
  byte_p = ER_pack_vect_el_mode (ER_vect (ctop)) == ER_NM_byte;
  str = ER_pack_els (ER_vect (ctop));
  if (in_str_p (str, byte_p, 'r')) mask |= S_IRGRP;
  if (in_str_p (str, byte_p, 'w')) mask |= S_IWGRP;
  if (in_str_p (str, byte_p, 'x')) mask |= S_IXGRP;
  general_chmod (pars_number, S_IRGRP | S_IWGRP | S_IXGRP, mask);
}

void chomod_call (int pars_number) {
  int mask = 0;
  const char *str;
  int byte_p;
  ...
  byte_p = ER_pack_vect_el_mode (ER_vect (ctop)) == ER_NM_byte;
  str = ER_pack_els (ER_vect (ctop));
  if (in_str_p (str, byte_p, 'r')) mask |= S_IROTH;
  if (in_str_p (str, byte_p, 'w')) mask |= S_IWOTH;
  if (in_str_p (str, byte_p, 'x')) mask |= S_IXOTH;
  general_chmod (pars_number, S_IROTH | S_IWOTH | S_IXOTH, mask);
}

static FILE *file_start (int pars_number, ER_node_t *file_instance) {
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  return get_file (pars_number, file_instance);
}

void isatty_call (int pars_number) {
  rint_t result;
  FILE *f;
...
void open_call (int pars_number) {
  FILE *f;
...
void close_call (int pars_number) {
  FILE *f;
  ...
  errno = 0;
  if ((cd = (conv_desc_t) ER_hide (var)) != NO_CONV_DESC) iconv_close (cd);
  var = IVAL (instance_vars, BC_var_num (file_ucode_ocd_bc_decl));
  if ((cd = (conv_desc_t) ER_hide (var)) != NO_CONV_DESC) iconv_close (cd);
  var = IVAL (instance_vars, BC_var_num (file_icd_bc_decl));
  if ((cd = (conv_desc_t) ER_hide (var)) != NO_CONV_DESC) iconv_close (cd);
  if (errno) process_system_errors (CLOSE_NAME);
#endif
  if (fclose (f) == EOF) process_system_errors (CLOSE_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

void flush_call (int pars_number) {
  FILE *f;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, FLUSH_NAME);
  f = get_file (pars_number, NULL);
  if (fflush (f) == EOF) process_system_errors (FLUSH_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

void popen_call (int pars_number) {
  FILE *f;
  const char *s;
  int byte_p;
  ...
  if (errno != 0) process_system_errors (POPEN_NAME);
  f = popen (strvect_to_world (ER_vect (below_ctop), TRUE), s);
  if (f == NULL) process_system_errors (POPEN_NAME);
  /* Place the result instead of the function. */
  place_file_instance (f, fun_result);
}

void pclose_call (int pars_number) {
  FILE *f;
  int res;
  ...
  errno = 0;
  res = pclose (f);
  if (res != 0 && errno) process_system_errors (PCLOSE_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

void tell_call (int pars_number) {
  FILE *f;
  rint_t pos;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, TELL_NAME);
  f = get_file (pars_number, NULL);
  pos = ftell (f);
  if (pos < 0) process_system_errors (TELL_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, pos);
}

void seek_call (int pars_number) {
  FILE *f;
  rint_t pos;
  int whence;
  int ch;
  ...
  if (pars_number != 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SEEK_NAME);
  f = get_file (pars_number, NULL);
  ...
  if (ch == 's')
    ...
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, SEEK_NAME);
  if (fseek (f, pos, whence) < 0) process_system_errors (SEEK_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_undef);
}

static void print_ch (int ch, int byte_p) {
...
/* Add representation of VAL to temp_varr which is byte (if BYTE_P)
   ...
   Return TRUE if temp_varr still contains a byte string.  */
static int print_val (ER_node_t val, int quote_flag, int full_p, int byte_p) {
  BC_node_t code;
  ER_node_t vect;
  ER_node_t tab;
  ER_node_t key;
  ...
  case ER_NM_nil: add_string_to_print ("nil", byte_p); break;
  case ER_NM_hide:
  ...
  case ER_NM_hideblock:
    ...
    for (i = 0; i < ER_hideblock_length (ER_hideblock (val)); i++) {
      if (i != 0) add_string_to_print (" ", byte_p);
      sprintf (str, "%x", (unsigned char) ER_hideblock_start (ER_hideblock (val))[i]);
      add_string_to_print (str, byte_p);
    }
    add_string_to_print (")", byte_p);
    break;
  case ER_NM_char:
    if (!quote_flag) {
      ...
      if (in_byte_range_p (ch) && byte_p) {
        ...
        VARR_PUSH_ARR (char, temp_varr, &b, sizeof (byte_t));
      } else {
        if (byte_p) {
        ...
        VARR_PUSH_ARR (char, temp_varr, (char *) &ch, sizeof (ucode_t));
      }
    } else {
      add_string_to_print ("\'", byte_p);
  ...
  case ER_NM_vect:
    ...
    if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect
      ...
      if (byte_p && ER_pack_vect_el_mode (vect) == ER_NM_char) {
        copy_varr (temp_varr2, temp_varr);
        str_to_ucode_varr (temp_varr, VARR_ADDR (char, temp_varr2), VARR_LENGTH (char, temp_varr2));
        byte_p = FALSE;
      }
      if (!quote_flag) {
        if (ER_pack_vect_el_mode (vect) == ER_NM_byte)
          ...
          VARR_PUSH_ARR (char, temp_varr, (char *) ER_pack_els (vect),
                         ucodestrlen ((ucode_t *) ER_pack_els (vect)));
      } else {
        add_string_to_print ("\"", byte_p);
        if (ER_pack_vect_el_mode (vect) == ER_NM_byte)
          for (string = (char *) ER_pack_els (vect); *string != '\0'; string++)
            print_ch (*(byte_t *) string, byte_p);
        else
          for (string = ER_pack_els (vect); *(ucode_t *) string != '\0'; string += sizeof (ucode_t))
      ...
      for (i = 0; i < ER_els_number (vect); i++) {
        ...
        byte_p = print_val (IVAL (ER_unpack_els (vect), i), TRUE, TRUE, byte_p);
        if (i < ER_els_number (vect) - 1) add_string_to_print (", ", byte_p);
      }
      add_string_to_print ("]", byte_p);
    } else {
      ...
      for (i = 0; i < ER_els_number (vect); i++) {
        ...
        memcpy ((char *) &temp_val + displ, (char *) ER_pack_els (vect) + i * el_size, el_size);
  ...
  case ER_NM_tab:
    ...
    for (num = i = 0; i < ER_els_bound (tab); i++) {
      ...
      if (ER_NODE_MODE (key) == ER_NM_empty_el) continue;
      num++;
      if (repl_flag && num > MAX_REPL_PRINTED_ELEMENTS) {
        add_string_to_print (", ...", byte_p);
        break;
      }
      if (flag) add_string_to_print (", ", byte_p);
      byte_p = print_val (key, TRUE, TRUE, byte_p);
      add_string_to_print (":", byte_p);
      byte_p = print_val (INDEXED_EL_VAL (ER_tab_els (tab), i), TRUE, TRUE, byte_p);
      flag = TRUE;
    }
    add_string_to_print ("]", byte_p);
    break;
  case ER_NM_code:
    ...
    if (print_context (ER_code_context (val), byte_p)) add_string_to_print (".", byte_p);
  ...
  case ER_NM_thread:
    if (ER_thread_block (ER_thread (val)) == NULL)
      ...
      for (stack = ER_saved_cstack (ER_thread (val)); stack != NULL; stack = ER_prev_stack (stack))
        if (BC_NODE_MODE (ER_stack_block (stack)) == BC_NM_fblock
            && BC_fiber_p (ER_stack_block (stack)))
          break;
      sprintf (str, "thread %ld ", (long int) ER_thread_number (ER_thread (val)));
      add_string_to_print (str, byte_p);
      if (!print_context (stack, byte_p)) d_unreachable ();
    }
    break;
  case ER_NM_type:
    switch (ER_type (val)) {
    case type_nil: string = "type (nil)"; break;
    case type_char: string = "char"; break;
    case type_int: string = "int"; break;
    case type_long: string = "long"; break;
    case type_float: string = "float"; break;
    case type_hide: string = "hide"; break;
    case type_hideblock: string = "hideblock"; break;
    case type_vect: string = "vector"; break;
    case type_tab: string = "table"; break;
    case type_fun: string = "fun"; break;
    case type_class: string = "class"; break;
    case type_fiber: string = "fiber"; break;
    case type_obj: string = "obj"; break;
    case type_thread: string = "thread"; break;
    case type_type: string = "type"; break;
    default: d_unreachable ();
    }
...
static inline void end_printed_string (int byte_p, int ln_flag) {
  if (byte_p) {
    if (ln_flag) VARR_PUSH (char, temp_varr, '\n');
    VARR_PUSH (char, temp_varr, 0);
  } else {
    ucode_t uc[] = {'\n', 0};

    if (ln_flag)
      VARR_PUSH_ARR (char, temp_varr, (char *) uc, sizeof (ucode_t) * 2);
    else
      VARR_PUSH_ARR (char, temp_varr, (char *) &uc[1], sizeof (ucode_t));
  }
}

void repl_print (ER_node_t val, int def_p) {
  int byte_p;
  size_t len;
  const char *str;

  if (def_p && ER_NODE_MODE (val) == ER_NM_undef) return;
  VARR_TRUNC (char, temp_varr, 0);
  byte_p = print_val (val, TRUE, FALSE, TRUE);
  end_printed_string (byte_p, TRUE);
  str = general_str_to_world (VARR_ADDR (char, temp_varr), temp_varr2, byte_p, curr_byte_cd,
                              curr_ucode_cd, curr_encoding_type, &len);
  fwrite (str, sizeof (char), len, stdout);
}

static FILE *file_function_call_start (int pars_number, ER_node_t *file_instance) {
  if (pars_number == 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  return get_file (pars_number, file_instance);
}

enum file_param_type { NO_FILE, STANDARD_FILE, GIVEN_FILE };

/* Output byte (if BYTE_P) or ucode string in temp_varr to file F
   ...
   occurs.  */
static int finish_output (FILE *f, int byte_p, conv_desc_t byte_cd, conv_desc_t unicode_cd,
                          encoding_type_t tp) {
  ER_node_t vect;

  if (f != NULL) {
    size_t len;
    const char *str = general_str_to_world (VARR_ADDR (char, temp_varr), temp_varr2, byte_p,
    ...
    vect = create_string (VARR_ADDR (char, temp_varr));
  else
    vect = create_ucodestr ((ucode_t *) VARR_ADDR (char, temp_varr));
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_vect);
  set_vect_dim (fun_result, vect, 0);
  return 0;
...
static void general_put_call (FILE *f, int pars_number, int ln_flag,
                              enum file_param_type param_type, conv_desc_t byte_cd,
                              conv_desc_t unicode_cd, encoding_type_t tp) {
  ...
  for (i = -pars_number + (param_type == GIVEN_FILE ? 1 : 0) + 1; i <= 0; i++) {
    ...
    if (res_byte_p && !byte_p) {
      ...
      str_to_ucode_varr (temp_varr2, start, strlen (start));
      start = VARR_ADDR (char, temp_varr2);
      byte_p = FALSE;
    }
    ch_size = byte_p ? sizeof (byte_t) : sizeof (ucode_t);
    VARR_PUSH_ARR (char, temp_varr, start, ER_els_number (ER_vect (var)) * ch_size);
  }
  end_printed_string (res_byte_p, ln_flag);
  if (finish_output (f, res_byte_p, byte_cd, unicode_cd, tp)) process_system_errors (ifun_name);
}

void put_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdout_bc_decl)));

  get_file_output_cds (file_instance, &byte_cd, &ucode_cd);
  general_put_call (stdout, pars_number, FALSE, STANDARD_FILE, byte_cd, ucode_cd,
                    get_file_encoding_type (file_instance));
}

void putln_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdout_bc_decl)));

  get_file_output_cds (file_instance, &byte_cd, &ucode_cd);
  general_put_call (stdout, pars_number, TRUE, STANDARD_FILE, byte_cd, ucode_cd,
                    get_file_encoding_type (file_instance));
}

void fput_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance;
...
void fputln_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance;
  FILE *f = file_function_call_start (pars_number, &file_instance);
...
void sput_call (int pars_number) {
  general_put_call (NULL, pars_number, FALSE, NO_FILE, NO_CONV_DESC, NO_CONV_DESC, OTHER_ENC);
}

void sputln_call (int pars_number) {
  general_put_call (NULL, pars_number, TRUE, NO_FILE, NO_CONV_DESC, NO_CONV_DESC, OTHER_ENC);
}

static void general_print_call (FILE *f, int pars_number, int ln_flag,
                                enum file_param_type param_type, conv_desc_t byte_cd,
                                conv_desc_t ucode_cd, encoding_type_t tp) {
  int i, byte_p;
  ...
  if (finish_output (f, byte_p, byte_cd, ucode_cd, tp)) process_system_errors (ifun_name);
}

void print_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdout_bc_decl)));
...
void println_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdout_bc_decl)));

  get_file_output_cds (file_instance, &byte_cd, &ucode_cd);
  general_print_call (stdout, pars_number, TRUE, STANDARD_FILE, byte_cd, ucode_cd,
                      get_file_encoding_type (file_instance));
}

void fprint_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance;
  FILE *f = file_function_call_start (pars_number, &file_instance);
...
void fprintln_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance;
  FILE *f = file_function_call_start (pars_number, &file_instance);
...
void sprint_call (int pars_number) {
  general_print_call (NULL, pars_number, FALSE, NO_FILE, NO_CONV_DESC, NO_CONV_DESC, OTHER_ENC);
}

void sprintln_call (int pars_number) {
  general_print_call (NULL, pars_number, TRUE, NO_FILE, NO_CONV_DESC, NO_CONV_DESC, OTHER_ENC);
}

static inline int get_file_char (FILE *f, rint_t *unget_char_ptr, conv_desc_t cd,
                                 encoding_type_t tp) {
  int uc;
  ...
  if (cd == NO_CONV_DESC)
    ...
    if (errno != 0) process_system_errors (ifun_name);
  }
  if (uc == UCODE_BOUND)
    eval_error (invencoding_bc_decl, call_pos (), DERR_unexpected_input_encoding, ifun_name);
  return uc;
}

static inline void unget_file_char (int ch, FILE *f, rint_t *unget_char_ptr) {
  d_assert (*unget_char_ptr == UCODE_BOUND);
  *unget_char_ptr = ch;
}

static void general_get_call (FILE *f, rint_t *unget_char_ptr, int file_flag, conv_desc_t cd,
                              encoding_type_t tp) {
  ucode_t ch;
  ...
  if (ch == UCODE_BOUND)
    eval_error (invencoding_bc_decl, call_pos (), DERR_unexpected_input_encoding, ifun_name);
  if (ch == EOF)
    eval_error (eof_bc_decl, call_pos (), DERR_eof_occured, file_flag ? FGET_NAME : GET_NAME);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_char);
  ER_set_ch (fun_result, ch);
}

static void general_get_ln_file_call (FILE *f, rint_t *unget_char_ptr, int param_flag, int ln_flag,
                                      int as_lns_p, conv_desc_t cd, encoding_type_t tp) {
  ER_node_t vect;
  int ch;
  ucode_t u;
  ...
  VARR_TRUNC (char, temp_varr, 0);
  if (!ln_flag && as_lns_p) VARR_TRUNC (char, temp_varr2, 0);
  ch_n = 0;
  for (;;) {
    ch = get_file_char (f, unget_char_ptr, cd, tp);
    if (ch == UCODE_BOUND)
    ...
    if ((ch == '\n' && (ln_flag || as_lns_p)) || ch == EOF) {
      if (ln_flag || !as_lns_p || ch == '\n' || VARR_LENGTH (char, temp_varr) != 0) {
        if (cd == NO_CONV_DESC) {
          VARR_PUSH (char, temp_varr, '\0');
          vect = create_string (VARR_ADDR (char, temp_varr));
        } else {
          u = '\0';
          VARR_PUSH_ARR (char, temp_varr, (char *) &u, sizeof (ucode_t));
          vect = create_ucodestr ((ucode_t *) VARR_ADDR (char, temp_varr));
        }
        if (!ln_flag && as_lns_p) {
          VARR_TRUNC (char, temp_varr, 0);
          VARR_PUSH_ARR (char, temp_varr2, (char *) &vect, sizeof (vect));
        }
      }
      if (ln_flag || ch == EOF) break;
    } else if (cd == NO_CONV_DESC)
      VARR_PUSH (char, temp_varr, ch);
    else {
      u = ch;
      VARR_PUSH_ARR (char, temp_varr, (char *) &u, sizeof (ucode_t));
    }
  ...
  if (!ln_flag && as_lns_p) {
    ...
    for (i = 0; i < els_number; i++)
      set_packed_vect_el (vect, i, ((ER_node_t *) VARR_ADDR (char, temp_varr2))[i]);
  }
  if (ch == EOF && ch_n == 0) eval_error (eof_bc_decl, call_pos (), DERR_eof_occured, ifun_name);
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_vect);
  set_vect_dim (fun_result, vect, 0);
}

void get_call (int pars_number) {
  ER_node_t file_instance;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GET_NAME);
  file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdin_bc_decl)));
  general_get_call (stdin, get_file_unget_char_ptr (file_instance), FALSE,
                    get_file_input_cd (file_instance), get_file_encoding_type (file_instance));
}

void getln_call (int pars_number) {
  ER_node_t file_instance;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GETLN_NAME);
  file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdin_bc_decl)));
  general_get_ln_file_call (stdin, get_file_unget_char_ptr (file_instance), FALSE, TRUE, FALSE,
                            get_file_input_cd (file_instance),
                            get_file_encoding_type (file_instance));
}

void getf_call (int pars_number) {
  ER_node_t file_instance;
  int flag = 0;

  if (pars_number > 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GETF_NAME);
  if (pars_number == 1) {
    implicit_int_conversion (ctop, NULL);
    if (!ER_IS_OF_TYPE (ctop, ER_NM_int))
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, GETF_NAME);
    flag = ER_i (ctop);
  }
  file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdin_bc_decl)));
  general_get_ln_file_call (stdin, get_file_unget_char_ptr (file_instance), FALSE, FALSE, flag != 0,
                            get_file_input_cd (file_instance),
                            get_file_encoding_type (file_instance));
}

static FILE *fget_function_call_start (int pars_number, ER_node_t *file_instance) {
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  return get_file (pars_number, file_instance);
}

void fget_call (int pars_number) {
  ER_node_t file_instance;
  FILE *f = fget_function_call_start (pars_number, &file_instance);

  general_get_call (f, get_file_unget_char_ptr (file_instance), TRUE,
                    get_file_input_cd (file_instance), get_file_encoding_type (file_instance));
}

void fgetln_call (int pars_number) {
  ER_node_t file_instance;
  FILE *f = fget_function_call_start (pars_number, &file_instance);

  general_get_ln_file_call (f, get_file_unget_char_ptr (file_instance), TRUE, TRUE, FALSE,
                            get_file_input_cd (file_instance),
                            get_file_encoding_type (file_instance));
}

void fgetf_call (int pars_number) {
  int flag = 0;
  ER_node_t file_instance;
  FILE *f;
  ...
  if (pars_number == 2) {
    implicit_int_conversion (ctop, NULL);
    if (!ER_IS_OF_TYPE (ctop, ER_NM_int))
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, FGETF_NAME);
    flag = ER_i (ctop);
  } else if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, FGETF_NAME);
  f = get_file (pars_number, &file_instance);
  general_get_ln_file_call (f, get_file_unget_char_ptr (file_instance), TRUE, FALSE, flag != 0,
                            get_file_input_cd (file_instance),
                            get_file_encoding_type (file_instance));
}
...
/* Var length string used by func yylval for text presentation of the
   symbol. */
static VARR (char) * el_text;

static void initiate_io (void) { VARR_CREATE (char, el_text, 0); }

static void finish_io (void) { VARR_DESTROY (char, el_text); }
...
/* The following function is analogous to `read_dino_string_code' in
   ...
   this function too. */
static int get_char_code (FILE *f, rint_t *unget_char_ptr, conv_desc_t cd, encoding_type_t tp,
  ...
  *correct_newln = *wrong_escape_code = FALSE;
  if (curr_char == '\\') {
    ...
    if (curr_char == 'n')
      ...
      *correct_newln = TRUE;
    else if (curr_char <= UCHAR_MAX && isdigit_ascii (curr_char) && curr_char != '8'
             && curr_char != '9') {
      char_code = value_of_digit (curr_char);
      curr_char = get_file_char (f, unget_char_ptr, cd, tp);
      if (curr_char > UCHAR_MAX || !isdigit_ascii (curr_char) || curr_char == '8'
          || curr_char == '9')
        unget_file_char (curr_char, f, unget_char_ptr);
      else {
      ...
      char_code = 0;
      for (i = (curr_char == 'x' ? 2 : curr_char == 'u' ? 4 : 8); i > 0; i--) {
        curr_char = get_file_char (f, unget_char_ptr, cd, tp);
        if (curr_char > UCHAR_MAX || !is_hex_digit (curr_char)) break;
...
static void invinput_error (FILE *f, rint_t *unget_char_ptr, conv_desc_t cd, encoding_type_t tp,
                            int ln_flag) {
  int curr_char;
...
static int n_getc (void) {
  return get_file_char (number_file, number_unget_char_ptr, number_file_cd,
                        number_file_encoding_type);
}

static void n_ungetc (int c) { unget_file_char (c, number_file, number_unget_char_ptr); }

/* The following function is analogous to `yylex' in Dino scanner.  If
   `yylex' is changed, please modify this function too. */
static struct token get_token (FILE *f, rint_t *unget_char_ptr, conv_desc_t cd, encoding_type_t tp,
                               int ln_flag) {
  int curr_char;
  int wrong_escape_code;
  struct token result;

  VARR_TRUNC (char, el_text, 0);
  for (;;) {
    curr_char = get_file_char (f, unget_char_ptr, cd, tp);
    ...
    case '[':
    case ']':
    case EOF: result.token_code = curr_char; return result;
    case 't':
      curr_char = get_file_char (f, unget_char_ptr, cd, tp);
      if (curr_char != 'a') invinput_error (f, unget_char_ptr, cd, tp, ln_flag);
      curr_char = get_file_char (f, unget_char_ptr, cd, tp);
      if (curr_char != 'b') invinput_error (f, unget_char_ptr, cd, tp, ln_flag);
    ...
    case '\'': {
      ...
      if (curr_char == '\'')
        ...
        curr_char = get_char_code (f, unget_char_ptr, cd, tp, curr_char, &correct_newln,
                                   &wrong_escape_code);
    ...
    case '\"': {
      ...
      for (;;) {
        ...
        if (no_escape_p || !correct_newln) VARR_PUSH (char, el_text, curr_char);
      }
      VARR_PUSH (char, el_text, '\0');
      result.val.str = VARR_ADDR (char, el_text);
    ...
    default: {
      ...
      if ((curr_char <= UCHAR_MAX && isdigit_ascii (curr_char))
        ...
        err_code = read_dino_number (curr_char, n_getc, n_ungetc, &read_ch_num, &repr, &base,
                                     &float_p, &long_p);
        if (err_code != NUMBER_OK) {
          ...
          if (errno) process_system_errors ("string-to-float conversion");
        } else {
          result.token_code = F_INT;
          result.val.i = a2i (repr, base);
          if (errno) process_system_errors ("string-to-int conversion");
        }
        return result;
      } else
        invinput_error (f, unget_char_ptr, cd, tp, ln_flag);
...
/* This resursive function reads a DINO value according to the
   ...
   function too. */
static val_t scanel (FILE *f, rint_t *unget_char_ptr, conv_desc_t cd, encoding_type_t tp,
                     struct token token, int ln_flag) {
  val_t result;
  ...
  case '[': {
    ...
    for (;;) {
      ...
      if (token.token_code == ':') {
        ...
        if (ER_NODE_MODE (ptr) != ER_NM_int) invinput_error (f, unget_char_ptr, cd, tp, ln_flag);
        repeat = ER_i (ptr);
        ...
        repeat = 1;
      if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect) vect = unpack_vector (vect);
      ...
      if (token.token_code == ',') token = get_token (f, unget_char_ptr, cd, tp, ln_flag);
    }
  }
  case F_TAB: {
    ER_node_t tab;
    ...
    tab = create_tab (40);
    token = get_token (f, unget_char_ptr, cd, tp, ln_flag);
    if (token.token_code != '[') invinput_error (f, unget_char_ptr, cd, tp, ln_flag);
    token = get_token (f, unget_char_ptr, cd, tp, ln_flag);
    for (;;) {
      if (token.token_code == ']') {
  ...
  default: invinput_error (f, unget_char_ptr, cd, tp, ln_flag);
  }
  d_unreachable ();
}

static void general_scan_call (FILE *f, rint_t *unget_char_ptr, conv_desc_t cd, encoding_type_t tp,
                               int file_flag, int ln_flag) {
  struct token token;
  val_t val;
  int curr_char;

  token = get_token (f, unget_char_ptr, cd, tp, ln_flag);
  if (token.token_code == EOF) eval_error (eof_bc_decl, call_pos (), DERR_eof_occured, ifun_name);
...
void scan_call (int pars_number) {
  ER_node_t file_instance;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SCAN_NAME);
  file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdin_bc_decl)));
...
void scanln_call (int pars_number) {
  ER_node_t file_instance;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SCANLN_NAME);
  file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdin_bc_decl)));
  general_scan_call (stdin, get_file_unget_char_ptr (file_instance),
                     get_file_input_cd (file_instance), get_file_encoding_type (file_instance),
...
void fscan_call (int pars_number) {
  ER_node_t file_instance;
  FILE *f = fget_function_call_start (pars_number, &file_instance);

  general_scan_call (f, get_file_unget_char_ptr (file_instance), get_file_input_cd (file_instance),
                     get_file_encoding_type (file_instance), TRUE, FALSE);
}

void fscanln_call (int pars_number) {
  ER_node_t file_instance;
  FILE *f = fget_function_call_start (pars_number, &file_instance);
...
static void int_function_end (rint_t result, int pars_number) {
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, result);
}

static void function_without_par (int pars_number) {
  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
}

void getpid_call (int pars_number) {
  function_without_par (pars_number);
  int_function_end (getpid (), pars_number);
}

static void str_function_end (char *result, int pars_number) {
  ER_node_t vect;
...
static void general_putf_call (FILE *f, int pars_number, enum file_param_type param_type,
                               conv_desc_t byte_cd, conv_desc_t unicode_cd, encoding_type_t tp) {
  ER_node_t val;
  int start, byte_p;

  start = 0;
  d_assert (param_type != NO_FILE || f == NULL);
  if (param_type == GIVEN_FILE) start = 1;
  ...
  if (ER_NODE_MODE (val) != ER_NM_vect || ER_NODE_MODE (ER_vect (val)) != ER_NM_heap_pack_vect
    ...
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, ifun_name);
  byte_p = form_format_string (ER_vect (val), IVAL (ctop, -pars_number + 2 + start),
                               pars_number - 1 - start, ifun_name, FALSE);
  if (finish_output (f, byte_p, byte_cd, unicode_cd, tp)) process_system_errors (ifun_name);
}

void putf_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance
    = ER_stack (IVAL (ER_stack_vars (get_obj_stack (io_bc_decl)), BC_var_num (stdout_bc_decl)));
...
void fputf_call (int pars_number) {
  conv_desc_t byte_cd, ucode_cd;
  ER_node_t file_instance;
  FILE *f = file_function_call_start (pars_number, &file_instance);
...
void sputf_call (int pars_number) {
  general_putf_call (NULL, pars_number, NO_FILE, NO_CONV_DESC, NO_CONV_DESC, OTHER_ENC);
}

void getun_call (int pars_number) {
  function_without_par (pars_number);
  str_function_end (getun (), pars_number);
}

void geteun_call (int pars_number) {
  function_without_par (pars_number);
  str_function_end (geteun (), pars_number);
}

void getgn_call (int pars_number) {
  function_without_par (pars_number);
  str_function_end (getgn (), pars_number);
}

void getegn_call (int pars_number) {
  function_without_par (pars_number);
  str_function_end (getegn (), pars_number);
}

void getgroups_call (int pars_number) {
  ER_node_t vect;
  size_t els_number, grs_n;
  size_t i;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GETGROUPS_NAME);
#if defined(HAVE_GETGROUPS)
  els_number = getgroups (0, NULL);
  VARR_TRUNC (char, temp_varr, 0);
  VARR_GROW (char, temp_varr, sizeof (GETGROUPS_T) * els_number);
  if (getgroups (els_number, (GETGROUPS_T *) VARR_ADDR (char, temp_varr)) < 0)
    process_system_errors (GETGROUPS_NAME);
  for (grs_n = i = 0; i < els_number; i++)
    if (getgrgid (((GETGROUPS_T *) VARR_ADDR (char, temp_varr))[i]) != NULL) grs_n++;
  if (grs_n == 0)
    vect = create_empty_vector ();
  else {
  ...
  if (grs_n != 0)
    for (grs_n = i = 0; i < els_number; i++) {
      ...
      gr = getgrgid (((GETGROUPS_T *) VARR_ADDR (char, temp_varr))[i]);
      if (gr != NULL) {
...
static void do_inline float_function_start (int pars_number) {
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
...
static void do_inline float_function_start2 (int pars_number) {
  if (pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  implicit_float_conversion (ctop, NULL);
  implicit_float_conversion (below_ctop, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_float || ER_NODE_MODE (below_ctop) != ER_NM_float)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, ifun_name);
  errno = 0;
}
...
static void general_rand_call (int pars_number, int rand_flag) {
  ...
  if (!rand_flag && pars_number == 1) {
    ...
    if (ER_NODE_MODE (ctop) == ER_NM_int)
      ...
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, SRAND_NAME);
  }
  if (rand_flag) {
    /* Place the result instead of the function. */
    ER_SET_MODE (fun_result, ER_NM_float);
...
void rand_call (int pars_number) { general_rand_call (pars_number, TRUE); }

void srand_call (int pars_number) { general_rand_call (pars_number, FALSE); }

void process_system_errors (const char *fname) {
  switch (errno) {
#ifdef EACCES
  case EACCES:
    /* Permission denied. */
...
#endif
#ifdef EAGAIN
  case EAGAIN: eval_error (eagain_bc_decl, call_pos (), DERR_eagain, fname); break;
#endif
#ifdef EBADF
  case EBADF: eval_error (ebadf_bc_decl, call_pos (), DERR_ebadf, fname); break;
#endif
#ifdef EBUSY
  case EBUSY: eval_error (ebusy_bc_decl, call_pos (), DERR_ebusy, fname); break;
#endif
#ifdef ECHILD
  case ECHILD: eval_error (echild_bc_decl, call_pos (), DERR_echild, fname); break;
#endif
#ifdef EDEADLK
  case EDEADLK: eval_error (edeadlk_bc_decl, call_pos (), DERR_edeadlk, fname); break;
#endif
#ifdef EDOM
  case EDOM: eval_error (edom_bc_decl, call_pos (), DERR_edom, fname); break;
#endif
#ifdef EEXIST
  case EEXIST: eval_error (eexist_bc_decl, call_pos (), DERR_eexist, fname); break;
#endif
#ifdef EFAULT
  case EFAULT: eval_error (efault_bc_decl, call_pos (), DERR_efault, fname); break;
#endif
#ifdef EFBIG
  case EFBIG: eval_error (efbig_bc_decl, call_pos (), DERR_efbig, fname); break;
#endif
#ifdef EINTR
  case EINTR: eval_error (eintr_bc_decl, call_pos (), DERR_eintr, fname); break;
#endif
#ifdef EINVAL
  case EINVAL: eval_error (einval_bc_decl, call_pos (), DERR_einval, fname); break;
#endif
#ifdef EIO
  case EIO: eval_error (eio_bc_decl, call_pos (), DERR_eio, fname); break;
#endif
#ifdef EISDIR
  case EISDIR: eval_error (eisdir_bc_decl, call_pos (), DERR_eisdir, fname); break;
#endif
#ifdef EMFILE
  case EMFILE: eval_error (emfile_bc_decl, call_pos (), DERR_emfile, fname); break;
#endif
#ifdef EMLINK
  case EMLINK: eval_error (emlink_bc_decl, call_pos (), DERR_emlink, fname); break;
#endif
#ifdef ENAMETOOLONG
  case ENAMETOOLONG:
    eval_error (enametoolong_bc_decl, call_pos (), DERR_enametoolong, fname);
    break;
#endif
#ifdef ENFILE
  case ENFILE: eval_error (enfile_bc_decl, call_pos (), DERR_enfile, fname); break;
#endif
#ifdef ENODEV
  case ENODEV: eval_error (enodev_bc_decl, call_pos (), DERR_enodev, fname); break;
#endif
#ifdef ENOENT
  case ENOENT:
...
#endif
#ifdef ENOEXEC
  case ENOEXEC: eval_error (enoexec_bc_decl, call_pos (), DERR_enoexec, fname); break;
#endif
#ifdef ENOLCK
  case ENOLCK: eval_error (enolck_bc_decl, call_pos (), DERR_enolck, fname); break;
#endif
#ifdef ENOMEM
  case ENOMEM: eval_error (enomem_bc_decl, call_pos (), DERR_enomem, fname); break;
#endif
#ifdef ENOSPC
  case ENOSPC: eval_error (enospc_bc_decl, call_pos (), DERR_enospc, fname); break;
#endif
#ifdef ENOSYS
  case ENOSYS: eval_error (enosys_bc_decl, call_pos (), DERR_enosys, fname); break;
#endif
#ifdef ENOTDIR
  case ENOTDIR:
  ...
  case ENOTEMPTY: eval_error (enotempty_bc_decl, call_pos (), DERR_enotempty, fname); break;
#endif
#endif
#ifdef ENOTTY
  case ENOTTY: eval_error (enotty_bc_decl, call_pos (), DERR_enotty, fname); break;
#endif
#ifdef ENXIO
  case ENXIO: eval_error (enxio_bc_decl, call_pos (), DERR_enxio, fname); break;
#endif
#ifdef EPERM
  case EPERM: eval_error (eperm_bc_decl, call_pos (), DERR_eperm, fname); break;
#endif
#ifdef EPIPE
  case EPIPE: eval_error (epipe_bc_decl, call_pos (), DERR_epipe, fname); break;
#endif
#ifdef ERANGE
  case ERANGE: eval_error (erange_bc_decl, call_pos (), DERR_erange, fname); break;
#endif
#ifdef EROFS
  case EROFS: eval_error (erofs_bc_decl, call_pos (), DERR_erofs, fname); break;
#endif
#ifdef ESPIPE
  case ESPIPE: eval_error (espipe_bc_decl, call_pos (), DERR_espipe, fname); break;
#endif
#ifdef ESRCH
  case ESRCH: eval_error (esrch_bc_decl, call_pos (), DERR_esrch, fname); break;
#endif
#ifdef EXDEV
  case EXDEV: eval_error (exdev_bc_decl, call_pos (), DERR_exdev, fname); break;
#endif
  default:
...
void process_errno_call (int pars_number) {
  const char *name;

  if (pars_number > 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, PROCESS_ERRNO_NAME);
  if (pars_number == 0)
    name = "";
  else {
  ...
  if (errno) process_system_errors (name);
  ER_SET_MODE (fun_result, ER_NM_undef);
}

void readdir_call (int pars_number) {
  ER_node_t result;
  ER_node_t vect;
  DIR *dir;
  ...
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, READDIR_NAME);
  to_vect_string_conversion (ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
      || (ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_char
          && ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_byte))
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, READDIR_NAME);
  ...
  if (dir == NULL)
    ...
    if (errno != 0)
      ...
      eval_error (internal_bc_decl, call_pos (), DERR_internal_error, READDIR_NAME);
    if (closedir (dir) != 0) /* Internall error: EBADF and may be something else. */
      eval_error (internal_bc_decl, call_pos (), DERR_internal_error, READDIR_NAME);
    dir = opendir (strvect_to_world (ER_vect (ctop), TRUE));
    if (dir == NULL)
      process_system_errors (READDIR_NAME);
      ...
      for (i = 0; i < dir_files_number; i++) {
        ...
        ucode_str
          = (ucode_t *) encode_byte_str_varr ((byte_t *) dirent->d_name, curr_reverse_ucode_cd,
                                              OTHER_ENC, temp_varr, &len);
        if (ucode_str == NULL)
          eval_error (invencoding_bc_decl, call_pos (), DERR_unexpected_input_encoding, ifun_name);
...
static void stat_start (int pars_number, struct stat *buf) {
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, ifun_name);
  to_vect_string_conversion (ctop, NULL, NULL);
  get_stat (ctop, buf);
...
void ftype_call (int pars_number) {
  struct stat buf;
  int result;
...
static void stat_finish (int pars_number, rint_t result) {
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, result);
}

void fuidn_call (int pars_number) {
  struct stat buf;
  ER_node_t result;
  char *str;
  struct passwd *p;
...
void fgrpn_call (int pars_number) {
  struct stat buf;
  ER_node_t result;
...
void fsize_call (int pars_number) {
  struct stat buf;
  rint_t result;
...
void fatime_call (int pars_number) {
  struct stat buf;
  rint_t result;
...
void fmtime_call (int pars_number) {
  struct stat buf;
  rint_t result;
...
void fctime_call (int pars_number) {
  struct stat buf;
  rint_t result;
...
static void mode_finish (int pars_number, const char *result) {
  ER_node_t vect;
...
void fumode_call (int pars_number) {
  struct stat buf;
  char result[5];
  char *str = result;

  stat_start (pars_number, &buf);
  if (buf.st_mode & S_ISVTX) *str++ = 's';
  if (buf.st_mode & S_IRUSR) *str++ = 'r';
  if (buf.st_mode & S_IWUSR) *str++ = 'w';
  if (buf.st_mode & S_IXUSR) *str++ = 'x';
  *str = '\0';
  mode_finish (pars_number, result);
}

void fgmode_call (int pars_number) {
  struct stat buf;
  char result[5];
  char *str = result;

  stat_start (pars_number, &buf);
  if (buf.st_mode & S_IRGRP) *str++ = 'r';
  if (buf.st_mode & S_IWGRP) *str++ = 'w';
  if (buf.st_mode & S_IXGRP) *str++ = 'x';
  *str = '\0';
  mode_finish (pars_number, result);
}

void fomode_call (int pars_number) {
  struct stat buf;
  char result[5];
  char *str = result;

  stat_start (pars_number, &buf);
  if (buf.st_mode & S_IROTH) *str++ = 'r';
  if (buf.st_mode & S_IWOTH) *str++ = 'w';
  if (buf.st_mode & S_IXOTH) *str++ = 'x';
  *str = '\0';
  mode_finish (pars_number, result);
}

void time_call (int pars_number) {
  time_t t;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, TIME_NAME);
  t = time (NULL);
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, t);
}

void strtime_call (int pars_number) {
  time_t t;
  struct tm *tm;
  const char *format;
  const char *str;
  ...
  if (pars_number > 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, STRTIME_NAME);
  if (pars_number == 2) {
    implicit_int_conversion (ctop, NULL);
    if (ER_NODE_MODE (ctop) != ER_NM_int)
      eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, STRTIME_NAME);
    t = ER_i (ctop);
  } else
    t = time (NULL);
  if (pars_number >= 1) {
    ...
    format_var
      = IVAL (ER_stack_vars (get_obj_stack (sys_bc_decl)), BC_var_num (time_format_bc_decl));
    to_vect_string_conversion (format_var, NULL, NULL);
    if (ER_NODE_MODE (format_var) == ER_NM_vect
        && ER_NODE_MODE (ER_vect (format_var)) == ER_NM_heap_pack_vect
        && ER_pack_vect_el_mode (ER_vect (format_var)) == ER_NM_byte)
      ...
      eval_error (invenvar_bc_decl, call_pos (), DERR_corrupted_environment_var, TIME_FORMAT_NAME);
  }
  tm = localtime (&t);
  for (percents_number = 0, str = format; *str != 0; str++)
    if (*str == '%') percents_number++;
...
void clock_call (int pars_number) {
  rfloat_t secs;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, CLOCK_NAME);
  secs = (rfloat_t) (clock () - start_time) / CLOCKS_PER_SECOND;
  ER_SET_MODE (fun_result, ER_NM_float);
  ER_set_f (fun_result, secs);
}

void gc_call (int pars_number) {
  ptrdiff_t offset;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GC_NAME);
...
void system_call (int pars_number) {
  int code;
  int error_flag;
  ER_node_t val;
  ER_node_t vect;
  ...
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SYSTEM_NAME);
  else {
    val = IVAL (ctop, -pars_number + 1);
    if (ER_NODE_MODE (val) != ER_NM_vect)
      ...
      if (ER_NODE_MODE (vect) != ER_NM_heap_pack_vect
        ...
        if (code == 127)
          eval_error (noshell_bc_decl, call_pos (), DERR_no_shell, SYSTEM_NAME);
        else if (code < 0)
          eval_error (systemfail_bc_decl, call_pos (), DERR_other_fail_in_system_call, SYSTEM_NAME);
        error_flag = FALSE;
      }
    }
    if (error_flag) eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, SYSTEM_NAME);
  }
  /* Place the result instead of the function. */
  ER_SET_MODE (fun_result, ER_NM_int);
...
/* Array of pointers to functions/calls which will be reported in the
   trace of calls.  */
VARR (trace_stack_elem_t) * trace_stack;

/* Output info about stack trace.  */
void print_trace_stack (void) {
  for (size_t i = 0; i < VARR_LENGTH (trace_stack_elem_t, trace_stack); i++) {
    struct trace_stack_elem elem = VARR_GET (trace_stack_elem_t, trace_stack, i);

    fprintf (stderr, "%s:%u:%u:calling %s\n", get_pos (elem.pc).file_name,
             get_pos (elem.pc).line_number, get_pos (elem.pc).column_number,
             BC_ident (BC_fdecl (elem.block)));
  }
}

void exit_call (int pars_number) {
  BC_node_t block;
  ER_node_t stack;
  struct trace_stack_elem elem;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, EXIT_NAME);
  implicit_int_conversion (ctop, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_int)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, EXIT_NAME);
  if (trace_flag) {
    VARR_TRUNC (trace_stack_elem_t, trace_stack, 0);

    for (stack = cstack; stack != uppest_stack; stack = ER_prev_stack (stack)) {
      block = ER_block_node (stack);
      if (BC_NODE_MODE (block) == BC_NM_block) continue;
      elem.block = block;
      elem.pc = ER_call_pc (stack);
      VARR_PUSH (trace_stack_elem_t, trace_stack, elem);
    }
  }
  dino_finish (ER_i (ctop));
}

static void fold_function (const void *el, val_t *fold_initval, BC_node_t fold_el_fun_block,
  ...
  *(val_t *) ctop = *fold_initval;
  ...
  if (fold_vect_el_type == ER_NM_val)
    *(val_t *) ctop = *(val_t *) el;
    ...
    memcpy ((char *) ctop + val_displ_table[fold_vect_el_type], (char *) el,
            type_size_table[fold_vect_el_type]);
    if (fold_vect_el_type == ER_NM_vect) ER_set_dim (ctop, 0);
  }
...
static void process_fold_vect_op (ER_node_t op, val_t *fold_initval, BC_node_t fold_el_fun_block,
                                  int dim, int depth) {
  size_t i, len;
  char *pack_els;
  ER_node_t v;
  ...
  if (dim > 1 && ER_NODE_MODE (op) == ER_NM_heap_pack_vect && fold_vect_el_type != ER_NM_vect)
  ...
  for (i = 0; i < len; i++) {
    if (ER_NODE_MODE (op) == ER_NM_heap_pack_vect) {
      ...
      if (dim > 1)
        process_fold_vect_op (((ER_node_t *) pack_els)[i], fold_initval, fold_el_fun_block, dim - 1,
                              depth + 1);
      else {
        DECR_CTOP (2); /* free place for fold function params.  */
        fold_function (pack_els + i * type_size_table[fold_vect_el_type], fold_initval,
                       fold_el_fun_block, fold_vect_el_type, dim + depth - 1);
        DECR_CTOP (-2);
      }
    } else {
      ...
      if (dim == 1) {
        DECR_CTOP (2); /* free place for fold function params.  */
        fold_function (v, fold_initval, fold_el_fun_block, fold_vect_el_type, dim + depth - 1);
        DECR_CTOP (-2);
      } else if (ER_NODE_MODE (v) != ER_NM_vect)
        eval_error (vecform_bc_decl, call_pos (), DERR_vector_form_type, depth);
      else
        process_fold_vect_op (ER_vect (v), fold_initval, fold_el_fun_block, dim - 1, depth + 1);
    }
    op = GET_TEMP_REF (0);
  }
  POP_TEMP_REF (1);
...
void fold_call (int pars_number) {
  ...
  if (pars_number != 3 && pars_number != 4)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, FOLD_NAME);
  par1 = IVAL (ctop, -pars_number + 1);
  par2 = IVAL (ctop, -pars_number + 2);
  if (!function_p (par1) || ER_NODE_MODE (par2) != ER_NM_vect)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, FOLD_NAME);
...
static int filter_function (const void *el, BC_node_t filter_el_fun_block,
  ...
  if (filter_vect_el_type == ER_NM_val)
    *(val_t *) ctop = *(val_t *) el;
    ...
    memcpy ((char *) ctop + val_displ_table[filter_vect_el_type], (char *) el,
            type_size_table[filter_vect_el_type]);
    if (filter_vect_el_type == ER_NM_vect) ER_set_dim (ctop, 0);
  }
  call_fun_class (filter_el_fun_block, context, 1, curr_from_c_code_p);
  TOP_UP;
  implicit_int_conversion (ctop, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_int)
    eval_error (invresult_bc_decl, call_pos (), DERR_invalid_result, FILTER_NAME);
  res = ER_i (ctop) != 0;
  TOP_DOWN;
  return res;
}

static ER_node_t process_filter_vect_op (ER_node_t op, BC_node_t filter_el_fun_block, int dim,
                                         int depth) {
  ...
  if (dim > 1 && ER_NODE_MODE (op) == ER_NM_heap_pack_vect && filter_vect_el_type != ER_NM_vect)
    eval_error (vecform_bc_decl, call_pos (), DERR_vector_form_type, depth);
  if (dim > 1)
  ...
  for (i = nel = 0; i < len; i++) {
    if (dim > 1) {
      if (ER_NODE_MODE (op) == ER_NM_heap_pack_vect)
        el = process_filter_vect_op (((ER_node_t *) ER_pack_els (op))[i], filter_el_fun_block,
                                     dim - 1, depth + 1);
      else if (ER_NODE_MODE (IVAL (ER_unpack_els (op), i)) != ER_NM_vect)
        eval_error (vecform_bc_decl, call_pos (), DERR_vector_form_type, depth);
      else
        el = process_filter_vect_op (ER_vect (IVAL (ER_unpack_els (op), i)), filter_el_fun_block,
                                     dim - 1, depth + 1);
      op = GET_TEMP_REF (0);
      ...
      flag = filter_function (ER_pack_els (op) + i * el_size, filter_el_fun_block,
      ...
      if (!flag) {
        ...
        op = GET_TEMP_REF (0);
        result = GET_TEMP_REF (1);
        memcpy (ER_pack_els (result) + nel * el_size, ER_pack_els (op) + i * el_size, el_size);
      }
    } else {
      TOP_DOWN; /* free place for filter function param.  */
      flag = filter_function (IVAL (ER_unpack_els (op), i), filter_el_fun_block,
                              filter_vect_el_type, dim + depth - 1);
      TOP_UP;
      op = GET_TEMP_REF (0);
      result = GET_TEMP_REF (1);
      if (!flag) continue;
      *(val_t *) IVAL (ER_unpack_els (result), nel) = *(val_t *) IVAL (ER_unpack_els (op), i);
    }
    nel++;
    ER_set_els_number (result, nel);
  }
  POP_TEMP_REF (2);
  if (dim == 1) try_full_pack (result);
  return result;
}

void filter_call (int pars_number) {
  ER_node_t context, vect;
  ER_node_t par1, par2;
  ...
  if (pars_number != 2 && pars_number != 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, FILTER_NAME);
  par1 = IVAL (ctop, -pars_number + 1);
  par2 = IVAL (ctop, -pars_number + 2);
  if (!function_p (par1) || ER_NODE_MODE (par2) != ER_NM_vect)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, FILTER_NAME);
  fun_result_offset = (val_t *) fun_result - (val_t *) cvars;
  vect = ER_vect (par2);
  if (pars_number == 2)
    filter_dim = 1;
...
static void map_function (const void *el, BC_node_t map_el_fun_block,
  ...
  if (map_vect_el_type == ER_NM_val)
    *(val_t *) ctop = *(val_t *) el;
    ...
    memcpy ((char *) ctop + val_displ_table[map_vect_el_type], (char *) el,
            type_size_table[map_vect_el_type]);
    if (map_vect_el_type == ER_NM_vect) ER_set_dim (ctop, 0);
  }
  call_fun_class (map_el_fun_block, context, 1, curr_from_c_code_p);
  TOP_UP;
}

static ER_node_t process_map_vect_op (ER_node_t op, BC_node_t map_el_fun_block, int dim,
                                      int depth) {
  ...
  if (dim > 1 && ER_NODE_MODE (op) == ER_NM_heap_pack_vect && map_vect_el_type != ER_NM_vect)
    eval_error (vecform_bc_decl, call_pos (), DERR_vector_form_type, depth);
  if (dim == 1)
    result = create_unpack_vector (len);
  else
  ...
  for (i = 0; i < len; i++) {
    if (dim == 1) {
      TOP_DOWN; /* free place for filter function param.  */
      if (ER_NODE_MODE (op) == ER_NM_heap_pack_vect)
        map_function (ER_pack_els (op) + i * el_size, map_el_fun_block, map_vect_el_type,
                      dim + depth - 1);
      else
        map_function (IVAL (ER_unpack_els (op), i), map_el_fun_block, map_vect_el_type,
                      dim + depth - 1);
      op = GET_TEMP_REF (0);
      result = GET_TEMP_REF (1);
      *(val_t *) IVAL (ER_unpack_els (result), i) = *(val_t *) ctop;
      ...
      if (ER_NODE_MODE (op) == ER_NM_heap_pack_vect)
        el = process_map_vect_op (((ER_node_t *) ER_pack_els (op))[i], map_el_fun_block, dim - 1,
                                  depth + 1);
      else if (ER_NODE_MODE (IVAL (ER_unpack_els (op), i)) != ER_NM_vect)
        eval_error (vecform_bc_decl, call_pos (), DERR_vector_form_type, depth);
      else
        el = process_map_vect_op (ER_vect (IVAL (ER_unpack_els (op), i)), map_el_fun_block, dim - 1,
                                  depth + 1);
      op = GET_TEMP_REF (0);
      result = GET_TEMP_REF (1);
...
void map_call (int pars_number) {
  ...
  if (pars_number != 2 && pars_number != 3)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, MAP_NAME);
  par1 = IVAL (ctop, -pars_number + 1);
  par2 = IVAL (ctop, -pars_number + 2);
  if (!function_p (par1) || ER_NODE_MODE (par2) != ER_NM_vect)
...
void transpose_call (int pars_number) {
  ...
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, TRANSPOSE_NAME);
  if (ER_NODE_MODE (ctop) != ER_NM_vect)
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, TRANSPOSE_NAME);
  vect = ER_vect (ctop);
  GO_THROUGH_REDIR (vect);
  try_full_pack (vect);
  ...
  if (nrows == 0 || ER_NODE_MODE (vect) == ER_NM_heap_unpack_vect
    ...
    eval_error (matrixform_bc_decl, call_pos (), DERR_matrix_form_type, TRANSPOSE_NAME);
  pack_els = ER_pack_els (vect);
  for (i = 0; i < nrows; i++) {
    row = ((ER_node_t *) pack_els)[i];
    GO_THROUGH_REDIR (row);
    eltp = (ER_NODE_MODE (row) == ER_NM_heap_pack_vect ? ER_pack_vect_el_mode (row)
                                                       : ER_NM__error); /* ??? */
    if (i == 0) {
  ...
  for (i = 0; i < ncols; i++)
    ((ER_node_t *) res_pack_els)[i]
      = (res_eltp != ER_NM__error ? create_pack_vector (nrows, res_eltp)
  ...
  for (i = 0; i < nrows; i++) {
    ...
    for (j = 0; j < ncols; j++) {
      ...
      if (res_eltp != ER_NM__error) {
        ...
        memcpy ((char *) ER_pack_els (col) + i * el_size, (char *) ER_pack_els (row) + j * el_size,
                el_size);
        ...
        memcpy ((char *) res_el + displ, (char *) ER_pack_els (row) + j * el_size, el_size);
      } else {
        d_assert (ER_NODE_MODE (col) == ER_NM_heap_unpack_vect);
        ((val_t *) ER_unpack_els (col))[i] = ((val_t *) ER_unpack_els (row))[j];
      }
    }
  }
  ER_SET_MODE (fun_result, ER_NM_vect);
...
void set_encoding_call (int pars_number) {
  ER_node_t result;
  conv_desc_t byte_cd, ucode_cd, reverse_ucode_cd;
  encoding_type_t tp;
  const char *str;
  ...
  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SET_ENCODING_NAME);
  to_vect_string_conversion (ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
      || (ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_char
          && ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_byte))
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, SET_ENCODING_NAME);
  if (ER_pack_vect_el_mode (ER_vect (ctop)) == ER_NM_char) /* All encodings are ASCII strings.  */
    eval_error (parvalue_bc_decl, call_pos (), DERR_parameter_value, SET_ENCODING_NAME);
  str = ER_pack_els (ER_vect (ctop));
  if (!set_conv_descs (str, &byte_cd, &ucode_cd, &reverse_ucode_cd, &tp))
    eval_error (parvalue_bc_decl, call_pos (), DERR_parameter_value, ifun_name);
  if (!check_encoding_on_ascii (str)) {
#ifdef HAVE_ICONV_H
    if (byte_cd != NO_CONV_DESC) iconv_close (byte_cd);
    if (ucode_cd != NO_CONV_DESC) iconv_close (ucode_cd);
    if (reverse_ucode_cd != NO_CONV_DESC) iconv_close (reverse_ucode_cd);
#endif
    eval_error (parvalue_bc_decl, call_pos (), DERR_non_ascii_default_encoding, str, ifun_name);
  }
#ifdef HAVE_ICONV_H
...
void get_encoding_call (int pars_number) {
  ER_node_t result;

  if (pars_number != 0)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GET_ENCODING_NAME);
  result = create_string (curr_encoding_name);
  ER_SET_MODE (fun_result, ER_NM_vect);
  set_vect_dim (fun_result, result, 0);
}

void set_file_encoding_call (int pars_number) {
  ...
  if (pars_number != 2)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, SET_FILE_ENCODING_NAME);
  to_vect_string_conversion (ctop, NULL, NULL);
  if (ER_NODE_MODE (ctop) != ER_NM_vect || ER_NODE_MODE (ER_vect (ctop)) != ER_NM_heap_pack_vect
      || (ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_char
          && ER_pack_vect_el_mode (ER_vect (ctop)) != ER_NM_byte))
    eval_error (partype_bc_decl, call_pos (), DERR_parameter_type, SET_FILE_ENCODING_NAME);
  if (ER_pack_vect_el_mode (ER_vect (ctop)) == ER_NM_char) /* All encodings are ASCII strings.  */
    eval_error (parvalue_bc_decl, call_pos (), DERR_parameter_value, SET_FILE_ENCODING_NAME);
  str = ER_pack_els (ER_vect (ctop));
  if (!set_conv_descs (str, &byte_cd, &ucode_cd, &reverse_ucode_cd, &tp))
    eval_error (parvalue_bc_decl, call_pos (), DERR_parameter_value, ifun_name);
...
#ifdef HAVE_ICONV_H
  if (prev_byte_cd != NO_CONV_DESC) iconv_close (prev_byte_cd);
  if (prev_ucode_cd != NO_CONV_DESC) iconv_close (prev_ucode_cd);
  if (prev_reverse_ucode_cd != NO_CONV_DESC) iconv_close (prev_reverse_ucode_cd);
#endif
  name = get_file_encoding_name (file_instance);
  result = create_string (name);
  set_file_encoding (ER_stack_vars (file_instance), str, byte_cd, ucode_cd, reverse_ucode_cd,
                     get_encoding_type (name));
  ER_SET_MODE (fun_result, ER_NM_vect);
  set_vect_dim (fun_result, result, 0);
}

void get_file_encoding_call (int pars_number) {
  ER_node_t result, file_instance;

  if (pars_number != 1)
    eval_error (parnumber_bc_decl, call_pos (), DERR_parameters_number, GET_FILE_ENCODING_NAME);
  get_file (pars_number, &file_instance);
  ER_SET_MODE (fun_result, ER_NM_vect);
  set_vect_dim (fun_result, create_string (get_file_encoding_name (file_instance)), 0);
}

static void initiate_vars (void) {
  ER_node_t var;
  ...
  for (i = 0; program_environment[i] != NULL; i++) {
    for (j = 0; program_environment[i][j] != '\0'; j++)
      if (program_environment[i][j] == '=') break;
  ...
  var = IVAL (ER_stack_vars (ostack), BC_var_num (main_thread_bc_decl));
  ER_SET_MODE (var, ER_NM_thread);
  ER_set_thread (var, cthread);
  d_assert (BC_decl_scope (curr_thread_bc_decl) == ER_block_node (ostack));
  var = IVAL (ER_stack_vars (ostack), BC_var_num (curr_thread_bc_decl));
  ER_SET_MODE (var, ER_NM_thread);
  ER_set_thread (var, cthread);
}
...
void init_call (int pars_number) {
...
static void call_external_fun (int pars_number, BC_node_t fdecl) {
  external_fun_t *fun;
  ER_node_t vect;
  ER_node_t val;
  ER_node_t v;
  ...
  INCREMENT_PC ();
  /* Pop all actual parameters. */
  *(val_t *) ctop = (*fun) (pars_number, (val_t *) IVAL (ER_unpack_els (vect), 0));
}
...
/* Set up formal parameters starting with VARS of BLOCK from
   ...
   NIL.  */
static void do_always_inline setup_pars (BC_node_t block, int actuals_num, ER_node_t vars,
                                         val_t *actual_start, int vars_number) {
  ...
  if (actuals_num < min_actuals_num)
    eval_error (parnumber_bc_decl, get_cpos (), DERR_too_few_actual_parameters,
                BC_ident (BC_fdecl (block)));
  else if (actuals_num > formals_num && !args_p)
    eval_error (parnumber_bc_decl, get_cpos (), DERR_too_many_actual_parameters,
                BC_ident (BC_fdecl (block)));
  copies_num = (formals_num < actuals_num ? formals_num : actuals_num);
  /* Transfer actuals.  */
  for (i = 0; i < copies_num; i++) *(val_t *) IVAL (vars, i) = *actual_start++;
  ...
  if (args_p) {
    ...
    for (i = 0; i < nargs; i++) *(val_t *) IVAL (ER_unpack_els (vect), i) = *actual_start++;
  }
}

static ER_node_t do_always_inline create_class_stack (BC_node_t class_block, ER_node_t context,
  ...
  setup_pars (class_block, actuals_num, ER_stack_vars (cstack), actuals_start,
              BC_vars_num (class_block));
  stack = cstack;
  if (simple_p) {
    heap_pop ();
    cpc = saved_cpc;
...
static void do_always_inline process_fun_call (val_t *par_start, BC_node_t code, ER_node_t context,
  ...
  if (BC_fiber_p (code) && sync_flag)
    /* We check it before creating a stack (see
       find_catch_pc).  */
    eval_error (syncthreadcall_bc_decl, get_cpos (), DERR_fiber_call_in_sync_stmt);
  real_fun_call_pc = cpc;
  d_assert (BC_NODE_MODE (code) == BC_NM_fblock && BC_fmode (code) != BC_builtin);
  if (tail_flag && !BC_ext_life_p (ER_block_node (cstack)) && context != cstack
      && cstack != uppest_stack
...
void do_always_inline process_imm_ifun_call (BC_node_t code, int actuals_num, int from_c_code_p) {
...
void do_inline process_fun_class_call (BC_node_t fdecl, ER_node_t context, ER_node_t call_start,
                                       ER_node_t actuals_start, int actuals_num, int tail_flag,
                                       int from_c_code_p) {
  BC_node_t fblock;
  implementation_fun_t ifunc;
  ER_node_t instance;

  d_assert (call_start == actuals_start || IVAL (call_start, 1) == actuals_start);
  ...
  if (BC_NODE_MODE (fdecl) != BC_NM_fdecl)
    eval_error (callop_bc_decl, get_cpos (), DERR_none_class_or_fun_before_left_bracket);
  fblock = BC_fblock (fdecl);
  if (BC_class_p (fblock)) {
    ctop = IVAL (call_start, -1);
    instance = create_class_stack (fblock, context, (val_t *) actuals_start, actuals_num,
                                   BC_simple_p (fblock));
    ...
    ctop = IVAL (call_start, -1);
    process_fun_call ((val_t *) actuals_start, fblock, context, actuals_num, tail_flag,
                      from_c_code_p);
  }
}
...
/* A general function processing tail (if TAIL_FLAG) fun/fiber/class
   ...
   CALL_START.  */
void process_call (ER_node_t call_start, int actuals_num, int tail_flag, int from_c_code_p) {
  ...
  process_fun_class_call (fdecl, context, call_start, IVAL (call_start, 1), actuals_num, tail_flag,
                          from_c_code_p);
}

void initiate_funcs (void) {
  if (!check_ucode_db ()) error (TRUE, no_position, "Unicode database is corrupted");
  if (trace_flag) VARR_CREATE (trace_stack_elem_t, trace_stack, 0);
  initiate_io ();
  initiate_regex_tab ();
  initiate_subst_map ();
#ifdef RFLOAT_NAN
...
void finish_funcs (void) {
  finish_subst_map ();
  finish_regex_tab ();
  finish_io ();
  if (trace_flag) VARR_DESTROY (trace_stack_elem_t, trace_stack);
}
...
/* The following function implements function set_grammar in class
   parser. */
void int_earley_parse_grammar (int npars) {
  struct grammar *g;
  int code;
  ...
  if (ER_NODE_MODE (par2) != ER_NM_int || ER_NODE_MODE (par3) != ER_NM_vect
    ...
    eval_error (partype_bc_decl, get_pos (real_fun_call_pc), DERR_parameter_type, name);
  g = (struct grammar *) ER_hide (par1);
  code = yaep_parse_grammar (g, ER_i (par2), strvect_to_world (ER_vect (par3), TRUE));
...
/* The following function implements function set_debug in class
   parser. */
void int_earley_set_debug_level (int npars) {
  int i;
  ER_node_t par1, par2;
...
/* The following function implements function set_one_parse in class
   parser. */
void int_earley_set_one_parse_flag (int npars) {
  int i;
  ER_node_t par1, par2;
...
/* The following function implements function set_lookahead in class
   parser. */
void int_earley_set_lookahead_level (int npars) {
  int i;
  ER_node_t par1, par2;
...
/* The following function implements function set_cost in class
   parser. */
void int_earley_set_cost_flag (int npars) {
  int i;
  ER_node_t par1, par2;
...
/* The following function implements function set_recovery in class
   parser. */
void int_earley_set_error_recovery_flag (int npars) {
  int i;
  ER_node_t par1, par2;
  ...
  if (ER_NODE_MODE (par2) != ER_NM_int)
    eval_error (partype_bc_decl, get_pos (real_fun_call_pc), DERR_parameter_type, name);
  i = yaep_set_error_recovery_flag ((struct grammar *) ER_hide (par1), ER_i (par2));
  ER_SET_MODE (fun_result, ER_NM_int);
  ER_set_i (fun_result, i);
}
...
/* The following function implements function set_recovery_match in
   class parser. */
void int_earley_set_recovery_match (int npars) {
  int i;
  ER_node_t par1, par2;
...
static int init_read_token (void **attr) {
  ER_node_t tok, code;
  BC_node_t decl;
  int n;
  const char *message;
  ...
  if (curr_token >= ER_els_number (tokens_vect)) return -1;
  ...
  if (!internal_isa_call (&message, (ER_node_t) &t2, (ER_node_t) &t))
    eval_error (invtoken_bc_decl, get_pos (real_fun_call_pc),
                "run time error (parse) -- invalid token #%lu", (unsigned long) curr_token);
  curr_token++;
  ...
  if (ER_NODE_MODE (code) != ER_NM_int)
    eval_error (invtoken_bc_decl, get_pos (real_fun_call_pc),
                "run time error (parse) -- invalid code of token #%d", curr_token - 1);
  return ER_i (code);
}
...
/* The following function is interface to DINO error function.  We
   ...
   int_earley_parse. */
static void init_syntax_token (int err_tok_num, void *err_tok_attr, int start_ignored_tok_num,
  ...
  call_fun_class (error_fun_block, error_fun_block_context, 6, curr_from_c_code_p);
}
...
typedef struct tree_heap_node *tree_heap_node_t;
DEF_HTAB (tree_heap_node_t);

static struct tree_heap_node temp_tree_heap_node;
static HTAB (tree_heap_node_t) * tree_heap_tab;

/* Hash of the node. */
static htab_hash_t tree_heap_node_hash (tree_heap_node_t node) {
  return dino_hash64 ((size_t) node->tree_node, 9);
}

/* Equality of nodes. */
static int tree_heap_node_eq (tree_heap_node_t node1, tree_heap_node_t node2) {
  return node1->tree_node == node2->tree_node;
}

/* The following function places abstract tree into heap and returns
   the result. */
static ER_node_t tree_to_heap (struct yaep_tree_node *root) {
  ER_node_t var, res, vect, name_vect;
  struct yaep_tree_node *node, *alt;
  tree_heap_node_t tree_heap_node, tab_tree_heap_node;
  int i;

  tree_heap_node = &temp_tree_heap_node;
  tree_heap_node->tree_node = root;
  if (HTAB_DO (tree_heap_node_t, tree_heap_tab, tree_heap_node, HTAB_FIND, tab_tree_heap_node))
    return tab_tree_heap_node->heap_node;
  TOP_UP;
  ...
  case YAEP_ERROR:
    var = IVAL (ER_stack_vars (get_obj_stack (yaep_bc_decl)),
                BC_var_num (root->type == YAEP_NIL ? nil_anode_bc_decl : error_anode_bc_decl));
  ...
  case YAEP_TERM:
    ...
    TOP_UP;
    ER_SET_MODE (ctop, ER_NM_stack);
    d_assert (ER_NODE_MODE ((ER_node_t) root->val.term.attr) == ER_NM_heap_stack);
    ER_set_stack (ctop, root->val.term.attr);
    DECR_CTOP (2);
    res = create_class_stack (anode_bc_decl, get_obj_stack (yaep_bc_decl), (val_t *) IVAL (ctop, 1),
                              2, TRUE);
    break;
  case YAEP_ANODE:
  ...
  if (root->type == YAEP_ANODE) {
    ...
    for (i = 0, alt = root; alt != NULL; alt = alt->val.alt.next, i++)
      ((ER_node_t *) ER_pack_els (vect))[i] = tree_to_heap (alt->val.alt.node);
    ER_set_els_number (vect, i);
  }
  return res;
}
...
/* The following function implements function parse in class
   parser. */
void int_earley_parse (int npars) {
  struct grammar *g;
  ...
  if (ER_NODE_MODE (par2) != ER_NM_vect || (ER_NODE_MODE (ER_vect (par2)) != ER_NM_heap_pack_vect)
      || (ER_pack_vect_el_mode (ER_vect (par2)) != ER_NM_stack) || !function_p (par3))
    eval_error (partype_bc_decl, get_pos (real_fun_call_pc), DERR_parameter_type, name);
  fun_result_offset = (val_t *) fun_result - (val_t *) cvars;
  ...
  code = yaep_parse (g, init_read_token, init_syntax_token, int_parse_alloc, NULL, &root,
                     &ambiguous_p);
  if (code == YAEP_NO_MEMORY) {
...
void int_earley_create_grammar (int npars) {
  struct grammar *g;
