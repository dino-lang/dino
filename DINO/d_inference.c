...
static const char *type_name[]
  = {"unknown", "int", "char", "float", "long", "vec", "tab", "undef", "varying"};
...
/* Add TOP and all nodes reachable from it to the reachable node
   set.  */
static void add_reachable_nodes (struct problem *problem, node_t top) {
  node_t node;
  edge_t e;
...
static void calculate_postorder_inverted (struct problem *problem, node_t *nodes, int nodes_num) {
  ...
  while (postorder_stack_bound > 0) {
    ...
    if (problem->get_visit_tick (top)) {
      ...
      for (e = problem->src_edges (top); e != NULL; e = problem->next_src_edge (e)) {
        node = problem->edge_src (e);
        add_node_to_consider (problem, node);
      }
...
static int propagate_forward (struct problem *problem, node_t node, bitmap_t pending, int tick) {
  ...
  if ((e = problem->src_edges (node)) == NULL) {
    if (problem->confl0 != NULL) problem->confl0 (node);
  } else
    for (; e != NULL; e = problem->next_src_edge (e)) {
      src = problem->edge_src (e);
      if (tick <= problem->get_visit_tick (src)) changed_p |= problem->confl (e);
    }
...
static int inline get_block_place_start (block_t block) {
...
static int get_insn_op_place (df_insn_t insn, int nop, int result_p) {
  ...
  case BC_NM_out:
  case BC_NM_stpop:
  case BC_NM_bend: return NO_MORE_OPERAND; break;
  case BC_NM_fbend: return (nop == 0 ? NO_MORE_REAL_OPERAND : NO_MORE_OPERAND);
  case BC_NM_btdef:
  case BC_NM_bt:
  case BC_NM_bf:
  case BC_NM_bfni:
  ...
  case BC_NM_ibfni:
    if (result_p)
      ...
      return NO_MORE_OPERAND;
    break;
  case BC_NM_leave: return result_p && nop == 0 ? NO_MORE_REAL_OPERAND : NO_MORE_OPERAND;
  case BC_NM_ret:
    if (result_p)
  ...
  case BC_NM_efun:
  case BC_NM_class:
    if (!result_p) return NO_MORE_OPERAND;
    if (nop == 0)
      res = BC_op1 (bc);
    else
      return NO_MORE_OPERAND;
  ...
  case BC_NM_foreach2:
    ...
    break;
  case BC_NM_stdecm:
    if (nop > 0) return NO_MORE_OPERAND;
    res = result_p ? BC_op1 (bc) : BC_op2 (bc);
    break;
  case BC_NM_stdecu:
    if (!result_p || nop > 0) return NO_MORE_OPERAND;
    res = BC_op1 (bc);
    break;
  case BC_NM_stinc:
    if (!result_p) return NO_MORE_OPERAND;
    /* We just take non-par vars into account:  */
    if (nop >= BC_op1 (bc) + BC_op3 (bc) - BC_op2 (bc)) return NO_MORE_OPERAND;
    res = BC_op2 (bc) + nop;
    break;
  case BC_NM_block:
    if (!result_p) return NO_MORE_OPERAND;
    if (nop >= BC_vars_num (insn->block->bc_block)) return NO_MORE_OPERAND;
  ...
  case BC_NM_fblock:
    ...
    if (nop >= BC_pars_num (bc)) return NO_MORE_OPERAND;
    return nop + BC_vars_num (tl_block->bc_block);
  case BC_NM_chvec:
  case BC_NM_chvend:
  case BC_NM_chvlen:
  ...
  case BC_NM_chtend:
  case BC_NM_chst:
  case BC_NM_chstend: return NO_MORE_OPERAND;
  case BC_NM_chvel:
    if (!result_p)
      return NO_MORE_OPERAND;
    else {
  ...
  case BC_NM_rmatchs:
    ...
    break;
  case BC_NM_compile: return NO_MORE_OPERAND;
...
static inline int get_insn_defs_num (df_insn_t insn) {
  BC_node_t bc = insn->bc;
  int i, res;

  if ((res = insn_defs_num_cache[BC_NODE_MODE (bc)]) >= 0) return res;
  for (i = 0;; i++)
    if (get_insn_op_place (insn, i, TRUE) == NO_MORE_OPERAND) {
      if (!BC_IS_OF_TYPE (bc, BC_NM_block) && !BC_IS_OF_TYPE (bc, BC_NM_stinc)
          && !BC_IS_OF_TYPE (bc, BC_NM_chvel) && !BC_IS_OF_TYPE (bc, BC_NM_chtel)
...
static inline int get_insn_uses_num (df_insn_t insn) {
  BC_node_t bc = insn->bc;
  int i, res;

  if ((res = insn_uses_num_cache[BC_NODE_MODE (bc)]) >= 0) return res;
  for (i = 0;; i++)
    if (get_insn_op_place (insn, i, FALSE) == NO_MORE_OPERAND) {
      if (!BC_IS_OF_TYPE (bc, BC_NM_block)) insn_uses_num_cache[BC_NODE_MODE (bc)] = i;
      return i;
    }
}
...
static VARR (df_insn_t) * insns_varr;
...
static void add_insn (df_insn_t insn) {
  ...
  insns = VARR_ADDR (df_insn_t, insns_varr);
  def_insns = VARR_ADDR (df_insn_t, def_insns_varr);
}

/* Create and return block for the BC_BLOCK.  Mark public variables as
   escaped.  */
static block_t create_block (BC_node_t bc_block) {
  ...
  if (all_vars_num == 0) all_vars_num = 1;
  ...
  block->fun_p = FALSE;
  for (bc_decl = BC_decls (bc_block); bc_decl != NULL; bc_decl = BC_next_decl (bc_decl))
    if (BC_IS_OF_TYPE (bc_decl, BC_NM_vdecl) && BC_public_p (bc_decl)) {
...
/* Create insns starting with BC block code START and definition
   map.  */
static void create_insns (BC_node_t start) {
  BC_node_t info;
  df_insn_t insn;
  int i, ndefs, vars_num;
  BC_node_t bc;
  block_t block;
  VARR (block_t) * block_stack_varr;
  struct df_insn temp_insn;

  VARR_CREATE (block_t, block_stack_varr, 0);
  ...
  for (info = BC_info (start); info != NULL; info = BC_next_info (info)) {
    ...
    if (BC_IS_OF_TYPE (bc, BC_NM_block)) {
      VARR_PUSH (block_t, block_stack_varr, block);
      block = BC_scope (bc) == NULL ? tl_block : create_block (bc);
    }
    /* That is a trick.  Size of the insn depends only on insn bc
       and its block.  */
    temp_insn.bc = bc;
    temp_insn.block = block;
    vars_num = get_insn_defs_num (&temp_insn);
    vars_num += get_insn_uses_num (&temp_insn);
    if (vars_num == 0) vars_num = 1;
    insn = pass_alloc (sizeof (struct df_insn) + sizeof (type_t) * (vars_num - 1));
    for (i = 0; i < vars_num; i++) insn->types[i] = TP_unknown;
    ...
    if (BC_IS_OF_TYPE (bc, BC_NM_bend)) {
      ...
      block = VARR_POP (block_t, block_stack_varr);
    }
    add_insn (insn);
  }
  assert (block == NULL
          ...
          || BC_next (start) == NULL);
  insns = VARR_ADDR (df_insn_t, insns_varr);
  assert (all_insns_num == insns_num && insns_num == VARR_LENGTH (df_insn_t, insns_varr));
  def_insns = VARR_ADDR (df_insn_t, def_insns_varr);
  assert (def_insns_num == VARR_LENGTH (df_insn_t, def_insns_varr));
  assert (VARR_LENGTH (block_t, block_stack_varr) == 0
          /* It might be forward decl fun without definition.  */
          || (BC_next (start) == NULL && VARR_LENGTH (block_t, block_stack_varr) == 1));
  VARR_DESTROY (block_t, block_stack_varr);
}

/* Remove variables accessible through lvar from consideration by
   modifying escape_p array corresponding to the var block.  */
static void mark_lvar_access_vars_as_escaped (void) {
  int i;
  ...
  for (i = 0; i < insns_num; i++) {
    insn = insns[i];
    bc = insn->bc;
    if (!BC_IS_OF_TYPE (bc, BC_NM_lvar) && !BC_IS_OF_TYPE (bc, BC_NM_lvarv)) continue;
    vdecl = BC_decl (bc);
...
static df_insn_t get_dest (df_insn_t insn, int n) {
  BC_node_t bc = insn->bc;
  ...
  if (n == 0) return BC_next (bc) == NULL ? NULL : BC_aux (BC_info (BC_next (bc)));
  if (BC_IS_OF_TYPE (bc, BC_NM_br)) {
    if (n == 1) return BC_aux (BC_info (BC_pc (bc)));
  } else if (BC_IS_OF_TYPE (bc, BC_NM_foreach)) {
    if (n == 1) return BC_aux (BC_info (BC_body_pc (bc)));
  } else if (BC_IS_OF_TYPE (bc, BC_NM_block)) {
    if (n == 1 && BC_excepts (bc) != NULL) return BC_aux (BC_info (BC_excepts (bc)));
  } else if (BC_IS_OF_TYPE (bc, BC_NM_except)) {
    if (n == 1 && BC_next_except (bc) != NULL) return BC_aux (BC_info (BC_next_except (bc)));
  } else if (BC_IS_OF_TYPE (bc, BC_NM_check)) {
    if (n == 1 && BC_fail_pc (bc) != NULL) return BC_aux (BC_info (BC_fail_pc (bc)));
  }
  return NULL;
}
...
static cfg_bb_t get_bb (df_insn_t head) {
  cfg_bb_t bb;

  if ((bb = head->bb) != NULL) return bb;
...
static void add_cfg_edge (cfg_bb_t src, cfg_bb_t dest) {
  cfg_edge_t e = pass_alloc (sizeof (struct cfg_edge));

  e->src = src;
  e->dest = dest;
  e->next_src = dest->srcs;
  dest->srcs = e;
  e->next_dest = src->dests;
  src->dests = e;
}

static int inline imcall_to_consider_p (BC_node_t bc) {
  BC_node_t fblock;
  df_insn_t dest_insn;

  if (!BC_IS_OF_TYPE (bc, BC_NM_imcall)) return FALSE;
  fblock = BC_cfblock (bc);
  /* We ignore impossible calls of forward decl fun without
     definition.  */
  if (BC_next (fblock) == NULL) return FALSE;
...
static void build_cfg (void) {
  ...
  for (i = 0; i < insns_num; i++) {
    ...
    insn->bb = NULL;
    bc = insn->bc;
    if (BC_IS_OF_TYPE (bc, BC_NM_fun) || BC_IS_OF_TYPE (bc, BC_NM_class)) {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (j = 0; (dest_insn = get_dest (insn, j)) != NULL; j++) {
      if (dest_insn->bb == NULL) worklist[n++] = get_bb (dest_insn);
    ...
    if (imcall_to_consider_p (bc)) {
      ...
      if (fblock_insn->bb == NULL) worklist[n++] = get_bb (fblock_insn);
  ...
  for (i = 0; i < insns_num; i++) {
    insn = insns[i];
    bc = insn->bc;
    if (!imcall_to_consider_p (bc)) continue;
    dest_insn = get_dest (insn, 0);
    dest_bb = dest_insn->bb;
    fblock_insn = BC_aux (BC_info (BC_cfblock (bc)));
  ...
  FREE (worklist);
  bbs = VARR_ADDR (cfg_bb_t, bbs_varr);
  assert (bbs_num == VARR_LENGTH (cfg_bb_t, bbs_varr));
}
...
static inline long get_place_ndef (df_insn_t insn, int place, int insn_ndef) {
  ...
  if (place < BC_vars_num (tl_block->bc_block)) {
    if (tl_block->escape_p[place]) return -1;
...
static inline void get_def_insn_ndef (int ndef, df_insn_t *def_insn, int *insn_ndef) {
  df_insn_t insn;
...
/* Return place (see comments for function get_insn_op_place) for
   NDEF.  */
static inline int get_def_place (int ndef) {
  int res;
  df_insn_t insn;
  int insn_ndef;
...
static void calculate_bb_avail (void) {
  static struct problem avail_problem
    = {get_bb_index,       get_bb_index, bb_srcs,       bb_dests,    next_src_cfg_edge,
       next_dest_cfg_edge, cfg_edge_src, cfg_edge_dest, get_bb_tick, set_bb_tick,
       get_bb_order,       set_bb_order, avail_confl0,  avail_confl, avail_transf};
  calculate_local ();
  if (dump_flag) printf ("// BB postorder inverted:");
  forward_dataflow (&avail_problem, (node_t *) bbs, bbs_num, TRUE);
}

/* Create def-use for (DEF, DE_NUM) and (USE, USE_NUM) and include it
   into chains.  */
static void create_def_use (df_insn_t def, int def_num, df_insn_t use, int use_num) {
  ...
  du->def = def;
  du->use = use;
  du->next_def = def->defs;
  def->defs = du;
  du->next_use = use->uses;
  use->uses = du;
}

/* Connect ret/leave INSN to the corresponding fbend.  */
static void build_ret_def_use (df_insn_t insn) {
  df_insn_t fblock_insn;
  BC_node_t bc_block, bc = insn->bc;

  if (!BC_IS_OF_TYPE (bc, BC_NM_ret) && !BC_IS_OF_TYPE (bc, BC_NM_leave)) return;
  for (bc_block = insn->block->bc_block; !BC_IS_OF_TYPE (bc_block, BC_NM_fblock);
...
static void build_call_def_use (df_insn_t insn, bitmap_t avail) {
  ...
  /* We ignore impossible calls of forward decl fun without
     definition.  */
  if (BC_next (fblock) == NULL) return;
  fblock_insn = BC_aux (BC_info (fblock));
  /* It might be accessible by generic call.  Don't connect args in
     this case.  */
  if (fblock_insn->block->fun_p) return;
  start = BC_op1 (bc);
  nargs = BC_op2 (bc);
  for (bc_decl = BC_decls (fblock); bc_decl != NULL; bc_decl = BC_next_decl (bc_decl)) {
...
static int type_confl (edge_t te) {
  ...
  if (from_tp == TP_unknown || to_tp == TP_varying || to_tp == from_tp)
    ...
    res_tp = TP_varying;
  if (res_tp == to_tp) return FALSE;
  use->types[use_num + use_defs_num] = res_tp;
  return TRUE;
}

static inline type_t arithm_type (type_t t) {
  if (t == TP_unknown) return TP_unknown;
  if (t == TP_char) t = TP_int;
  if (t == TP_int || t == TP_long || t == TP_float) return t;
  return TP_varying;
}

static inline type_t arithm_int_type (type_t t) {
  if (t == TP_unknown) return TP_unknown;
  if (t == TP_char || t == TP_long || t == TP_float || t == TP_vec) t = TP_int;
  if (t == TP_int) return t;
  return TP_varying;
}

static inline type_t bin_arithm_type (type_t t1, type_t t2) {
  if (t1 == TP_unknown || t2 == TP_unknown) return TP_unknown;
  if (t1 == TP_char) t1 = TP_int;
  if (t2 == TP_char) t2 = TP_int;
  if ((t1 == TP_int || t1 == TP_long) && (t2 == TP_long || t2 == TP_float)) return t2;
  if ((t2 == TP_int || t2 == TP_long) && (t1 == TP_long || t1 == TP_float)) return t1;
  if (t1 == TP_int && t2 == TP_int) return TP_int;
  if ((t1 == TP_float || t1 == TP_int) && (t2 == TP_int || t2 == TP_float)) return TP_float;
  return TP_varying;
}

static inline type_t bin_arithm_int_type (type_t t1, type_t t2) {
  if (t1 == TP_unknown || t2 == TP_unknown) return TP_unknown;
  if (t1 == TP_char || t1 == TP_long || t1 == TP_float || t1 == TP_vec) t1 = TP_int;
  if (t2 == TP_char || t2 == TP_long || t2 == TP_float || t2 == TP_vec) t2 = TP_int;
  if (t1 == TP_int && t2 == TP_int) return TP_int;
  return TP_varying;
}

static inline type_t eq_type (type_t t1, type_t t2) {
  if (t1 == TP_unknown || t2 == TP_unknown) return TP_unknown;
  if (t1 != TP_varying && t2 != TP_varying) return TP_int;
  return TP_varying;
}

static inline type_t cmp_type (type_t t1, type_t t2) {
  if (t1 == TP_unknown || t2 == TP_unknown) return TP_unknown;
...
static int type_transf (node_t node) {
  int i, n, change_p;
  ...
  switch (BC_NODE_MODE (bc)) {
  case BC_NM_ldch:
  case BC_NM_chof: res_tp = TP_char; break;
  case BC_NM_ldi:
  case BC_NM_ldbi:
  case BC_NM_iof: res_tp = TP_int; break;
  case BC_NM_ldl:
  case BC_NM_lof: res_tp = TP_long; break;
  case BC_NM_ldf:
  case BC_NM_fof: res_tp = TP_float; break;
  case BC_NM_lds:
  case BC_NM_ldus:
  case BC_NM_vec:
  case BC_NM_vecof:
  case BC_NM_fmtvecof: res_tp = TP_vec; break;
  case BC_NM_tab:
  case BC_NM_tabof: res_tp = TP_tab; break;
  case BC_NM_plus:
  case BC_NM_minus:
  case BC_NM_not: res_tp = arithm_type (insn->types[1]); break;
  case BC_NM_bnot: res_tp = arithm_int_type (insn->types[1]); break;
  case BC_NM_add:
  case BC_NM_sub:
  case BC_NM_mult:
  case BC_NM_div:
  case BC_NM_mod: res_tp = bin_arithm_type (insn->types[1], insn->types[2]); break;
  case BC_NM_madd:
    res_tp = bin_arithm_type (bin_arithm_type (insn->types[1], insn->types[2]), insn->types[3]);
    break;
  case BC_NM_addi:
  case BC_NM_multi: res_tp = bin_arithm_type (insn->types[1], TP_int); break;
  case BC_NM_or:
  case BC_NM_xor:
  case BC_NM_and:
  case BC_NM_lsh:
  case BC_NM_rsh:
  case BC_NM_ash: res_tp = bin_arithm_int_type (insn->types[1], insn->types[2]); break;
  case BC_NM_id:
  case BC_NM_unid:
  case BC_NM_eq:
  case BC_NM_ne: res_tp = eq_type (insn->types[1], insn->types[2]); break;
  case BC_NM_eqi:
  case BC_NM_nei: res_tp = eq_type (insn->types[1], TP_int); break;
  case BC_NM_lt:
  case BC_NM_gt:
  case BC_NM_le:
  case BC_NM_ge: res_tp = cmp_type (insn->types[1], insn->types[2]); break;
  case BC_NM_lti:
  case BC_NM_gti:
  case BC_NM_lei:
  case BC_NM_gei: res_tp = cmp_type (insn->types[1], TP_int); break;
  case BC_NM_ldnil:
  case BC_NM_ldthis:
  case BC_NM_ldtp:
  case BC_NM_tpof: res_tp = TP_varying; break;
  case BC_NM_flat: res_tp = TP_vec; break;
  case BC_NM_var:
  case BC_NM_lvar:
  case BC_NM_lvarv:
  case BC_NM_evar:
  ...
  case BC_NM_ind2:
  case BC_NM_sl:
  case BC_NM_lslv: res_tp = TP_varying; break;
  case BC_NM_fun:
  case BC_NM_efun:
  case BC_NM_class:
  case BC_NM_ofun:
  case BC_NM_oclass: res_tp = TP_varying; break;
  case BC_NM_const:
  case BC_NM_new: res_tp = insn->types[1]; break;
  case BC_NM_length:
  ...
  case BC_NM_brts:
  case BC_NM_brfs:
  case BC_NM_lconv: res_tp = TP_int; break;
  case BC_NM_bteqinc:
  case BC_NM_btneinc:
  case BC_NM_btgeinc:
  case BC_NM_btltinc:
  ...
  case BC_NM_btgtinc:
    ...
    if (insn->types[0] == res_tp && insn->types[1] == res_tp2) return FALSE;
    insn->types[0] = res_tp;
    insn->types[1] = res_tp2;
    return TRUE;
  case BC_NM_foreach:
    res_tp = TP_varying; /* We can not know index type */
    if (insn->types[0] == res_tp) return FALSE;
    insn->types[0] = res_tp;
    return TRUE;
  case BC_NM_foreach2: res_tp = TP_varying; break;
  case BC_NM_move: res_tp = insn->types[1]; break;
  case BC_NM_stvt:
  case BC_NM_sts:
  case BC_NM_ste:
  case BC_NM_stvtu:
  ...
  case BC_NM_titcall:
  case BC_NM_cicall:
  case BC_NM_citcall: res_tp = insn->types[1]; break;
  case BC_NM_b:
  case BC_NM_btdef:
  case BC_NM_bt:
  case BC_NM_bf:
  ...
  case BC_NM_ibt:
  case BC_NM_ibf:
  case BC_NM_ibfni: return FALSE; /* No output */
  case BC_NM_bteq:
  case BC_NM_btne:
  case BC_NM_btge:
  case BC_NM_btlt:
  case BC_NM_btle:
  case BC_NM_btgt: res_tp = cmp_type (insn->types[1], insn->types[2]); break;
  case BC_NM_bteqi:
  case BC_NM_btnei:
  case BC_NM_btgei:
  case BC_NM_btlti:
  case BC_NM_btlei:
  case BC_NM_btgti: res_tp = cmp_type (insn->types[1], TP_int); break;
  case BC_NM_out:
  case BC_NM_stpop:
  case BC_NM_bend:
  case BC_NM_throw:
  case BC_NM_waitcond:
  case BC_NM_waitend:
  case BC_NM_except: return FALSE; /* No output */
  case BC_NM_leave: res_tp = TP_undef; break;
  case BC_NM_fbend: res_tp = insn->types[1]; break;
  case BC_NM_stdecm: res_tp = insn->types[1]; break;
  case BC_NM_stdecu:
  ...
  case BC_NM_stinc:
    ...
    for (i = BC_op1 (bc) + BC_op3 (bc) - BC_op2 (bc) - 1; i >= 0; i--) insn->types[i] = TP_undef;
    return FALSE;
  case BC_NM_block:
  case BC_NM_fblock: {
    ...
    for (i = n - 1; i >= 0; i--) {
      ...
      type_t t = (insn->uses == NULL ? TP_varying : insn->types[vars_num + i]);

      if (insn->types[i] != t) change_p = TRUE;
      insn->types[i] = t;
    }
    /* See comment for stdecu.  */
    for (i = vars_num - 1; i >= n; i--) insn->types[i] = TP_undef;
    return change_p;
    break;
  }
  case BC_NM_ret: res_tp = insn->types[1]; break;
  case BC_NM_chvec:
  case BC_NM_chvend:
  case BC_NM_chvlen:
  case BC_NM_chtab:
  case BC_NM_chtend:
  case BC_NM_chst:
  case BC_NM_chstend: return FALSE;
  case BC_NM_chvel:
    if (!BC_ch_op5 (bc)) return FALSE;
    break;
  case BC_NM_chtel:
    if (!BC_ch_op4 (bc)) return FALSE;
    break;
  case BC_NM_chstel:
    if (!BC_ch_op5 (bc)) return FALSE;
    break;
  case BC_NM_rmatch:
  case BC_NM_rmatchs: break;
  case BC_NM_compile: return FALSE; break;
...
static void calculate_types (void) {
  static struct problem type_problem
    = {get_insn_index, get_insn_id,    insn_uses, insn_defs,     next_use_edge,
       next_def_edge,  edge_def,       edge_use,  get_insn_tick, set_insn_tick,
       get_insn_order, set_insn_order, NULL,      type_confl,    type_transf};
  calculate_local ();
  if (dump_flag) printf ("// INSN postorder inverted:");
  forward_dataflow (&type_problem, (node_t *) insns, all_insns_num, TRUE);
}

static void specialize_insn (df_insn_t insn) {
  BC_node_t bc = insn->bc;
  ...
  case BC_NM_minus:
    ...
    break;
  case BC_NM_not:
    if (insn->types[1] == TP_int) nm = BC_NM_inot;
    break;
  case BC_NM_bnot:
    if (insn->types[1] == TP_int) nm = BC_NM_ibnot;
    break;
  case BC_NM_add:
  ...
  case BC_NM_multi:
    ...
    break;
  case BC_NM_or:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ior;
    break;
  case BC_NM_xor:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ixor;
    break;
  case BC_NM_and:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_iand;
    break;
  case BC_NM_lsh:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ilsh;
    break;
  case BC_NM_rsh:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_irsh;
    break;
  case BC_NM_ash:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_iash;
    break;
  case BC_NM_id:
  case BC_NM_unid: break;
  case BC_NM_eq:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ieq;
    break;
  case BC_NM_ne:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ine;
    break;
  case BC_NM_eqi:
    if (insn->types[1] == TP_int) nm = BC_NM_ieqi;
    break;
  case BC_NM_nei:
    if (insn->types[1] == TP_int) nm = BC_NM_inei;
    break;
  case BC_NM_lt:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ilt;
    break;
  case BC_NM_gt:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_igt;
    break;
  case BC_NM_le:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ile;
    break;
  case BC_NM_ge:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int) nm = BC_NM_ige;
    break;
  case BC_NM_lti:
    if (insn->types[1] == TP_int) nm = BC_NM_ilti;
    break;
  case BC_NM_gti:
    if (insn->types[1] == TP_int) nm = BC_NM_igti;
    break;
  case BC_NM_lei:
    if (insn->types[1] == TP_int) nm = BC_NM_ilei;
    break;
  case BC_NM_gei:
    if (insn->types[1] == TP_int) nm = BC_NM_igei;
    break;
  case BC_NM_stinc:
  case BC_NM_stdecm:
  case BC_NM_stdecu:
  ...
  case BC_NM_brts:
  case BC_NM_brfs:
  case BC_NM_lconv: break;
  case BC_NM_bteqinc:
    if (insn->types[2] == TP_int && insn->types[3] == TP_int) nm = BC_NM_ibteqinc;
    break;
  case BC_NM_btneinc:
    if (insn->types[2] == TP_int && insn->types[3] == TP_int) nm = BC_NM_ibtneinc;
    break;
  case BC_NM_btgeinc:
    if (insn->types[2] == TP_int && insn->types[3] == TP_int) nm = BC_NM_ibtgeinc;
    break;
  case BC_NM_btltinc:
    if (insn->types[2] == TP_int && insn->types[3] == TP_int) nm = BC_NM_ibtltinc;
    break;
  case BC_NM_btleinc:
    if (insn->types[2] == TP_int && insn->types[3] == TP_int) nm = BC_NM_ibtleinc;
    break;
  case BC_NM_btgtinc:
    if (insn->types[2] == TP_int && insn->types[3] == TP_int) nm = BC_NM_ibtgtinc;
    break;
  case BC_NM_foreach:
  case BC_NM_foreach2: break;
  case BC_NM_move:
    if (insn->types[1] == TP_int)
      nm = BC_NM_imove;
    else if (insn->types[1] == TP_float)
  ...
  case BC_NM_citcall:
  case BC_NM_b:
  case BC_NM_btdef: break;
  case BC_NM_bt:
    if (insn->types[0] == TP_int) nm = BC_NM_ibt;
    break;
  case BC_NM_bf:
    if (insn->types[0] == TP_int) nm = BC_NM_ibf;
    break;
  case BC_NM_bfni:
    if (insn->types[0] == TP_int) nm = BC_NM_ibfni;
    break;
  case BC_NM_bteq:
    if (insn->types[1] == TP_int && insn->types[2] == TP_int)
      nm = BC_NM_ibteq;
  ...
  case BC_NM_btgt:
    ...
    break;
  case BC_NM_bteqi:
    if (insn->types[1] == TP_int) nm = BC_NM_ibteqi;
    break;
  case BC_NM_btnei:
    if (insn->types[1] == TP_int) nm = BC_NM_ibtnei;
    break;
  case BC_NM_btgei:
    if (insn->types[1] == TP_int) nm = BC_NM_ibtgei;
    break;
  case BC_NM_btlti:
    if (insn->types[1] == TP_int) nm = BC_NM_ibtlti;
    break;
  case BC_NM_btlei:
    if (insn->types[1] == TP_int) nm = BC_NM_ibtlei;
    break;
  case BC_NM_btgti:
    if (insn->types[1] == TP_int) nm = BC_NM_ibtgti;
    break;
  case BC_NM_out:
  case BC_NM_stpop:
  case BC_NM_bend:
  ...
  case BC_NM_rmatch:
  case BC_NM_rmatchs:
  case BC_NM_compile: break;
  default: d_assert (FALSE);
  }
  if (BC_NODE_MODE (bc) != nm) BC_SET_MODE (bc, nm);
  if (exchange_p) {
    int t = BC_op2 (bc);
...
static void specialize_program (void) {
  int i;

  for (i = 0; i < insns_num; i++) specialize_insn (insns[i]);
}

static void print_op (df_insn_t insn, int num) {
  BC_node_t bc = insn->bc;
...
static inline const char *get_type_name (type_t t) {
  assert (0 <= t && t <= TP_varying && t < sizeof (type_name) / sizeof (char *));
  return type_name[t];
}

static int print_defs_uses (df_insn_t insn, int indent) {
  int n, i, res = FALSE;
  def_use_t du;
  ...
  for (n = i = 0;; i++) {
    if (get_insn_op_place (insn, i, TRUE) == NO_MORE_OPERAND) break;
    if (n % 10 == 0) {
      if (n != 0) printf ("\n");
      print_indent (indent);
      printf (n == 0 ? "  //  types:" : "  //    ");
  ...
  for (i = 0;; i++) {
    if (get_insn_op_place (insn, i, FALSE) == NO_MORE_OPERAND) break;
    if (i == 0) {
      if (n != 0)
        printf (" <=");
      else {
...
int print_inference_info_before_insn (BC_node_t info, int indent) {
  int n, res = FALSE;
  df_insn_t insn;
  cfg_bb_t bb;
  BC_node_t bc;
  ...
  if (bb->head == insn) {
    ...
    for (e = bb->srcs; e != NULL; e = e->next_src) {
      ...
      if (e->next_src != NULL) printf (",");
    }
    printf ("\n");
    if (block->bc_block == bc) {
      BC_node_t bc_decl;

      for (n = 0, bc_decl = BC_decls (bc); bc_decl != NULL; bc_decl = BC_next_decl (bc_decl)) {
        if (!BC_IS_OF_TYPE (bc_decl, BC_NM_vdecl) || !block->escape_p[BC_var_num (bc_decl)])
          continue;
        if (n % 10 == 0) {
          if (n != 0) printf ("\n");
          print_indent (indent);
...
int print_inference_info_after_insn (BC_node_t info, int indent) {
  df_insn_t insn;
  cfg_bb_t bb;
  cfg_edge_t e;
...
void initiate_inference_pass (void) {
  ...
  VARR_CREATE (df_insn_t, def_insns_varr, 0);
  insns_num = all_insns_num = def_insns_num = 0;
  VARR_CREATE (cfg_bb_t, bbs_varr, 0);
}

void inference_pass (BC_node_t top_block, VARR (BC_node_t) * all_fblocks) {
  BC_node_t *bc_ptr;
  ...
  for (size_t i = 0; i < VARR_LENGTH (BC_node_t, all_fblocks); i++)
    create_insns (VARR_GET (BC_node_t, all_fblocks, i));
...
void finish_inference_pass (void) {
  finish_bbs ();
  VARR_DESTROY (cfg_bb_t, bbs_varr);
