...
typedef struct option_description option_description_t;
DEF_VARR (option_description_t);
...
static VARR (option_description_t) * all_options;
...
static int compare_options (const void *option1, const void *option2) {
...
int start_command_line_processing (int argc, char **argv, const char *description) {
  VARR (char) * suffixes, *name;
  int suffix_length;
  char *end_name, *str, *start;
  struct option_description option_description;

  argument_vector = argv;
  argument_count = argc;
  description_of_options = description;
  VARR_CREATE (option_description_t, all_options, 50);
  /* Processing first part of the description (see file header above). */
  VARR_CREATE (char, suffixes, 100);
  for (;;) {
    while (*description == ' ' || *description == '\t' || *description == '\n') description++;
    if (*description == '\0' || (*description == '%' && description[1] == '%')) break;
    do {
      VARR_PUSH (char, suffixes, *description);
      description++;
    } while (*description != '\0' && *description != ' ' && *description != '\t'
             && *description != '\n' && (*description != '%' || description[1] != '%'));
    VARR_PUSH (char, suffixes, '\0');
  }
  if (*description != '%') return 1 /* TRUE */;
  /* Processing the second part of the description. */
  for (;;) {
    while (*description != '\0' && *description != '`') description++;
    if (*description == '\0') {
      /* End of the description.  Sorting option descriptions. */
      qsort (VARR_ADDR (option_description_t, all_options),
             VARR_LENGTH (option_description_t, all_options), sizeof (struct option_description),
             compare_options);
      return 1 /* TRUE */;
    }
    description++;
    if (*description != '-') {
      ...
      VARR_DESTROY (option_description_t, all_options);
      VARR_DESTROY (char, suffixes);
      return 0 /* FALSE */;
    }
    VARR_CREATE (char, name, 50);
    do {
      VARR_PUSH (char, name, *description);
      description++;
    } while (*description != '\0' && *description != '\'');
    VARR_PUSH (char, name, '\0');
    if (*description == '\0') {
    ...
    end_name = VARR_ADDR (char, name) + VARR_LENGTH (char, name) - 1;
    /* Testing suffix presence. */
    option_description.option_suffix = NULL;
    option_description.separate_suffix_option = 0 /* FALSE */;
    for (str = start = VARR_ADDR (char, suffixes); str < start + VARR_LENGTH (char, suffixes);
         str += suffix_length + 1) {
      ...
      if (suffix_length + 2 <= VARR_LENGTH (char, name) - 1
          && strcmp (str, end_name - suffix_length) == 0) {
        option_description.option_suffix = str;
        end_name -= suffix_length;
        while (*(end_name - 1) == ' ' || *(end_name - 1) == '\t' || *(end_name - 1) == '\n') {
          option_description.separate_suffix_option = 1 /* TRUE */;
          suffix_length++;
          end_name--;
        }
        VARR_TRUNC (char, name, VARR_LENGTH (char, name) - suffix_length - 1);
        VARR_PUSH (char, name, '\0');
        break;
      }
    }
    VARR_TAILOR (char, name, VARR_LENGTH (char, name));
    option_description.option_name = VARR_ADDR (char, name);
    VARR_PUSH (option_description_t, all_options, option_description);
  }
}
...
void output_command_line_description (void) {
  ...
  for (current_char_ptr = description_of_options;
       *current_char_ptr != '\0' && (*current_char_ptr != '%' || current_char_ptr[1] != '%');)
    current_char_ptr++;
  if (*current_char_ptr == '%') current_char_ptr += 2;
  fprintf (stderr, "%s\n", current_char_ptr);
}
...
static struct option_description *option_description (int argument_number) {
  ...
  option_description_table = VARR_ADDR (option_description_t, all_options);
  for (left = 0, right = VARR_LENGTH (option_description_t, all_options) - 1; left <= right;) {
    middle = (left + right) / 2;
    comparison_code = strcmp (option_description_table[middle].option_name, option_in_command_line);
  ...
  for (option_description_ptr = option_description_table + right;
       option_description_ptr >= VARR_ADDR (option_description_t, all_options)
    ...
    if (option_description_ptr->option_suffix != NULL
        ...
        && strncmp (option_description_ptr->option_name, option_in_command_line,
                    strlen (option_description_ptr->option_name))
             == 0)
...
int next_operand (int flag_of_first) {
  ...
  for (; next_operand_number < argument_count; next_operand_number++) {
    if (only_operands || *argument_vector[next_operand_number] != '-'
      ...
      if (option_description_ptr != NULL && option_description_ptr->separate_suffix_option)
        next_operand_number++;
    }
  }
  return 0;
...
int number_of_operands (void) {
  ...
  result = 0;
  for (current_operand_number = next_operand (1 /* TRUE */); current_operand_number != 0;
...
char *option_characteristics (int argument_number, int *option_has_argument) {
  struct option_description *option_description_ptr;
...
int last_option_place (const char *option_name) {
  ...
  while (argument_number != 0) {
    ...
    if (current_option_name != NULL && strcmp (current_option_name, option_name) == 0)
