...
/* Context error.  */
static void cont_err (position_t pos, const char *format, ...) {
  va_list arguments;

  va_start (arguments, format);
  d_verror (FALSE, pos, format, arguments);
  va_end (arguments);
  if (repl_flag) longjmp (context_exit_longjump_buff, 1);
}

/* Contex error followed by an append message.  */
static void cont_err_start (position_t pos, const char *format, ...) {
  va_list arguments;
...
static void cont_err_finish (position_t pos, const char *format, const char *str) {
  if (!repl_flag)
...
/* Report error MSG if DES is a slice.  Use position of POS_NODE for
   this.  */
static void check_slice (IR_node_t pos_node, IR_node_t des, const char *msg) {
  if (des == NULL || !IR_IS_OF_TYPE (des, IR_NM_slice)) return;
...
static IR_node_t include_decl (IR_node_t decl) {
  IR_node_t result;

  result = include_node (decl);
  if (IR_pos (decl).file_name != NULL && IR_IS_OF_TYPE (decl, IR_NM_fun_class))
...
static void set_field_ident_number (IR_node_t unique_ident) {
  if (IR_field_ident_number (unique_ident) >= 0) return;
...
/* The following recursive func passes (correctly setting up
   ...
   the expression.  */
static void first_expr_processing (IR_node_t expr, enum pattern_type pattern) {
  if (expr == NULL) return;
  ...
  case IR_NM_type_type: break;
  case IR_NM_this: {
    IR_node_t scope;

    for (scope = curr_scope; scope != NULL; scope = IR_block_scope (scope))
      if (IR_fun_class (scope) != NULL) break;
    if (scope == NULL)
  ...
  case IR_NM_ident: {
    ...
    if (decl == NULL) {
      ...
      include_decl (decl);
      cont_err (source_position, ERR_undeclared_ident, IR_ident_string (IR_unique_ident (expr)));
    }
    break;
  }
  case IR_NM_period: {
    ...
    if (des != NULL && IR_IS_OF_TYPE (des, IR_NM_period)
        && (decl = IR_decl (IR_component (des))) != NULL && IR_IS_OF_TYPE (decl, IR_NM_var))
      obj_block = IR_obj_block (decl);
    else if (des != NULL && IR_IS_OF_TYPE (des, IR_NM_ident) && (decl = IR_decl (des)) != NULL) {
      if (IR_IS_OF_TYPE (decl, IR_NM_expose_decl)) decl = IR_decl_in_object (decl);
      if (IR_IS_OF_TYPE (decl, IR_NM_var)) obj_block = IR_obj_block (decl);
    }
    if (obj_block != NULL) {
      IR_set_decl (IR_component (expr), find_decl_in_given_scope (IR_component (expr), obj_block));
      if (IR_decl (IR_component (expr)) == NULL)
        cont_err (source_position, ERR_no_public_declaration_in_object,
                  IR_ident_string (IR_unique_ident (IR_component (expr))));
    }
    set_field_ident_number (IR_unique_ident (IR_component (expr)));
    check_slice (expr, IR_designator (expr), ERR_period_ident_applied_to_slice);
  } break;
  ...
  case IR_NM_minus: {
    ...
    SET_SOURCE_POSITION (expr);
    first_expr_processing (IR_left_operand (expr), no_patterns);
    if (r != NULL && IR_IS_OF_TYPE (r, IR_NM_int) && IR_int_value (IR_unique_int (r)) != MAX_RINT) {
      IR_set_right_operand (expr, get_int_node (-IR_int_value (IR_unique_int (r)), IR_pos (r)));
  ...
  case IR_NM_tab:
    SET_SOURCE_POSITION (expr);
    {
      ...
      for (elements_number = 0, elist = IR_elist (expr); elist != NULL; elist = next) {
        ...
        /* We permits `...' as element (key without val) in the
           table pattern.  */
        tab_dots_p
          = (pattern == any_pattern && rep != NULL && !vec_p && IR_IS_OF_TYPE (rep, IR_NM_dots));
        ...
        first_expr_processing (elist_expr,
                               /* wildcard pattern is permitted in
                                  ...
                                  on top level.  */
                               pattern != wildcard_pattern ? pattern : no_patterns);
  ...
  case IR_NM_index:
    ...
    first_expr_processing (IR_designator (expr), no_patterns);
    check_slice (expr, IR_designator (expr), ERR_vec_tab_element_access_applied_to_slice);
  ...
  case IR_NM_class_fun_fiber_call:
    SET_SOURCE_POSITION (expr);
    {
      ...
      for (parameters_number = 0, elist = IR_actuals (expr); elist != NULL; elist = next) {
        next = IR_next_elist (elist);
        parameters_number++;
        SET_SOURCE_POSITION (elist);
        d_assert (IR_repetition_key (elist) == NULL && !IR_repetition_key_flag (elist));
        elist_expr = IR_expr (elist);
        first_expr_processing (elist_expr, pattern);
        if (pattern == any_pattern && elist_expr != NULL && IR_IS_OF_TYPE (elist_expr, IR_NM_dots)
            && next != NULL)
          cont_err (IR_pos (elist_expr), ERR_dots_in_the_list_middle);
      }
      IR_set_class_fun_fiber_call_parameters_number (expr, parameters_number);
      break;
    case IR_NM_wildcard:
      if (pattern == no_patterns) cont_err (IR_pos (expr), ERR_wildcard_in_expression);
      break;
    case IR_NM_dots:
      if (pattern != any_pattern) cont_err (IR_pos (expr), ERR_dots_in_expression);
      break;
    }
...
static IR_node_t first_block_passing (IR_node_t first_level_stmt, int curr_block_level);
...
/* Given designator DES return the correspoding decl, set up
   ...
   return NULL.  */
static IR_node_t find_expose_designator_decl (IR_node_t des, IR_node_t *top_object_class) {
  ...
  if (IR_IS_OF_TYPE (des, IR_NM_ident)) {
    ...
    *top_object_class = NULL;
    if (decl != NULL && IR_IS_OF_TYPE (decl, IR_NM_expose_decl)) decl = IR_decl_in_object (decl);
    if (decl == NULL)
      cont_err (IR_pos (des), ERR_undeclared_ident, IR_ident_string (IR_unique_ident (des)));
    else if (IR_IS_OF_TYPE (decl, IR_NM_var) && (obj_block = IR_obj_block (decl)) != NULL)
    ...
    /* It is a sequence (class; block; obj var) and we did not
      ...
      d_assert (!(IR_IS_OF_TYPE (decl, IR_NM_class) && IR_next_stmt (decl) != NULL
                  ...
                  && (IR_obj_block (IR_next_stmt (IR_next_stmt (decl))) == IR_next_stmt (decl))));
    return decl;
  }
  ...
  if (!IR_IS_OF_TYPE (decl, IR_NM_var) || (obj_block = IR_obj_block (decl)) == NULL)
    decl = NULL;
  else
...
/* Create expose declaration with given parameters and insert it into
   table.  */
static void create_expose_decl (IR_node_t decl, IR_node_t top_object_class, IR_node_t expose_alias,
                                IR_node_t pos_node) {
  IR_node_t expose_decl = create_node_with_pos (IR_NM_expose_decl, IR_pos (pos_node));
  IR_set_next_stmt (expose_decl, NULL);
  IR_set_scope (expose_decl, curr_scope);
  IR_set_ident (expose_decl, expose_alias != NULL ? expose_alias : IR_ident (decl));
...
/* Process friend list of BLOCK and create the friends for
   the current scope. */
static void process_friend_list (IR_node_t block) {
  IR_node_t curr_friend_ident, friend, decl;
  IR_node_t scope = block;

  for (curr_friend_ident = IR_friend_list (block); curr_friend_ident != NULL;
       curr_friend_ident = IR_next_friend_ident (curr_friend_ident)) {
    decl = find_decl (IR_ident_in_clause (curr_friend_ident), scope);
    if (decl == NULL)
      cont_err (IR_pos (IR_ident_in_clause (curr_friend_ident)), ERR_undeclared_ident_friend_list,
                IR_ident_string (IR_unique_ident (IR_ident_in_clause (curr_friend_ident))));
    else if (!IR_IS_OF_TYPE (decl, IR_NM_fun_or_class))
      cont_err (IR_pos (IR_ident_in_clause (curr_friend_ident)), ERR_invalid_friend,
                IR_ident_string (IR_unique_ident (IR_ident_in_clause (curr_friend_ident))));
    else {
...
static void set_block_level (IR_node_t block, int level) {
  IR_set_block_level (block, level);
  if (max_block_level < level) max_block_level = level;
}
...
/* Make copied redirs starting with index START flat.  It means make
   ...
   non-flat redirs.  */
static void make_flat_redirs (int start) {
  int i, n;
  ...
  for (i = start; i < n; i++) {
    redir = redirs[i];
    to = IR_to (redir);
    if (!IR_IS_OF_TYPE (to, IR_NM_decl_redir)) continue;
...
static int use_item_cmp (const void *p1, const void *p2) {
  int diff;
  ...
  if ((diff = strcmp (id1, id2)) != 0) return diff;
...
static long int find_use_item_index (IR_node_t ident, IR_node_t use_clause) {
  long int l, r, m;
  int cmp;
  IR_node_t item, muid;
...
static int check_decl_matching (IR_node_t prev_decl, IR_node_t decl) {
  if (IR_NODE_MODE (decl) != IR_NODE_MODE (prev_decl)
      || (IR_NODE_MODE (decl) == IR_NM_fun && IR_fiber_flag (prev_decl) != IR_fiber_flag (decl))) {
    cont_err_start (IR_pos (decl), ERR_forward_and_matched_decls_are_different_entities,
                    IR_ident_string (IR_unique_ident (IR_ident (decl))));
    cont_err_finish (IR_pos (IR_ident (prev_decl)), ERR_previous_decl_location,
                     IR_ident_string (IR_unique_ident (IR_ident (decl))));
    return FALSE;
  }
  if ((IR_IS_OF_TYPE (decl, IR_NM_fun_class) && IR_final_flag (prev_decl) != IR_final_flag (decl))
      || IR_access (prev_decl) != IR_access (decl)) {
    cont_err_start (IR_pos (IR_ident (decl)), ERR_forward_and_matched_decls_have_different_attrs,
                    IR_ident_string (IR_unique_ident (IR_ident (decl))));
    cont_err_finish (IR_pos (IR_ident (prev_decl)), ERR_previous_decl_location,
                     IR_ident_string (IR_unique_ident (IR_ident (decl))));
    return FALSE;
...
static void set_next_stmt (IR_node_t stmt, IR_node_t next) {
  IR_set_next_stmt (stmt, next);
  if (next != NULL && IR_IS_OF_TYPE (next, IR_NM_fun_or_class) && IR_forward_decl_flag (next))
    IR_set_forward_prev (next, stmt);
}

/* Add STMTS to statement TO.  If TO is NULL, set up FIRST to
   STMTS.  */
static void add_to_stmt (IR_node_t stmts, IR_node_t to, IR_node_t *first) {
  if (to == NULL)
    *first = stmts;
  else
...
static IR_node_t make_alias (IR_node_t item, IR_node_t orig_decl, IR_node_t use_clause,
  ...
  if ((ident = IR_alias (item)) != NULL) {
    ...
    if (decl != NULL && (!IR_alias_flag (decl) || IR_use_clause (decl) != use_clause)) {
      cont_err_start (IR_pos (use_clause), ERR_alias_redefines_prev_one,
                      IR_ident_string (IR_unique_ident (ident)));
      cont_err_finish (IR_pos (IR_ident (decl)), ERR_previous_decl_location,
                       IR_ident_string (IR_unique_ident (ident)));
    } else {
...
static IR_node_t process_use_clause (IR_node_t use_clause, IR_node_t origin_block,
  ...
  for (start_ptr = VARR_ADDR (IR_node_t, use_items),
    ...
    if (IR_redefine_flag (*item_ptr)) {
      ...
      if (!err_p) {
        err_p = TRUE;
        cont_err_start (IR_pos (IR_use_item_ident (*item_ptr)),
                        ERR_repeated_use_item_ident_occurence_in_use,
                        IR_ident_string (IR_unique_ident (IR_use_item_ident (*item_ptr))));
        cont_err_finish (IR_pos (IR_use_item_ident (item_ptr[1])),
                         ERR_previous_use_item_ident_location,
                         IR_ident_string (IR_unique_ident (IR_use_item_ident (item_ptr[1]))));
      }
    } else {
      err_p = FALSE;
...
/* Copy attributes of non-forward fun/class decl FROM to fun/class
   decl TO.  */
static void copy_fun_class_attrs (IR_node_t to, IR_node_t from) {
  IR_node_t next_stmt = IR_next_stmt (from);

  d_assert (IR_IS_OF_TYPE (to, IR_NM_fun_or_class) && IR_IS_OF_TYPE (from, IR_NM_fun_or_class)
            && !IR_forward_decl_flag (from) && next_stmt != NULL
            && IR_NODE_MODE (next_stmt) == IR_NM_block);
  IR_set_forward_decl_flag (to, FALSE); /* Make it non-forward */
  IR_set_parameters_number (to, IR_parameters_number (from));
  IR_set_min_actual_parameters_number (to, IR_min_actual_parameters_number (from));
...
static int process_redecl_after (IR_node_t prev_decl, IR_node_t curr_decl) {
  long int n;
  ...
  if (prev_decl != NULL
    ...
    cont_err_start (IR_pos (id), ERR_alias_redefinition, IR_ident_string (uid));
    cont_err_finish (IR_pos (IR_ident (prev_decl)), ERR_previous_decl_location,
                     IR_ident_string (uid));
    return TRUE;
  }
  ...
  if (n >= 0) {
    ...
    for (; n < VARR_LENGTH (IR_node_t, use_items); n++) {
      ...
      IR_set_redefine_flag (item, TRUE);
      for (redir = IR_redirs (item); redir != NULL; redir = IR_next_redir (redir))
        if (check_decl_matching (IR_redir_origin (redir), curr_decl)) IR_set_to (redir, curr_decl);
    }
  }
  if (prev_decl == NULL || IR_use_clause (prev_decl) == IR_use_clause (curr_decl)) return FALSE;
  ...
  d_assert (IR_use_clause (prev_decl) != NULL && IR_use_clause (curr_decl) == NULL);
  cont_err_start (IR_pos (IR_ident (curr_decl)), ERR_used_decl_redefinition_not_mentioned_in_later,
                  IR_ident_string (uid));
  cont_err_finish (IR_pos (IR_use_clause (prev_decl)), ERR_here_is_corresponding_use_clause,
                   IR_ident_string (uid));
  return TRUE;
}
...
/* Add DECL to the current block.  Add also all uses of DECL block
   recursively. */
static void add_block_use (IR_node_t decl) {
  IR_node_t use, block;

  d_assert (IR_IS_OF_TYPE (decl, IR_NM_fun_class));
  block = IR_next_stmt (decl);
  d_assert (block != NULL && IR_IS_OF_TYPE (block, IR_NM_block));
  for (use = IR_block_uses (curr_scope); use != NULL; use = IR_next_block_use (use))
    if (IR_use_decl (use) == decl) return; /* Already there  */
...
static void add_block_to_tree (IR_node_t block) {
  IR_node_t child;

  if (curr_scope == NULL) return;
...
static IR_node_t create_pattern_vars (IR_node_t pattern_from, IR_node_t expr, IR_node_t last) {
  IR_node_t elist;

  if (expr == NULL) return last;
  switch (IR_NODE_MODE (expr)) {
  case IR_NM_ident: {
    IR_node_t decl;
...
/* Process decl with given previous, next and first level statement.
   ...
   current.  */
static IR_node_t process_decl (IR_node_t stmt, IR_node_t *prev_stmt_ptr, IR_node_t next_stmt,
  ...
  if (IR_access (stmt) == DEFAULT_ACCESS)
    IR_set_access (stmt, curr_scope != NULL && IR_fun_class (curr_scope) != NULL
  ...
  if (process_redecl_after (prev_decl, stmt)) return stmt;
  d_assert (prev_decl == NULL || IR_use_clause (prev_decl) == IR_use_clause (stmt));
  if (node_mode == IR_NM_var) {
    ...
    if (IR_par_flag (stmt) && (scope = IR_scope (stmt)) != NULL
      ...
      IR_set_parameters_number (fun_class, IR_parameters_number (fun_class) + 1);
    }
  } else if (prev_decl == NULL || !IR_IS_OF_TYPE (prev_decl, IR_NM_fun_or_class)
    ...
    /* A forward fun/class decl followed by non-forward one
       ...
       decl. */
    d_assert (next_stmt != NULL && IR_IS_OF_TYPE (next_stmt, IR_NM_block));
    ...
    if (*prev_stmt_ptr == prev_decl) *prev_stmt_ptr = redir;
...
/* Put LIST of vars before stmt.  Modify previous or first level
   statement of STMT.  */
static void put_var_list_before (IR_node_t list, IR_node_t stmt, IR_node_t *prev_stmt_ptr,
                                 IR_node_t *first_level_stmt_ptr) {
  ...
  for (el = list; el != NULL; *prev_stmt_ptr = el, el = next_el) {
    ...
    process_decl (el, prev_stmt_ptr, next_el != NULL ? next_el : stmt, first_level_stmt_ptr, TRUE);
  }
...
static IR_node_t first_block_passing (IR_node_t first_level_stmt, int curr_block_level) {
  ...
  for (prev_stmt = NULL, stmt = first_level_stmt; stmt != NULL;
       prev_stmt = stmt, stmt = next_stmt) {
    next_stmt = IR_next_stmt (stmt);
    SET_SOURCE_POSITION (stmt);
    node_mode = IR_NODE_MODE (stmt);
    switch (node_mode) {
    case IR_NM_expr_stmt: first_expr_processing (IR_stmt_expr (stmt), no_patterns); break;
    case IR_NM_assign:
    ...
    case IR_NM_minus_assign: {
      ...
      if (expr != NULL && IR_IS_OF_TYPE (expr, IR_NM_int)
        ...
        IR_set_assignment_expr (stmt,
                                get_int_node (-IR_int_value (IR_unique_int (expr)), IR_pos (expr)));
    ...
    case IR_NM_if_stmt:
      ...
      IR_set_if_part (stmt, first_block_passing (IR_if_part (stmt), curr_block_level));
      IR_set_else_part (stmt, first_block_passing (IR_else_part (stmt), curr_block_level));
      break;
    case IR_NM_for_stmt:
      IR_set_for_initial_stmt (stmt,
                               first_block_passing (IR_for_initial_stmt (stmt), curr_block_level));
      first_expr_processing (IR_for_guard_expr (stmt), no_patterns);
      IR_set_for_iterate_stmt (stmt,
                               first_block_passing (IR_for_iterate_stmt (stmt), curr_block_level));
      IR_set_for_stmts (stmt, first_block_passing (IR_for_stmts (stmt), curr_block_level));
      break;
    ...
    case IR_NM_return_without_result: break;
    case IR_NM_return_with_result:
      first_expr_processing (IR_returned_expr (stmt), no_patterns);
      break;
    case IR_NM_throw: first_expr_processing (IR_throw_expr (stmt), no_patterns); break;
    case IR_NM_wait_stmt:
      first_expr_processing (IR_wait_guard_expr (stmt), no_patterns);
      IR_set_wait_stmt (stmt, first_block_passing (IR_wait_stmt (stmt), curr_block_level));
      break;
    ...
    case IR_NM_block: {
      ...
      if (IR_case_pattern (stmt) != NULL) {
        ...
        int pmatch_p = (match_stmt != NULL && IR_IS_OF_TYPE (match_stmt, IR_NM_pmatch_stmt));
        IR_node_t list = NULL, prev = NULL, first = IR_block_stmts (stmt);

        if (pmatch_p)
          list = create_pattern_vars (stmt, IR_case_pattern (stmt), NULL);
        else if (match_stmt != NULL && IR_IS_OF_TYPE (match_stmt, IR_NM_rmatch_stmt)) {
          IR_node_t decl = create_node_with_pos (IR_NM_var, IR_pos (stmt));

          IR_set_rmatch_result_var (stmt, decl);
          IR_set_next_stmt (decl, decl);
          IR_set_ident (decl, get_ident_node (RMATCH_RESULT_NAME, IR_pos (stmt)));
          IR_set_scope (decl, curr_scope);
          IR_set_access (decl, DEFAULT_ACCESS);
        ...
        first_expr_processing (IR_case_pattern (stmt), pmatch_p ? any_pattern : wildcard_pattern);
        first_expr_processing (IR_case_cond (stmt), no_patterns);
      }
      IR_set_block_stmts (stmt, first_block_passing (IR_block_stmts (stmt), curr_block_level + 1));
      ...
      if (curr_fdecl_flag || IR_exceptions (stmt) != NULL)
        for (curr_block = stmt; curr_block != NULL && IR_inline_flag (curr_block);
             curr_block = IR_block_scope (curr_block))
          IR_set_inline_flag (curr_block, FALSE);
      fun_class = IR_fun_class (stmt);
      if (IR_exceptions (stmt) != NULL || fun_class != NULL) IR_set_inline_flag (stmt, FALSE);
      if (!IR_inline_flag (stmt)) finish_block_inter_stmt_holders (block_foreach_start);
      curr_scope = saved_curr_scope;
      curr_fdecl_flag = saved_curr_fdecl_flag;
      for (curr_except = IR_exceptions (stmt); curr_except != NULL;
        ...
        if (IR_catch_block (curr_except) != NULL) {
          ...
          IR_set_catch_block (curr_except,
                              first_block_passing (IR_catch_block (curr_except), curr_block_level));
        }
      }
    ...
    case IR_NM_external_fun: {
      IR_node_t block, prev_decl, redir, ident = IR_ident (stmt);
      int assign_p = (node_mode == IR_NM_var && next_stmt != NULL
      ...
      stmt = process_decl (stmt, &prev_stmt, next_stmt, &first_level_stmt,
                           assign_p && (IR_NODE_MODE (next_stmt) == IR_NM_par_assign));
      break;
    }
    case IR_NM_pattern_var: {
      IR_node_t list;
      int assign_p = (next_stmt != NULL
      ...
      if (assign_p)
        /* Process idents in init expr before the
           declaration.  */
        first_expr_processing (IR_assignment_expr (next_stmt), no_patterns);
      list = uncycle_stmt_list (create_pattern_vars (stmt, IR_pattern (stmt), NULL));
      if (list == NULL)
    ...
    case IR_NM_expose: {
      ...
      decl = find_expose_designator_decl (IR_expose_designator (stmt), &top_object_class);
      if (decl != NULL) {
        if (!IR_expose_internals_flag (stmt))
          create_expose_decl (decl, top_object_class, IR_expose_alias (stmt), stmt);
        else if (!IR_IS_OF_TYPE (decl, IR_NM_var) || (obj_block = IR_obj_block (decl)) == NULL)
          cont_err (IR_pos (stmt), ERR_non_object_in_expose_clause);
        else
          for (block_stmt = IR_block_stmts (obj_block); block_stmt != NULL;
               block_stmt = IR_next_stmt (block_stmt))
            if (IR_IS_OF_TYPE (block_stmt, IR_NM_decl) && IR_access (block_stmt) == PUBLIC_ACCESS)
              create_expose_decl (block_stmt, top_object_class, NULL, stmt);
      }
    ...
    case IR_NM_use: {
        ...
        if (!err_p && decl == NULL) {
          cont_err (IR_pos (des), ERR_non_object_in_use_qual_ident,
                    IR_ident_string (IR_unique_ident (
                      IR_IS_OF_TYPE (des, IR_NM_ident) ? des : IR_component (des))));
          err_p = TRUE;
        }
      }
      if (decl != NULL && IR_IS_OF_TYPE (decl, IR_NM_expose_decl)) decl = IR_decl_in_object (decl);
      ...
      if (decl == NULL) {
        ...
        cont_err (IR_pos (stmt), ERR_use_of_non_fun_class,
                  IR_ident_string (IR_unique_ident (ident)));
      else if (IR_forward_decl_flag (decl))
        cont_err (IR_pos (stmt), ERR_use_of_forward_declaration,
                  IR_ident_string (IR_unique_ident (ident)));
      else if (IR_final_flag (decl))
        cont_err (IR_pos (stmt), ERR_use_of_final, IR_ident_string (IR_unique_ident (ident)));
      else {
        for (scope = curr_scope; scope != NULL; scope = IR_block_scope (scope))
          if (IR_fun_class (scope) == decl) {
            cont_err (IR_pos (qual_ident), ERR_use_of_class_inside_the_class,
                      IR_ident_string (IR_unique_ident (ident)));
            break;
          }
        if (scope == NULL) {
          subst = process_use_clause (stmt, IR_next_stmt (decl), next_stmt);
          add_block_use (decl);
        }
      }
      ...
      stmt = prev_stmt;
      if (subst != NULL) /* The used clause might be empty.  */
        next_stmt = subst;
    } break;
    case IR_NM_decl_redir: break;
    case IR_NM_require: {
      ...
      if (IR_IS_OF_TYPE (version, IR_NM_int))
        ...
        v = (rfloat_t) ((rint_t) (IR_float_value (IR_unique_float (version)) * 100.0 + 0.5))
            / 100.0;
      else if (mpz_ok_for_rint_p (*IR_mpz_ptr (IR_unique_long (version))))
        v = (rfloat_t) mpz2i (*IR_mpz_ptr (IR_unique_long (version)));
      else {
        d_error (!repl_flag, IR_pos (IR_required_version (stmt)), ERR_too_huge_version);
        if (repl_flag) longjmp (context_exit_longjump_buff, 1);
      }
      if (incompatible_lang_version_p (v)) {
        d_error (!repl_flag, IR_pos (IR_required_version (stmt)), ERR_incompatible_version,
                 DINO_LANG_VERSION, v);
        if (repl_flag) longjmp (context_exit_longjump_buff, 1);
      }
      break;
...
static int compare_codes (const void *n1, const void *n2) {
  int res;
...
static void inline_blocks (IR_node_t block, IR_node_t container, int vars_base) {
...
static do_inline int value_type (IR_node_t expr) {
  if (expr == NULL) return EVT_UNKNOWN;
...
static void type_test (IR_node_t expr, type_mask_t type_mask, char *message) {
  if (expr == NULL) return;
  if (!IT_IS_OF_TYPE (expr, type_mask)) cont_err (source_position, message);
}
...
static void inline link_info (BC_node_t info) {
  d_assert (info != NULL && BC_IS_OF_TYPE (info, BC_NM_info) && BC_prev_info (info) == NULL
            && BC_next_info (info) == NULL);
  if (curr_info != NULL) BC_set_next_info (curr_info, info);
  BC_set_prev_info (info, curr_info);
  curr_info = info;
}

/* Unlink the last info node.  */
static void inline unlink_last_info (void) {
  BC_node_t prev_info;
...
/* Add NODE with info to the generated byte code.  It can be done only
   once for the NODE.  */
static void inline add_to_bcode (BC_node_t node) {
  link_info (BC_info (node));
  prev_pc = curr_pc;
  if (curr_pc != NULL) BC_set_next (curr_pc, node);
  curr_pc = node;
}
...
static int do_inline get_temp_stack_slot (int *temp_vars_num) {
...
static int do_inline setup_result_var_number (int *result, int *temp_vars_num) {
...
static BC_node_t do_inline new_bc_code (BC_node_mode_t bnm, BC_node_t origin) {
  BC_node_t res = BC_create_node (bnm);
...
static BC_node_t do_inline new_bc_code_with_src (BC_node_mode_t bnm, IR_node_t origin) {
  BC_node_t res = BC_create_node (bnm);
...
static IR_node_t second_expr_processing (IR_node_t expr, int fun_class_assign_p, int *result,
                                         int *curr_temp_vars_num, int lvalue_p, BC_node_t false_pc,
                                         BC_node_t true_pc, int next_false_p);

static void do_inline process_unary_op (IR_node_t op, int *result, int *curr_temp_vars_num) {
  ...
  IR_set_operand (op, second_expr_processing (IR_operand (op), FALSE, &op_result, &temp_vars_num,
...
static do_inline int bin_slice_p (IR_node_t l, IR_node_t r) {
  return ((l != NULL && IR_value_type (l) == EVT_SLICE)
          || (r != NULL && IR_value_type (r) == EVT_SLICE));
}

/* Return true if OP is a slice.  */
static do_inline int unary_slice_p (IR_node_t op) {
  return op != NULL && IR_value_type (op) == EVT_SLICE;
}

static do_inline int int_p (rint_t i) { return INT_MIN <= i && i <= INT_MAX; }
...
static void do_inline process_binary_op (IR_node_t op, int *result, int *curr_temp_vars_num,
  ...
  if (new_op_mode != BC_NM__error) {
    if (l != NULL && IR_IS_OF_TYPE (l, IR_NM_int)) {
      IR_set_right_operand (op, second_expr_processing (r, FALSE, &r_op_result, &temp_vars_num,
                                                        FALSE, NULL, NULL, FALSE));
      skip_right_p = TRUE;
      if (!int_p (IR_int_value (IR_unique_int (l))) || unary_slice_p (IR_right_operand (op)))
        ;
      else {
        skip_left_p = TRUE;
      ...
      IR_set_left_operand (op, second_expr_processing (l, FALSE, &l_op_result, &temp_vars_num,
                                                       FALSE, NULL, NULL, FALSE));
      skip_left_p = TRUE;
      if (!int_p (IR_int_value (IR_unique_int (r))) || unary_slice_p (IR_left_operand (op)))
        ;
      else {
        skip_right_p = TRUE;
  ...
  if (IR_left_operand (op) != NULL) BC_set_pos2 (src, IR_pos (IR_left_operand (op)));
  if (IR_right_operand (op) != NULL) BC_set_pos3 (src, IR_pos (IR_right_operand (op)));
...
/* Check EXPR whose value in OP_NUM for a slice and, if it is so, add
   flatten node.  */
static do_inline void add_flatten_node_if_necessary (IR_node_t expr, int op_num) {
  BC_node_t flat;

  if (expr != NULL && IR_value_type (expr) == EVT_SLICE) {
    flat = new_bc_code (BC_NM_flat, new_bc_node (BC_NM_source, IR_pos (expr)));
...
static BC_node_mode_t bc_decl_mode (IR_node_t decl) {
  d_assert (decl != NULL);
...
static void set_new_decl_num (BC_node_t bc_decl) {
  last_decl_num++;
...
static BC_node_t get_bcode_decl (IR_node_t decl) {
  BC_node_t bc_decl, bc_block;
  IR_node_t unique_ident, scope;

  d_assert (decl != NULL);
  if ((bc_decl = IR_bc_decl (decl)) != NULL) return bc_decl;
  if (IR_IS_OF_TYPE (decl, IR_NM_expose_decl)) decl = IR_decl_in_object (decl);
  scope = IR_scope (decl);
  if (IR_NODE_MODE (decl) == IR_NM_var) {
    bc_decl = new_bc_node (BC_NM_vdecl, IR_pos (decl));
    BC_set_var_num (bc_decl, IR_var_number_in_block (decl) + IR_vars_base (scope));
  } else if (IR_NODE_MODE (decl) == IR_NM_external_var)
...
/* The func creates bc_decl_node (var, func or class) for
   ...
   var, func or class. */
static BC_node_t create_bc_decl (IR_node_t decl, IR_node_t pos_node) {
...
static BC_node_t create_bc_ident (IR_node_t ident) {
...
static int process_actuals (IR_node_t fun_decl, int fun_op_num, IR_node_t actuals,
                            int *temp_vars_num) {
  int par_num, n;
  IR_node_t elist;

  for (n = 0, elist = actuals; elist != NULL; elist = IR_next_elist (elist), n++) {
    SET_SOURCE_POSITION (elist);
    par_num = *temp_vars_num + curr_temp_vars_base;
    IR_set_expr (elist, second_expr_processing (IR_expr (elist), TRUE, NULL, temp_vars_num, FALSE,
                                                NULL, NULL, FALSE));
...
static void add_move (IR_node_t pos_node, BC_node_t decl, int result_num, int var_num) {
...
static int ir2er_type (IR_node_mode_t irnm) {
  switch (irnm) {
...
/* Create and return copy of a bcode ORIGIN_BCODE.  It also creates a
   copy of info node.  */
static BC_node_t copy_bcode (BC_node_t origin_bcode) {
...
static inline BC_node_t get_fblock (IR_node_t fun_decl) {
  BC_node_t bc, fdecl;
  IR_node_t src, origin, block = IR_next_stmt (fun_decl);

  src = block;
  if (block == NULL || !IR_IS_OF_TYPE (block, IR_NM_block) || IR_fun_class (block) != fun_decl) {
    /* Its is a forward declaration or used declaration. */
    d_assert (IR_forward_decl_flag (fun_decl) || IR_origin_decl (fun_decl) != NULL);
    src = fun_decl;
  }
...
static BC_node_t gen_int_load (rint_t i, IR_node_t src) {
  BC_node_t bc;
...
static void gen_decl_access (IR_node_t ident, IR_node_t decl, int lvalue_p, int *result,
                             int *curr_temp_vars_num) {
  IR_node_t scope;
  BC_node_t bc;

  scope = IR_scope (decl);
  if (IR_NODE_MODE (decl) == IR_NM_var && curr_real_scope == IR_real_block_scope (scope)) {
    /* local var */
    int var_num = IR_var_number_in_block (decl) + IR_vars_base (scope);

    if (result == NULL)
      add_move (ident, get_bcode_decl (decl), setup_result_var_number (result, curr_temp_vars_num),
                var_num);
    else
      *result = var_num;
...
static BC_node_t create_obj_decl_access (IR_node_t decl, IR_node_t pos_node) {
  BC_node_t bc;
...
static IR_node_t second_expr_processing (IR_node_t expr, int fun_class_assign_p, int *result,
  ...
  d_assert ((false_pc != NULL && true_pc != NULL) || (false_pc == NULL && true_pc == NULL));
  ...
  case IR_NM_ident: {
    ...
    decl = IR_decl (ident);
    SET_SOURCE_POSITION (expr);
    if (decl == NULL || IR_scope (decl) == NULL || IR_pos (decl).file_name == NULL)
    ...
    if (fun_class_assign_p
        ...
        && (IR_IS_OF_TYPE (decl, IR_NM_fun) || IR_IS_OF_TYPE (decl, IR_NM_class))) {
  ...
  case IR_NM_period: {
    ...
    d_assert (!lvalue_p || result == NULL || *result == not_defined_result);
    SET_SOURCE_POSITION (expr);
    IR_set_designator (expr, second_expr_processing (IR_designator (expr), FALSE, &op_result,
                                                     &temp_vars_num, FALSE, NULL, NULL, FALSE));
    d_assert (IR_component (expr) != NULL && IR_NODE_MODE (IR_component (expr)) == IR_NM_ident);
    des = IR_designator (expr);
    decl = IR_decl (IR_component (expr));
    bc = NULL;
    if (decl != NULL) bc = create_obj_decl_access (decl, expr);
    if (bc == NULL) {
      bc = new_bc_code_with_src (BC_NM_fld, expr); /* ??? should be decl */
      BC_set_fldid (bc, IR_ident_string (IR_unique_ident (IR_component (expr))));
      BC_set_op3 (bc, IR_field_ident_number (IR_unique_ident (IR_component (expr))));
    }
    BC_set_op2 (bc, op_result);
    if (lvalue_p) {
      BC_set_op1 (bc, -1);
    ...
    if (des != NULL
        ...
        && !(
          ((IR_IS_OF_TYPE (des, IR_NM_ident) && (decl = IR_decl (des)) != NULL)
           || (IR_IS_OF_TYPE (des, IR_NM_period) && (decl = IR_decl (IR_component (des))) != NULL))
          && IR_IS_OF_TYPE (decl, IR_NM_var) && IR_obj_block (decl) != NULL)) {
      if (!IR_it_is_declared_in_block (IR_unique_ident (IR_component (expr))))
  ...
  case IR_NM_logical_and: {
    ...
    if (false_pc != NULL) nop = new_bc_code_with_src (BC_NM_nop, expr);
    IR_set_operand (expr,
                    second_expr_processing (IR_operand (expr), FALSE, &op_result, &temp_vars_num,
                                            FALSE, node_mode == IR_NM_logical_or ? nop : false_pc,
                                            node_mode == IR_NM_logical_or ? true_pc : nop,
                                            node_mode == IR_NM_logical_or));
    if (false_pc != NULL)
      add_to_bcode (nop);
    else {
      bc = new_bc_code_with_src (node_mode == IR_NM_logical_or ? BC_NM_brts : BC_NM_brfs, expr);
    ...
    op_result = (result == NULL ? not_defined_result : *result);
    IR_set_cont_operand (expr,
                         second_expr_processing (IR_cont_operand (expr), FALSE, &op_result,
                                                 &temp_vars_num, FALSE, false_pc, true_pc, FALSE));
    if (false_pc == NULL) {
  ...
  case IR_NM_not:
    if (false_pc != NULL) {
      ...
      second_expr_processing (IR_operand (expr), fun_class_assign_p, &op_result, curr_temp_vars_num,
                              lvalue_p, true_pc, false_pc, TRUE);
      IR_set_value_type (expr, EVT_INT);
      false_pc = true_pc = NULL;
    } else {
      process_unary_op (expr, result, curr_temp_vars_num);
      l = IR_operand (expr);
      type_test (l, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_logical_operation_operand_type);
      IR_set_value_type (expr, unary_slice_p (l) ? EVT_SLICE : EVT_INT);
    }
    break;
  case IR_NM_eq:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_eqi, BC_NM_eqi);
    goto common_eq;
  case IR_NM_ne:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_nei, BC_NM_nei);
    goto common_eq;
  case IR_NM_identity:
  case IR_NM_unidentity:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM__error, BC_NM__error);
  common_eq:
    l = IR_left_operand (expr);
    r = IR_right_operand (expr);
    IR_set_value_type (expr, bin_slice_p (l, r) ? EVT_SLICE : EVT_INT);
    break;
  case IR_NM_lt:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_lti, BC_NM_gti);
    goto common_cmp;
  case IR_NM_gt:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_gti, BC_NM_lti);
    goto common_cmp;
  case IR_NM_le:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_lei, BC_NM_gei);
    goto common_cmp;
  case IR_NM_ge:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_gei, BC_NM_lei);
  common_cmp:
    l = IR_left_operand (expr);
    r = IR_right_operand (expr);
    type_test (l, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_order_comparison_operation_operand_type);
    type_test (r, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_order_comparison_operation_operand_type);
    IR_set_value_type (expr, bin_slice_p (l, r) ? EVT_SLICE : EVT_INT);
    break;
  case IR_NM_concat:
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM__error, BC_NM__error);
    l = IR_left_operand (expr);
    r = IR_right_operand (expr);
    type_test (l, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_concat_operation_operand_type);
    type_test (r, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_concat_operation_operand_type);
  ...
  case IR_NM_ashift:
    ...
    l = IR_left_operand (expr);
    r = IR_right_operand (expr);
    type_test (l, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_arithmetic_operation_operand_type);
    type_test (r, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_arithmetic_operation_operand_type);
    IR_set_value_type (expr, bin_slice_p (l, r) ? EVT_SLICE : EVT_INT);
    break;
  case IR_NM_plus:
    l = IR_left_operand (expr);
    r = IR_right_operand (expr);
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_addi, BC_NM_addi);
    goto common_ar_op;
  case IR_NM_mult:
    l = IR_left_operand (expr);
    r = IR_right_operand (expr);
    process_binary_op (expr, result, curr_temp_vars_num, BC_NM_multi, BC_NM_multi);
    goto common_ar_op;
  case IR_NM_minus:
  case IR_NM_div:
  case IR_NM_mod:
  ...
  common_ar_op : {
    ...
    if ((type_mask1 == EVT_FLOAT || type_mask2 == EVT_FLOAT) && (type_mask1 & EVT_SLICE) == 0
      ...
      IR_set_value_type (expr, bin_slice_p (l, r) ? EVT_SLICE : EVT_NUMBER_MASK);
  } break;
  case IR_NM_unary_plus:
  case IR_NM_unary_minus: {
    ...
    if (type_mask == EVT_FLOAT)
      ...
      IR_set_value_type (expr, unary_slice_p (l) ? EVT_SLICE : EVT_NUMBER_MASK);
  } break;
  case IR_NM_bitwise_not:
    process_unary_op (expr, result, curr_temp_vars_num);
    l = IR_operand (expr);
  ...
  case IR_NM_length:
    process_unary_op (expr, result, curr_temp_vars_num);
    l = IR_operand (expr);
    type_test (l, EVT_NUMBER_VEC_TAB_SLICE_MASK, ERR_invalid_length_operand_type);
    IR_set_value_type (expr, unary_slice_p (l) ? EVT_SLICE : EVT_INT);
    break;
  case IR_NM_paren:
    IR_set_operand (expr, second_expr_processing (IR_operand (expr), fun_class_assign_p, result,
                                                  curr_temp_vars_num, lvalue_p, false_pc, true_pc,
                                                  next_false_p));
    IR_set_value_type (expr,
                       IR_operand (expr) == NULL ? EVT_UNKNOWN : IR_value_type (IR_operand (expr)));
    break;
  case IR_NM_fold_plus:
  case IR_NM_fold_mult:
    process_unary_op (expr, result, curr_temp_vars_num);
    type_test (IR_operand (expr), EVT_SLICE, ERR_invalid_fold_arithmetic_operation_operand_type);
  ...
  case IR_NM_fold_concat:
    process_unary_op (expr, result, curr_temp_vars_num);
    type_test (IR_operand (expr), EVT_SLICE, ERR_invalid_fold_concat_operation_operand_type);
    IR_set_value_type (expr, EVT_VEC);
    break;
  case IR_NM_typeof:
    process_unary_op (expr, result, curr_temp_vars_num);
    IR_set_value_type (expr, unary_slice_p (IR_operand (expr)) ? EVT_SLICE : EVT_TYPE);
    break;
  case IR_NM_charof:
    process_unary_op (expr, result, curr_temp_vars_num);
    l = IR_operand (expr);
    type_test (l, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_conversion_to_char_operand_type);
  ...
  case IR_NM_longof:
    ...
    type_test (l, EVT_NUMBER_VEC_SLICE_MASK,
               node_mode == IR_NM_intof ? ERR_invalid_conversion_to_int_operand_type
  ...
  case IR_NM_floatof:
    process_unary_op (expr, result, curr_temp_vars_num);
    l = IR_operand (expr);
    type_test (l, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_conversion_to_float_operand_type);
    IR_set_value_type (expr, unary_slice_p (l) ? EVT_SLICE : EVT_FLOAT);
    break;
  case IR_NM_vecof:
    process_unary_op (expr, result, curr_temp_vars_num);
    l = IR_operand (expr);
    type_test (l, EVT_NUMBER_VEC_TAB_SLICE_MASK, ERR_invalid_conversion_to_vector_operand_type);
  ...
  case IR_NM_tabof:
    process_unary_op (expr, result, curr_temp_vars_num);
    l = IR_operand (expr);
    type_test (l, EVT_NUMBER_VEC_TAB_SLICE_MASK, ERR_invalid_conversion_to_table_operand_type);
  ...
  case IR_NM_cond: {
    ...
    SET_SOURCE_POSITION (expr);
    op_result = not_defined_result;
    IR_set_cond_expr (expr, second_expr_processing (IR_cond_expr (expr), FALSE, &op_result,
                                                    &temp_vars_num, FALSE, NULL, NULL, FALSE));
    ...
    temp_vars_num = *curr_temp_vars_num;
    /* Put it on stack.  */
    IR_set_true_expr (expr, second_expr_processing (IR_true_expr (expr), fun_class_assign_p, NULL,
                                                    &temp_vars_num, FALSE, NULL, NULL, FALSE));
    d_assert (temp_vars_num == *curr_temp_vars_num + 1);
    true_path_end = BC_bc (curr_info);
    temp_vars_num = *curr_temp_vars_num;
    /* Put it on stack.  */
    IR_set_false_expr (expr, second_expr_processing (IR_false_expr (expr), fun_class_assign_p, NULL,
                                                     &temp_vars_num, FALSE, NULL, NULL, FALSE));
    if (result != NULL) *result = not_defined_result;
    ...
    type_test (IR_cond_expr (expr), EVT_NUMBER_STRING_MASK, ERR_invalid_cond_type);
    IR_set_value_type (expr, value_type (IR_true_expr (expr)) | value_type (IR_false_expr (expr)));
  } break;
  case IR_NM_vec:
  case IR_NM_tab: {
    int n = 0;
    ...
    SET_SOURCE_POSITION (expr);
    bc = new_bc_code_with_src (node_mode == IR_NM_vec ? BC_NM_vec : BC_NM_tab, expr);
    BC_set_op2 (bc, *curr_temp_vars_num + curr_temp_vars_base);
    BC_set_op1 (bc, setup_result_var_number (result, curr_temp_vars_num));
    for (elist = IR_elist (expr); elist != NULL; elist = IR_next_elist (elist)) {
      n++;
      SET_SOURCE_POSITION (elist);
      IR_set_repetition_key (elist,
                             second_expr_processing (IR_repetition_key (elist), FALSE, NULL,
                                                     &temp_vars_num, FALSE, NULL, NULL, FALSE));
      if (node_mode == IR_NM_vec)
        type_test (IR_repetition_key (elist), EVT_NUMBER_STRING_MASK, ERR_invalid_repetition_type);
      IR_set_expr (elist, second_expr_processing (IR_expr (elist), TRUE, NULL, &temp_vars_num,
                                                  FALSE, NULL, NULL, FALSE));
    }
    BC_set_op3 (bc, n);
  ...
  case IR_NM_index: {
    ...
    IR_set_designator (expr, second_expr_processing (IR_designator (expr), FALSE, &op_result,
                                                     &temp_vars_num, FALSE, NULL, NULL, FALSE));
    BC_set_op2 (bc, op_result);
    if (lvalue_p) *curr_temp_vars_num = temp_vars_num;
    op_result = not_defined_result;
    IR_set_component (expr, second_expr_processing (IR_component (expr), FALSE, &op_result,
                                                    &temp_vars_num, FALSE, NULL, NULL, FALSE));
    ...
    if (lvalue_p) {
      ...
      *curr_temp_vars_num = temp_vars_num;
      ...
      BC_set_op1 (bc, setup_result_var_number (result, curr_temp_vars_num));
    add_to_bcode (bc);
    type_test (IR_designator (expr), EVT_VEC_TAB_SLICE_MASK, ERR_invalid_vec_tab_type);
    break;
  }
  case IR_NM_slice: {
    ...
    if (des != NULL) {
      ...
      des = second_expr_processing (des, FALSE, NULL, &temp_vars_num,
                                    IR_IS_OF_TYPE (des, IR_NM_slice), NULL, NULL, FALSE);
    ...
    IR_set_component (expr, second_expr_processing (IR_component (expr), FALSE, NULL,
                                                    &temp_vars_num, FALSE, NULL, NULL, FALSE));
    IR_set_bound (expr, second_expr_processing (IR_bound (expr), FALSE, NULL, &temp_vars_num, FALSE,
                                                NULL, NULL, FALSE));
    IR_set_step (expr, second_expr_processing (IR_step (expr), FALSE, NULL, &temp_vars_num, FALSE,
                                               NULL, NULL, FALSE));
    if (lvalue_p) {
      BC_set_op1 (bc, -1);
      *curr_temp_vars_num = temp_vars_num;
      ...
      BC_set_op1 (bc, setup_result_var_number (result, curr_temp_vars_num));
    add_to_bcode (bc);
    type_test (IR_designator (expr), EVT_VEC_SLICE_MASK, ERR_invalid_vec_slice_type);
    type_test (IR_component (expr), EVT_NUMBER_STRING_MASK, ERR_invalid_slice_start_type);
    type_test (IR_bound (expr), EVT_NUMBER_STRING_MASK, ERR_invalid_slice_bound_type);
    type_test (IR_step (expr), EVT_NUMBER_STRING_MASK, ERR_invalid_slice_step_type);
  ...
  case IR_NM_class_fun_fiber_call: {
    ...
    IR_set_fun_expr (expr, second_expr_processing (IR_fun_expr (expr), FALSE, NULL, &temp_vars_num,
                                                   FALSE, NULL, NULL, FALSE));
    if (BC_NODE_MODE (curr_pc) == BC_NM_fld && saved_prev_pc != curr_pc
        && BC_op2 (curr_pc) < call_start) {
      fld = curr_pc;
      general_p = FALSE;
    } else if ((BC_NODE_MODE (curr_pc) == BC_NM_ofun || BC_NODE_MODE (curr_pc) == BC_NM_oclass)
               && saved_prev_pc != curr_pc && BC_op2 (curr_pc) < call_start) {
      ofc = curr_pc;
      general_p = FALSE;
      ...
      env_p = strcmp (IR_pos (fun_decl).file_name, ENVIRONMENT_PSEUDO_FILE_NAME) == 0;
      top_p = IR_block_scope (IR_scope (fun_decl)) == NULL;
      general_p = ((!env_p
                    && (IR_args_flag (fun_decl)
                        || (IR_IS_OF_TYPE (fun_decl, IR_NM_fun) && IR_fiber_flag (fun_decl))
    ...
    if (!general_p) {
      ...
      d_assert (call_start - curr_temp_vars_base + 1 == temp_vars_num);
      temp_vars_num--;
      curr_pc = prev_pc;
      prev_pc = saved_prev_pc;
      curr_info = BC_prev_info (curr_info);
    }
    pars_num = process_actuals (fun_decl, call_start, IR_actuals (expr), &temp_vars_num);
    if (result != NULL) *result = not_defined_result;
    setup_result_var_number (result, curr_temp_vars_num);
    if (pars_num == 0) get_temp_stack_slot (&temp_vars_num); /* reverve for result */
    if (general_p)
      ...
      env_p = strcmp (IR_pos (fun_decl).file_name, ENVIRONMENT_PSEUDO_FILE_NAME) == 0;
      if (env_p || IR_args_flag (fun_decl)
          || (IR_IS_OF_TYPE (fun_decl, IR_NM_fun) && IR_fiber_flag (fun_decl))
          || (IR_class_fun_fiber_call_parameters_number (expr)
              != IR_parameters_number (fun_decl))) {
      ...
      bc = new_bc_code_with_src (env_p
                                   ? BC_NM_ibcall
                                   : top_p ? BC_NM_ticall
                                           : IR_scope (fun_decl) == curr_real_scope ? BC_NM_cicall
                                                                                    : BC_NM_icall,
                                 expr);
      fblock = get_fblock (fun_decl);
      BC_set_cfblock (bc, fblock);
      if (fun_decl != NULL && !IR_forward_decl_flag (fun_decl)
          && (block = IR_next_stmt (fun_decl)) != NULL && IR_IS_OF_TYPE (block, IR_NM_block)
          && IR_fun_class (block) == fun_decl && IR_hint (block) == INLINE_HINT)
    ...
    if (!IT_IS_OF_TYPE (IR_fun_expr (expr), EVT_FUN)
      ...
      cont_err (source_position, ERR_invalid_class_fun_fiber_designator);
    IR_set_value_type (expr, EVT_UNKNOWN);
    break;
  }
  case IR_NM_dots:
  case IR_NM_wildcard: d_assert (number_of_errors != 0); break;
  default: d_unreachable ();
  }
  if (false_pc != NULL) {
    int op_result
      = setup_result_var_number (node_mode != IR_NM_ident ? result : NULL, curr_temp_vars_num);
    ...
    if (IR_value_type (expr) != EVT_INT) {
      ...
      add_to_bcode (bc);
      type_test (expr, EVT_NUMBER_VEC_SLICE_MASK, ERR_invalid_logical_operation_operand_type);
      IR_set_value_type (expr, EVT_INT);
    }
    if (next_false_p) {
...
static BC_node_mode_t make_designator_lvalue (BC_node_t designator, const char *error_message,
                                              int val_p) {
  switch (BC_NODE_MODE (designator)) {
  case BC_NM_fld: return (val_p ? BC_NM_lfldv : BC_NM_lfld);
  case BC_NM_ovfld: return (val_p ? BC_NM_lovfldv : BC_NM_lovfld);
  case BC_NM_ind: return BC_NM_ind;
  case BC_NM_sl: return (val_p ? BC_NM_lslv : BC_NM_sl);
  case BC_NM_var: return (val_p ? BC_NM_lvarv : BC_NM_lvar);
  case BC_NM_evar: return (val_p ? BC_NM_levarv : BC_NM_lvar);
  default:
    cont_err (BC_pos (BC_source (BC_info (designator))), error_message);
...
/* Return the corresponding binary operation mode to an assignment
   node ASSIGN.  */
static BC_node_mode_t make_op_mode (IR_node_t assign) {
...
static BC_node_mode_t slice_assign_mode (BC_node_t assign) {
...
static IR_node_t find_covered_fun_class (IR_node_t scope) {
  while (scope != NULL && IR_fun_class (scope) == NULL) scope = IR_block_scope (scope);
  if (scope != NULL)
...
static void get_lvalue_location (BC_node_t designator, IR_node_t ir_des, int *temp_vars_num,
...
/* Return pointer to the block of first function decl surrounding
   ...
   function decl.  */
static IR_node_t surrounding_fun_block (IR_node_t scope) {
  IR_node_t fun_class;

  for (; scope != NULL; scope = IR_block_scope (scope)) {
    fun_class = IR_fun_class (scope);
    if (fun_class != NULL && IR_NODE_MODE (fun_class) == IR_NM_fun) return scope;
  }
  return NULL;
}

/* Add BC_DECL to cycle decl list of BC_BLOCK.  */
static void inline add_decl_to_block (BC_node_t bc_decl, BC_node_t bc_block) {
  BC_node_t last;
...
static void copy_and_link_bcode (BC_node_t bc) {
  BC_node_t new_bc;
...
static void copy_bc_decls (BC_node_t bc_block, BC_node_t origin_bc_block, int var_base) {
  BC_node_t bc, bc_decl, last_bc_decl, next_bc_decl, origin_bc_decl;

  for (last_bc_decl = BC_decls (bc_block);
       last_bc_decl != NULL && (next_bc_decl = BC_next_decl (last_bc_decl)) != NULL;
       last_bc_decl = next_bc_decl)
    ;
  /* Make decls copy:  */
  for (origin_bc_decl = BC_decls (origin_bc_block); origin_bc_decl != NULL;
...
static void copy_bc_block (BC_node_t origin_bc_block, BC_node_t bc_block) {
  ...
  if (!BC_forward_p (origin_bc_block)) {
    ...
    /* Make bcode copy:  */
    for (info = BC_next_info (BC_info (origin_bc_block));; info = BC_next_info (info)) {
      bc = BC_bc (info);
...
static void modify_copied_pc (int (*except) (BC_node_t)) {
  ...
  for (size_t i = 0; i < VARR_LENGTH (BC_node_t, bc_copies); i++) {
    ...
    if (BC_IS_OF_TYPE (bc, BC_NM_block)) {
      if ((fv = BC_scope (bc)) != NULL && (fv = BC_subst (BC_info (fv))) != NULL)
        BC_set_scope (bc, fv);
      if ((fv = BC_excepts (bc)) != NULL && (fv = BC_subst (BC_info (fv))) != NULL)
        BC_set_excepts (bc, fv);
      if ((fv = BC_friends (bc)) != NULL && (fv = BC_subst (BC_info (fv))) != NULL)
        BC_set_friends (bc, fv);
      if ((fv = BC_uses (bc)) != NULL && (fv = BC_subst (BC_info (fv))) != NULL)
        BC_set_uses (bc, fv);
    }
...
/* Generate code for matching value in OP and CEXPR modifying
   ...
   IR_NM_rmatch_stmt.  */
static void generate_case (IR_node_t case_block, IR_node_t cexpr, int op, BC_node_t fail_pc,
                           int *curr_temp_vars_num) {
  int_t var_num;
  ...
  IR_node_mode_t match_mode = (match_stmt == NULL ? IR_NM_pmatch_stmt : IR_NODE_MODE (match_stmt));
  ...
  if (match_mode == IR_NM_pmatch_stmt && IR_IS_OF_TYPE (cexpr, IR_NM_ident)) {
    ...
    if (call_p) {
      ...
      IR_set_fun_expr (cexpr, second_expr_processing (IR_fun_expr (cexpr), FALSE, &fun_op,
                                                      &temp_start, FALSE, NULL, NULL, FALSE));
      temp_vars_num++;
    ...
    for (n = 0, el = (call_p ? IR_actuals (cexpr) : IR_elist (cexpr)); el != NULL;
         el = next_el, n++) {
      ...
      if (!call_p
        ...
        IR_set_repetition_key (el, second_expr_processing (rep, FALSE, &rep_op, &temp_start, FALSE,
                                                           NULL, NULL, FALSE));
        if (vec_p)
          type_test (IR_repetition_key (el), EVT_NUMBER_STRING_MASK, ERR_invalid_repetition_type);
      }
      if (IR_IS_OF_TYPE (expr, IR_NM_ident) || IR_IS_OF_TYPE (expr, IR_NM_wildcard)) {
        ...
        IR_set_expr (el, second_expr_processing (IR_expr (el), FALSE, &el_op, &temp_start, FALSE,
                                                 NULL, NULL, FALSE));
        if (vec_p) {
          bc = new_bc_code_with_src (BC_NM_chvel, expr);
          BC_set_ch_op2 (bc, ind);
    ...
    if (el == NULL) {
      if (call_p) {
        ...
        bc = new_bc_code_with_src (vec_p ? BC_NM_chvend : BC_NM_chtend, cexpr);
        BC_set_op1 (bc, op);
      }
    ...
    if (match_mode == IR_NM_pmatch_stmt) {
      second_expr_processing (cexpr, FALSE, &pat_op, &temp_start, FALSE, NULL, NULL, FALSE);
      el_op = not_defined_result;
...
static void generate_break_continue (int break_p, position_t pos) {
  int n;
  IR_node_t scope, first_inline_block, stop_block;
  BC_node_t bc;

  stop_block = curr_for_stmt_or_case_block;
  for (n = 0, first_inline_block = NULL, scope = curr_scope; scope != NULL && scope != stop_block;
       scope = IR_block_scope (scope))
    if (IR_inline_flag (scope)) {
      if (first_inline_block == NULL) first_inline_block = scope;
    } else {
      n++;
  ...
  if (first_inline_block != NULL) {
    ...
    BC_set_op1 (bc, IR_vars_base (first_inline_block) + IR_vars_number (first_inline_block)
                      - IR_vars_base (stop_block) - IR_vars_number (stop_block));
    BC_set_op2 (bc, 0);
    add_to_bcode (bc);
  }
  if (break_p)
...
static void second_block_passing (IR_node_t first_level_stmt, int block_p) {
  ...
  for (stmt = first_level_stmt; stmt != NULL; stmt = next_stmt) {
    ...
    case IR_NM_expr_stmt:
      ...
      IR_set_stmt_expr (stmt, second_expr_processing (IR_stmt_expr (stmt), TRUE, &result,
                                                      &temp_vars_num, FALSE, NULL, NULL, FALSE));
      bc = NULL;
      if (repl_flag && IR_block_scope (curr_scope) == NULL)
        bc = (new_bc_code_with_src (IR_IS_OF_TYPE (IR_stmt_expr (stmt), IR_NM_class_fun_fiber_call)
                                      ? BC_NM_rpr_def
                                      : BC_NM_rpr,
                                    stmt));
      /* Last expr-stmt of fun is returned.  */
      else if (block_p && next_stmt == NULL && (temp = IR_fun_class (curr_scope)) != NULL
               && IR_IS_OF_TYPE (temp, IR_NM_fun) && !IR_fiber_flag (temp)) {
      ...
      break;
    case IR_NM_assign:
    case IR_NM_var_assign: bc_node_mode = BC_NM_sts; goto common_assign;
    case IR_NM_mult_assign: bc_node_mode = BC_NM_mult_st; goto common_assign;
    case IR_NM_div_assign: bc_node_mode = BC_NM_div_st; goto common_assign;
    case IR_NM_mod_assign: bc_node_mode = BC_NM_mod_st; goto common_assign;
    case IR_NM_plus_assign: bc_node_mode = BC_NM_add_st; goto common_assign;
    case IR_NM_minus_assign: bc_node_mode = BC_NM_sub_st; goto common_assign;
    case IR_NM_concat_assign: bc_node_mode = BC_NM_concat_st; goto common_assign;
    case IR_NM_lshift_assign: bc_node_mode = BC_NM_lsh_st; goto common_assign;
    case IR_NM_rshift_assign: bc_node_mode = BC_NM_rsh_st; goto common_assign;
    case IR_NM_ashift_assign: bc_node_mode = BC_NM_ash_st; goto common_assign;
    case IR_NM_and_assign: bc_node_mode = BC_NM_and_st; goto common_assign;
    case IR_NM_xor_assign: bc_node_mode = BC_NM_xor_st; goto common_assign;
    case IR_NM_or_assign:
    ...
    common_assign:
      ...
      temp = second_expr_processing (IR_assignment_var (stmt), FALSE, &var_result, &temp_vars_num,
                                     TRUE, NULL, NULL, FALSE);
      if (temp != NULL) IR_set_assignment_var (stmt, temp);
      ...
      if (temp != NULL && var_bc != NULL) {
        ...
        BC_SET_MODE (var_bc,
                     make_designator_lvalue (var_bc, ERR_non_variable_in_assignment, val_p));
        if (IR_IS_OF_TYPE (temp, IR_NM_index_designator)) {
          BC_set_op1 (bc, BC_op2 (var_bc));
          BC_set_op2 (bc, BC_op3 (var_bc));
          if (IR_IS_OF_TYPE (temp, IR_NM_slice)) BC_SET_MODE (bc, slice_assign_mode (bc));
        ...
        if (val_p && (var_mode == BC_NM_var || var_mode == BC_NM_evar))
          ...
          if (val_p) BC_set_op4 (bc, get_temp_stack_slot (&temp_vars_num));
      ...
      if (stmt_mode != IR_NM_var_assign && stmt_mode != IR_NM_var_pattern_assign && temp != NULL
        ...
        cont_err (IR_pos (temp), ERR_const_assignment,
                  IR_ident_string (IR_unique_ident (IR_ident (IR_decl (temp)))));
      if (temp != NULL && var_bc == NULL
      ...
      if (var_bc != NULL) {
        ...
        IR_set_assignment_expr (stmt,
                                second_expr_processing (IR_assignment_expr (stmt), TRUE, &result,
                                                        &temp_vars_num, FALSE, NULL, NULL, FALSE));
      ...
      } else /* assignment to the local var.  */
      {
        ...
        if (stmt_mode == IR_NM_assign || stmt_mode == IR_NM_var_assign
            || stmt_mode == IR_NM_var_pattern_assign) {
          IR_set_assignment_expr (stmt, second_expr_processing (val, TRUE, &result, &temp_vars_num,
                                                                FALSE, NULL, NULL, FALSE));
          if (temp != NULL && result != var_result) {
            IR_node_t ident = IR_assignment_expr (stmt);

            if (!IR_IS_OF_TYPE (ident, IR_NM_ident)) ident = temp;
            add_move (stmt, get_bcode_decl (IR_decl (ident)), var_result, result);
          }
        } else {
          IR_node_mode_t op_mode = IR_NODE_MODE (stmt);

          if ((op_mode == IR_NM_plus_assign || op_mode == IR_NM_mult_assign)
              && IR_IS_OF_TYPE (val, IR_NM_int) && temp != NULL
              && !IR_IS_OF_TYPE (temp, IR_NM_slice)) {
            bc = new_bc_code (op_mode == IR_NM_plus_assign ? BC_NM_addi : BC_NM_multi, src);
            BC_set_op3 (bc, IR_int_value (IR_unique_int (val)));
            BC_set_op2 (bc, var_result);
          } else {
            IR_set_assignment_expr (stmt,
                                    second_expr_processing (val, TRUE, &result, &temp_vars_num,
                                                            FALSE, NULL, NULL, FALSE));
    ...
    common_par_assign : {
      ...
      var_bc = before_pc != curr_pc ? curr_pc : NULL; /* local var */
      d_assert (temp == NULL
                || ((var_bc == NULL && var_result >= 0 && temp_vars_num == 0) || var_bc != NULL));
      if (var_bc != NULL) {
      ...
      if (temp != NULL) {
        if (var_bc == NULL)
          ...
          result = not_defined_result;
          bc_node_mode = make_designator_lvalue (var_bc, ERR_non_variable_in_assignment, FALSE);
          if (IR_IS_OF_TYPE (temp, IR_NM_ident))
            lvalue_bc = new_bc_code (bc_node_mode, get_bcode_decl (IR_decl (temp)));
          else
      ...
      if (lvalue_bc != NULL) {
        ...
        add_move (stmt, get_bcode_decl (IR_decl (temp)), var_result, result);
    ...
    case IR_NM_par_pattern_assign:
      ...
      if (temp != NULL && IR_IS_OF_TYPE (temp, IR_NM_ident)) {
        if (stmt_mode == IR_NM_par_pattern_assign) goto common_par_assign;
    ...
    case IR_NM_if_stmt: {
      ...
      result = not_defined_result;
      IR_set_if_expr (stmt,
                      second_expr_processing (IR_if_expr (stmt), FALSE, &result, &temp_vars_num,
                                              FALSE, false_pc, true_pc, FALSE));
      ...
      if (!BC_IS_OF_TYPE (bc, BC_NM_out) && (!BC_IS_OF_TYPE (bc, BC_NM_stpop) || BC_op2 (bc)))
    ...
    case IR_NM_for_stmt: {
      ...
      result = not_defined_result;
      IR_set_for_guard_expr (stmt, second_expr_processing (IR_for_guard_expr (stmt), FALSE, &result,
                                                           &temp_vars_num, FALSE,
                                                           for_or_match_finish, before_body, TRUE));
      type_test (IR_for_guard_expr (stmt), EVT_NUMBER_STRING_MASK, ERR_invalid_for_guard_expr_type);
    ...
    case IR_NM_foreach_stmt: {
      ...
      IR_set_foreach_tab (stmt, second_expr_processing (tab, FALSE, &result, &temp_vars_num, FALSE,
                                                        NULL, NULL, FALSE));
      if (tab != NULL) source_position = IR_pos (tab);
      ...
      var_result = not_defined_result;
      before_pc = curr_pc;
      temp = second_expr_processing (IR_foreach_index_designator (stmt), FALSE, &var_result,
                                     &temp_vars_num, TRUE, NULL, NULL, FALSE);
      if (temp != NULL) IR_set_foreach_index_designator (stmt, temp);
      var_bc = before_pc != curr_pc ? curr_pc : NULL; /* local var */
      bc = new_bc_code (var_bc == NULL ? BC_NM_foreach : BC_NM_foreach2, src);
      BC_set_op1 (bc, result);
      BC_set_op2 (bc, BC_op1 (ldi_bc));
      if (temp != NULL) source_position = IR_pos (temp);
      before_pc = curr_pc;
      if (var_bc == NULL)
        BC_set_op3 (bc, var_result);
      else {
        BC_SET_MODE (var_bc,
                     make_designator_lvalue (var_bc, ERR_non_variable_in_foreach_index, FALSE));
        if (temp != NULL) {
          ...
          get_lvalue_location (var_bc, temp, &temp_vars_num, &container_num, &index_num);
      ...
      before_pc = curr_pc;
      add_to_bcode (for_or_match_finish);
      BC_set_body_pc (bc, BC_next (bc) != for_or_match_finish ? BC_next (bc) : continue_target);
    ...
    case IR_NM_continue_stmt:
      ...
      else if (stmt_mode == IR_NM_break_stmt && for_or_match_finish == NULL)
        cont_err (source_position, ERR_break_is_not_in_loop_or_case);
      else
        generate_break_continue (stmt_mode == IR_NM_break_stmt, IR_pos (stmt));
      break;
    ...
    case IR_NM_return_with_result: {
      ...
      if (fun_class == NULL)
        cont_err (source_position, ERR_return_outside_fun_class);
        ...
        if (IR_IS_OF_TYPE (fun_class, IR_NM_class))
          cont_err (source_position, ERR_return_with_result_in_class);
        else if (IR_IS_OF_TYPE (fun_class, IR_NM_fun) && IR_fiber_flag (fun_class))
          cont_err (source_position, ERR_return_with_result_in_fiber);
        result = not_defined_result;
        IR_set_returned_expr (stmt,
                              second_expr_processing (IR_returned_expr (stmt), TRUE, &result,
                                                      &temp_vars_num, FALSE, NULL, NULL, FALSE));
        add_flatten_node_if_necessary (IR_returned_expr (stmt), result);
        bc = new_bc_code_with_src (BC_NM_ret, stmt);
        BC_set_op1 (bc, result);
        BC_set_ret_decl (bc, NULL);
        if (IR_returned_expr (stmt) != NULL && IR_IS_OF_TYPE (IR_returned_expr (stmt), IR_NM_ident))
          BC_set_ret_decl (bc, get_bcode_decl (IR_decl (IR_returned_expr (stmt))));
        /* Check tail call.  */
        if (curr_pc != NULL) {
          if (BC_NODE_MODE (curr_pc) == BC_NM_icall && BC_op1 (curr_pc) == result)
            bc_mode = BC_NM_itcall;
          else if (BC_NODE_MODE (curr_pc) == BC_NM_ticall && BC_op1 (curr_pc) == result)
            bc_mode = BC_NM_titcall;
          else if (BC_NODE_MODE (curr_pc) == BC_NM_cicall && BC_op1 (curr_pc) == result)
            bc_mode = BC_NM_citcall;
          else
            bc_mode = BC_NODE_MODE (curr_pc);
          if (BC_NODE_MODE (curr_pc) != bc_mode && IR_block_scope (curr_real_scope) != NULL) {
            d_assert (BC_IS_OF_TYPE (BC_cfblock (curr_pc), BC_NM_fblock));
    ...
    case IR_NM_wait_stmt: {
      ...
      result = not_defined_result;
      IR_set_wait_guard_expr (stmt,
                              second_expr_processing (IR_wait_guard_expr (stmt), FALSE, &result,
                                                      &temp_vars_num, FALSE, NULL, NULL, FALSE));
    ...
    case IR_NM_throw:
      ...
      IR_set_throw_expr (stmt, second_expr_processing (IR_throw_expr (stmt), FALSE, NULL,
                                                       &temp_vars_num, FALSE, NULL, NULL, FALSE));
      type_test (IR_throw_expr (stmt), EVT_UNKNOWN, ERR_invalid_throw_expr_type);
      add_to_bcode (bc);
      break;
    case IR_NM_pmatch_stmt:
    case IR_NM_rmatch_stmt: {
      ...
      match_expr = second_expr_processing (IR_match_expr (stmt), FALSE, &result, &temp_vars_num,
                                           FALSE, NULL, NULL, FALSE);
      IR_set_match_expr (stmt, match_expr);
      if (result != place) add_move (stmt, get_bcode_decl (decl), place, result);
      for_or_match_finish = new_bc_code_with_src (BC_NM_nop, stmt);
      continue_target = new_bc_code_with_src (BC_NM_nop, stmt);
    ...
    case IR_NM_block: {
      ...
      if (fun_class == NULL) {
        if (inline_block_flag) {
          ...
          bc = new_bc_code (BC_NM_stinc, new_bc_node (BC_NM_source, IR_pos (stmt)));
          ...
          if ((bc = IR_bc_block (stmt)) == NULL)
            bc = new_bc_code (BC_NM_block, new_bc_node (BC_NM_source, IR_pos (stmt)));
        ...
        BC_set_fun_p (bc, IR_IS_OF_TYPE (fun_class, IR_NM_fun) && !IR_fiber_flag (fun_class));
        BC_set_class_p (bc, IR_IS_OF_TYPE (fun_class, IR_NM_class));
        BC_set_fiber_p (bc, IR_IS_OF_TYPE (fun_class, IR_NM_fun) && IR_fiber_flag (fun_class));
        BC_set_args_p (bc, IR_args_flag (fun_class));
        BC_set_pars_num (bc, IR_parameters_number (fun_class));
        BC_set_min_pars_num (bc, IR_min_actual_parameters_number (fun_class));
        BC_set_ext_life_p (bc, (IR_IS_OF_TYPE (fun_class, IR_NM_class) || IR_fiber_flag (fun_class)
                                || IR_extended_life_context_flag (stmt)));
        BC_set_fmode (bc, IR_hint (stmt) == JIT_HINT ? BC_gen : BC_no_gen);
      ...
      if (!inline_block_flag) {
        ...
        for (friend = IR_friends (stmt); friend != NULL; friend = IR_next_friend (friend)) {
        ...
        BC_set_uses (bc, NULL);
        for (use = IR_block_uses (stmt); use != NULL; use = IR_next_block_use (use)) {
      ...
      if (match_stmt != NULL) {
        ...
        if (inline_block_flag)
          result = IR_var_number_in_block (IR_match_expr_var (match_stmt));
        else {
          BC_node_t var_bc = create_bc_decl (IR_match_expr_var (match_stmt), match_stmt);
          result = not_defined_result;
          BC_set_op1 (var_bc, setup_result_var_number (&result, &temp_vars_num));
          add_to_bcode (var_bc);
        }
        generate_case (stmt, IR_case_pattern (stmt), result, continue_target, &temp_vars_num);
        if ((cond = IR_case_cond (stmt)) != NULL) {
          BC_node_t true_pc = new_bc_code_with_src (BC_NM_nop, cond);

          result = not_defined_result;
          IR_set_case_cond (stmt, second_expr_processing (cond, FALSE, &result, &temp_vars_num,
                                                          FALSE, continue_target, true_pc, FALSE));
          add_to_bcode (true_pc);
        }
      }
      ...
      if (inline_block_flag) {
        ...
        block_finish = new_bc_code (BC_NM_stpop, new_bc_node (BC_NM_source, source_position));
        BC_set_op1 (block_finish, IR_vars_number (stmt));
        BC_set_op2 (block_finish, 1);
      } else {
        /* Function or the implicit top block.  */
        block_finish = new_bc_code (fun_class == NULL ? BC_NM_bend : BC_NM_fbend,
                                    new_bc_node (BC_NM_source, source_position));
        BC_set_block (block_finish, curr_bc_scope);
      }
      if (BC_IS_OF_TYPE (block_finish, BC_NM_fbend) && !BC_IS_OF_TYPE (curr_pc, BC_NM_ret)
      ...
      if (fun_class != NULL) {
        ...
        for (curr_except = IR_exceptions (stmt); curr_except != NULL;
          ...
          d_assert (BC_IS_OF_TYPE (previous_node_catch_list_pc, BC_NM_block)
                    || BC_IS_OF_TYPE (previous_node_catch_list_pc, BC_NM_except));
          result = not_defined_result;
          temp_vars_num = 0;
          except_bc = new_bc_code_with_src (BC_NM_except, curr_except);
          /* The exception instance in the first local
             temporary variable.  */
          BC_set_op1 (except_bc, setup_result_var_number (NULL, &temp_vars_num));
          IR_set_exception_class_expr (curr_except,
                                       second_expr_processing (IR_exception_class_expr (
                                                                 curr_except),
                                                               FALSE, &result, &temp_vars_num,
                                                               FALSE, NULL, NULL, FALSE));
          BC_set_op2 (except_bc, result);
          type_test (IR_exception_class_expr (curr_except), EVT_CLASS, ERR_invalid_catch_expr_type);
          if (BC_IS_OF_TYPE (previous_node_catch_list_pc, BC_NM_block))
            BC_set_excepts (previous_node_catch_list_pc, BC_next (catches_finish));
          else
            BC_set_next_except (previous_node_catch_list_pc, BC_next (catches_finish));
          ...
          if (IR_catch_block (curr_except) != NULL) {
            ...
            BC_set_next (except_bc, BC_next (last_except_with_block));
      ...
      if (!inline_block_flag) {
        IR_set_temporary_vars_number (stmt, IR_temporary_vars_number (stmt) + ADD_TEMP_VARS_NUMBER);
    ...
    case IR_NM_external_fun: {
      ...
      d_assert (block != NULL && IR_NODE_MODE (block) == IR_NM_block);
      IR_set_it_is_declared_in_block (IR_unique_ident (IR_ident (curr_decl)), TRUE);
      /* Add decl to the block */
      bc_block = IR_bc_block (block);
      bc_decl = get_bcode_decl (curr_decl);
      add_decl_to_block (bc_decl, bc_block);
      if (curr_decl_mode == IR_NM_class && (block = surrounding_fun_block (block)) != NULL)
      ...
      if ((decl = IR_origin_decl (curr_decl)) != NULL) {
        ...
        if (IR_IS_OF_TYPE (decl, IR_NM_fun_or_class)) {
          BC_set_subst (BC_info (get_fblock (decl)), get_fblock (curr_decl));
          copy_fun_bc_block (stmt, decl);
        }
      } else if ((stmt_mode == IR_NM_class || stmt_mode == IR_NM_fun)
                 && IR_fdecl_bc_block (curr_decl) == NULL && IR_forward_decl_flag (curr_decl)) {
    ...
    case IR_NM_decl_redir: {
      ...
      if (IR_IS_OF_TYPE (origin, IR_NM_fun_or_class))
        BC_set_subst (BC_info (get_fblock (origin)), get_fblock (curr_decl));
    } break;
    case IR_NM_require:
      /* Do nothing */
      break;
...
static inline int nop_p (BC_node_t bc) {
...
static BC_node_t skip_nops (BC_node_t bc) {
...
static BC_node_t branch_combine (BC_node_t bc) {
  ...
  if ((BC_NODE_MODE (first_bc) == BC_NM_addi || BC_NODE_MODE (first_bc) == BC_NM_iaddi)
    ...
    if (second_bc == NULL
        ...
        || (BC_NODE_MODE (next_bc) != BC_NM_bt && BC_NODE_MODE (next_bc) != BC_NM_ibt)
        /* Immediate is only increment for now: */
        || BC_NODE_MODE (second_bc) == BC_NM_eqi || BC_NODE_MODE (second_bc) == BC_NM_nei
        || BC_NODE_MODE (second_bc) == BC_NM_gei || BC_NODE_MODE (second_bc) == BC_NM_lti
        || BC_NODE_MODE (second_bc) == BC_NM_lei || BC_NODE_MODE (second_bc) == BC_NM_gti
        || BC_NODE_MODE (second_bc) == BC_NM_ieqi || BC_NODE_MODE (second_bc) == BC_NM_inei
        || BC_NODE_MODE (second_bc) == BC_NM_igei || BC_NODE_MODE (second_bc) == BC_NM_ilti
        || BC_NODE_MODE (second_bc) == BC_NM_ilei || BC_NODE_MODE (second_bc) == BC_NM_igti) {
  ...
  if ((next_bc = BC_next (second_bc)) != NULL
      && ((bt_p = (BC_NODE_MODE (next_bc) == BC_NM_bt || BC_NODE_MODE (next_bc) == BC_NM_ibt))
          || BC_NODE_MODE (next_bc) == BC_NM_bf || BC_NODE_MODE (next_bc) == BC_NM_ibf))
    ...
    case BC_NM_gt:
      ...
      break;
    case BC_NM_eqi: node_mode = bt_p ? BC_NM_bteqi : BC_NM_btnei; break;
    case BC_NM_nei: node_mode = bt_p ? BC_NM_btnei : BC_NM_bteqi; break;
    case BC_NM_gei: node_mode = bt_p ? BC_NM_btgei : BC_NM_btlti; break;
    case BC_NM_lti: node_mode = bt_p ? BC_NM_btlti : BC_NM_btgei; break;
    case BC_NM_lei: node_mode = bt_p ? BC_NM_btlei : BC_NM_btgti; break;
    case BC_NM_gti: node_mode = bt_p ? BC_NM_btgti : BC_NM_btlei; break;
    case BC_NM_ieq:
    ...
    case BC_NM_igt:
      ...
      break;
    case BC_NM_ieqi: node_mode = bt_p ? BC_NM_ibteqi : BC_NM_ibtnei; break;
    case BC_NM_inei: node_mode = bt_p ? BC_NM_ibtnei : BC_NM_ibteqi; break;
    case BC_NM_igei: node_mode = bt_p ? BC_NM_ibtgei : BC_NM_ibtlti; break;
    case BC_NM_ilti: node_mode = bt_p ? BC_NM_ibtlti : BC_NM_ibtgei; break;
    case BC_NM_ilei: node_mode = bt_p ? BC_NM_ibtlei : BC_NM_ibtgti; break;
    case BC_NM_igti: node_mode = bt_p ? BC_NM_ibtgti : BC_NM_ibtlei; break;
    default: break;
    }
  if (node_mode != BC_NM__error && (op1 == not_defined_result || BC_op2 (second_bc) == op1)
      && BC_op1 (second_bc) >= curr_temp_vars_start) {
    bc = new_bc_code (node_mode, BC_source (BC_info (next_bc)));
    insert_info (BC_info (bc), info);
    if (op1 != not_defined_result) BC_set_binc_inc (bc, BC_op3 (first_bc));
...
static BC_node_t madd_combine (BC_node_t bc) {
  BC_node_t info, next_bc;
  int op1, op2, op3, op4;

  op1 = not_defined_result;
  if (BC_NODE_MODE (bc) == BC_NM_mult && BC_op1 (bc) >= curr_temp_vars_start
      && (next_bc = BC_next (bc)) != NULL && BC_NODE_MODE (next_bc) == BC_NM_add) {
    op1 = BC_op1 (next_bc);
    op2 = BC_op2 (bc);
    op3 = BC_op3 (bc);
    if (BC_op2 (next_bc) == BC_op1 (bc))
      ...
      op1 = not_defined_result;
  } else if (BC_NODE_MODE (bc) == BC_NM_add && (next_bc = BC_next (bc)) != NULL
             && BC_NODE_MODE (next_bc) == BC_NM_mult && BC_op1 (next_bc) >= curr_temp_vars_start) {
    op1 = BC_op1 (next_bc);
    op2 = BC_op2 (bc);
    op3 = BC_op3 (bc);
    if (BC_op2 (next_bc) == BC_op1 (bc))
...
static BC_node_t ind_combine (BC_node_t bc) {
  BC_node_t info, next_bc;
  int op1, op2, op3, op4;

  if (BC_NODE_MODE (bc) == BC_NM_ind && BC_op1 (bc) >= curr_temp_vars_start
      && (next_bc = BC_next (bc)) != NULL && BC_NODE_MODE (next_bc) == BC_NM_ind
...
static BC_node_t go_through (BC_node_t start_bc) {
  BC_node_t bc, subst, info;

  if (start_bc == NULL) return NULL;
  bc = skip_nops (start_bc);
  if (bc == NULL) return NULL;
  info = BC_info (bc);
  if (BC_subst (info) != NULL) return BC_subst (info);
  subst = branch_combine (bc);
  if (subst == bc) subst = madd_combine (bc);
  if (subst == bc) subst = ind_combine (bc);
  BC_set_subst (info, subst);
  return subst;
}

/* Modify call BC to tail calls if it is possible.  */
static void process_tail_imcall (BC_node_t bc) {
  BC_node_t next_pc;

  if (BC_IS_OF_TYPE (bc, BC_NM_imcall) && (next_pc = BC_next (bc)) != NULL
      && (BC_NODE_MODE (next_pc) == BC_NM_leave || BC_NODE_MODE (next_pc) == BC_NM_fbend)) {
    BC_node_mode_t bc_mode;
...
static void modify_copied_ops (int base) {
  ...
  for (size_t i = 0; i < VARR_LENGTH (BC_node_t, bc_copies); i++) {
    ...
    if (BC_IS_OF_TYPE (bc, BC_NM_op2) && !BC_IS_OF_TYPE (bc, BC_NM_op2i)
        && !BC_IS_OF_TYPE (bc, BC_NM_op2i12) && !BC_IS_OF_TYPE (bc, BC_NM_op3i2)
        && !BC_IS_OF_TYPE (bc, BC_NM_op4i2) && (op = BC_op2 (bc)) >= 0)
      BC_set_op2 (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_op3) && !BC_IS_OF_TYPE (bc, BC_NM_op3i) && (op = BC_op3 (bc)) >= 0)
      BC_set_op3 (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_op4) && !BC_IS_OF_TYPE (bc, BC_NM_op4i24)
        && (op = BC_op4 (bc)) >= 0)
      BC_set_op4 (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_brs) && (op = BC_res (bc)) >= 0) BC_set_res (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_bcmp)) {
      if (!BC_IS_OF_TYPE (bc, BC_NM_bcmpi) && (op = BC_bcmp_op2 (bc)) >= 0)
        BC_set_bcmp_op2 (bc, op + base);
      if ((op = BC_bcmp_res (bc)) >= 0) BC_set_bcmp_res (bc, op + base);
    }
    if (BC_IS_OF_TYPE (bc, BC_NM_check2) && !BC_IS_OF_TYPE (bc, BC_NM_check2i)
        && !BC_IS_OF_TYPE (bc, BC_NM_check3i2) && (op = BC_ch_op2 (bc)) >= 0)
      BC_set_ch_op2 (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_check3) && (op = BC_ch_op3 (bc)) >= 0)
      BC_set_ch_op3 (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_check4) && !BC_IS_OF_TYPE (bc, BC_NM_check4i)
        && !BC_IS_OF_TYPE (bc, BC_NM_check5i45) && (op = BC_ch_op4 (bc)) >= 0)
      BC_set_ch_op4 (bc, op + base);
    if (BC_IS_OF_TYPE (bc, BC_NM_check5) && !BC_IS_OF_TYPE (bc, BC_NM_check5i)
        && (op = BC_ch_op5 (bc)) >= 0)
...
static BC_node_t inline_call (BC_node_t info) {
  int level;
  unsigned int len;
  ...
  if (BC_ext_life_p (fblock) || in_inline_stack_p (fblock) || fblock == curr_bc_block)
  ...
  tvars_num
    = (var_base - BC_vars_num (curr_bc_block) + BC_vars_num (fblock) + BC_tvars_num (fblock));
  if (tvars_num > BC_tvars_num (curr_bc_block)) BC_set_tvars_num (curr_bc_block, tvars_num);
  curr_info = NULL;
  res = BC_info (start);
  link_info (res);
  last_finish = new_bc_code (BC_NM_stdecu, new_bc_node (BC_NM_source, no_position));
  ...
  for (info = BC_next_info (BC_info (fblock));; info = BC_next_info (info)) {
    ...
    if (BC_IS_OF_TYPE (bc, BC_NM_fbend)) {
      ...
      BC_set_pos (BC_source (BC_info (last_finish)), BC_pos (BC_source (info)));
      break;
    } else if (BC_IS_OF_TYPE (bc, BC_NM_leave)) {
      d_assert (level == 0);
      if (finish == NULL) {
        finish = new_bc_code (BC_NM_stdecu, new_bc_node (BC_NM_source, BC_pos (BC_source (info))));
      ...
      if (BC_NODE_MODE (last) == BC_NM_tcall)
        ...
        bc_mode = BC_NM__error;
      if (bc_mode != BC_NM__error) BC_SET_MODE (last, bc_mode);
    }
    if (BC_IS_OF_TYPE (bc, BC_NM_block))
      level++;
    else if (BC_IS_OF_TYPE (bc, BC_NM_bend)) {
...
static void optimize_bc (BC_node_t start) {
  ...
  for (info = BC_info (start); info != NULL; info = BC_next_info (info)) {
    ...
    if ((next = BC_next (bc)) != NULL) BC_set_next (bc, go_through (next));
    if (BC_IS_OF_TYPE (bc, BC_NM_br) && (next = BC_pc (bc)) != NULL)
      BC_set_pc (bc, go_through (next));
    if (BC_IS_OF_TYPE (bc, BC_NM_foreach) && (next = BC_body_pc (bc)) != NULL)
      BC_set_body_pc (bc, go_through (next));
    if (BC_IS_OF_TYPE (bc, BC_NM_block) && (next = BC_excepts (bc)) != NULL)
      BC_set_excepts (bc, go_through (next));
    if (BC_IS_OF_TYPE (bc, BC_NM_except) && (next = BC_next_except (bc)) != NULL)
      BC_set_next_except (bc, go_through (next));
    if (BC_IS_OF_TYPE (bc, BC_NM_check) && (next = BC_fail_pc (bc)) != NULL)
...
static void mark_reachable_info (BC_node_t bc) {
  BC_node_t info;

  for (; bc != NULL; bc = BC_next (bc)) {
    info = BC_info (bc);
    if (BC_reachable_p (info)) return;
    BC_set_reachable_p (info, TRUE);
    if (BC_IS_OF_TYPE (bc, BC_NM_br)) mark_reachable_info (BC_pc (bc));
    if (BC_IS_OF_TYPE (bc, BC_NM_foreach)) mark_reachable_info (BC_body_pc (bc));
    if (BC_IS_OF_TYPE (bc, BC_NM_block)) mark_reachable_info (BC_excepts (bc));
    if (BC_IS_OF_TYPE (bc, BC_NM_except)) mark_reachable_info (BC_next_except (bc));
    if (BC_IS_OF_TYPE (bc, BC_NM_check)) mark_reachable_info (BC_fail_pc (bc));
  }
}

/* Process nodes from START removing uneccessary nodes.  */
static void dce_bc (BC_node_t start) {
  BC_node_t info, bc, next, next_info, stinc;
...
void initiate_context (void) {
  ...
  if (optimize_flag) initiate_inference_pass ();
}
