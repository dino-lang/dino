...
static void do_always_inline extract_op1 (ER_node_t *op1) { *op1 = get_op (BC_op1 (cpc)); }

static void do_always_inline extract_op2 (ER_node_t *op1, ER_node_t *op2) {
  extract_op1 (op1);
  *op2 = get_op (BC_op2 (cpc));
}

static void do_always_inline extract_op3 (ER_node_t *op1, ER_node_t *op2, ER_node_t *op3) {
  extract_op2 (op1, op2);
  *op3 = get_op (BC_op3 (cpc));
}

ER_node_t do_inline find_context_by_scope (BC_node_t scope) {
  ER_node_t container;
...
#endif
  for (container = cstack, curr_scope = ER_block_node (container); scope != curr_scope;
       container = ER_context (container), curr_scope = ER_block_node (container))
    ;
#ifndef NO_CONTAINER_CACHE
...
void do_inline process_var_ref_and_val (ER_node_t res, BC_node_t vdecl, int val_too_p) {
  ...
  if (val_too_p)
    *(val_t *) IVAL (res, 2) = *(val_t *) IVAL (ER_stack_vars (container), var_number_in_block);
}

static ER_node_t do_inline get_var_val_ref (BC_node_t vdecl) {
  int var_number_in_block = BC_var_num (vdecl);
  BC_node_t scope = BC_decl_scope (vdecl);
  ER_node_t container;
...
void do_inline process_var_val (ER_node_t res, BC_node_t vdecl) {
  ER_node_t ref;

  ref = get_var_val_ref (vdecl);
  if (ER_NODE_MODE (ref) == ER_NM_undef)
    eval_error (accessop_bc_decl, get_cpos (), DERR_undefined_value_access, BC_ident (vdecl));
  *(val_t *) res = *(val_t *) ref;
}

void do_inline process_external_var (ER_node_t res, BC_node_t evdecl, int lvalue_p, int val_too_p) {
  void *addr;
  ...
  if (lvalue_p) {
    ...
    if (val_too_p) {
      if (ER_NODE_MODE ((ER_node_t) addr) == ER_NM_undef)
        eval_error (accessop_bc_decl, get_cpos (), DERR_undefined_value_access, BC_ident (evdecl));
...
position_t get_designator_pos (void) {
...
static void do_inline check_member_access (BC_node_t decl, BC_node_t fblock) {
  if (!BC_public_p (decl)) {
    BC_node_t curr_block, friend;

    /* ??? Is it worth to make it more effective (e.g. bit tables). */
    for (curr_block = ER_block_node (cstack); curr_block != NULL;
         curr_block = BC_scope (curr_block))
      if (curr_block == fblock)
        break;
      else {
        for (friend = BC_friends (fblock); friend != NULL; friend = BC_next_friend (friend))
          if (BC_friend (friend) == curr_block) break;
        if (friend != NULL) break;
      }
    if (curr_block == NULL)
      eval_error (accessop_bc_decl, get_designator_pos (),
                  DERR_private_decl_access_from_outside_block, BC_ident (decl));
  }
}
...
/* Find and return decl with BLOCK_DECL_IDENT_NUMBER starting with
   *CONTAINER.  Pass the container of the found decl through
   CONTAINER.  */
BC_node_t do_inline find_field (int block_decl_ident_number, ER_node_t *container) {
  ...
  for (curr_container = *container;;) {
    ...
    if (block_decl_ident_number >= 0) decl = LV_BLOCK_IDENT_DECL (block, block_decl_ident_number);
    if (decl != NULL || BC_NODE_MODE (block) != BC_NM_fblock || !BC_class_p (block)) break;
  ...
  if (decl == NULL)
    eval_error (accessop_bc_decl, get_cpos (), DERR_decl_is_absent_in_given_class_or_block);
  else
...
void do_inline execute_general_period_operation (int block_decl_ident_number, ER_node_t res,
                                                 ER_node_t op, int lvalue_p, int lvalue_val_p) {
  BC_node_t decl;
  ER_node_t container, val;

  if (ER_NODE_MODE (op) != ER_NM_stack)
    eval_error (accessop_bc_decl, get_cpos (), DERR_value_is_not_class_instance_or_stack);
  ...
  case BC_NM_vdecl:
    ...
    if (!lvalue_p)
      ...
      if (lvalue_val_p) val = IVAL (res, 2);
    }
    if (val != NULL) {
      ER_node_t ref = IVAL (ER_stack_vars (container), BC_var_num (decl));

      if (ER_NODE_MODE (ref) == ER_NM_undef)
        eval_error (accessop_bc_decl, get_cpos (), DERR_undefined_value_access, BC_ident (decl));
      *(val_t *) val = *(val_t *) ref;
    }
    BC_set_hint (cpc, decl);
    break;
  case BC_NM_evdecl: process_external_var (res, decl, lvalue_p, lvalue_val_p); break;
  case BC_NM_fdecl: {
    BC_node_t fblock = BC_fblock (decl);

    if (BC_forward_p (fblock))
      eval_error (accessvalue_bc_decl, get_cpos (), DERR_undefined_class_or_fun, BC_ident (decl));
    if (lvalue_p)
      eval_error (accessop_bc_decl, get_cpos (),
                  BC_NODE_MODE (fblock) == BC_NM_fblock && BC_class_p (fblock)
                    ? DERR_class_as_variable
                    : DERR_fun_as_variable);
  ...
  case BC_NM_efdecl:
    if (lvalue_p) eval_error (accessop_bc_decl, get_cpos (), DERR_fun_as_variable);
...
void do_inline load_packed_vector_element (ER_node_t to, ER_node_t vect, rint_t index_val) {
  ER_node_t t;
  ...
  case 255: abort ();
#endif
  case ER_NM_nil: break;
  case ER_NM_hide: ER_set_hide (to, ((hide_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_char: ER_set_ch (to, ((ucode_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_byte:
    ER_SET_MODE (to, ER_NM_char);
    ER_set_ch (to, ((byte_t *) ER_pack_els (vect))[index_val]);
    break;
  case ER_NM_int: ER_set_i (to, ((rint_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_float: ER_set_f (to, ((rfloat_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_long: ER_set_l (to, ((ER_node_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_type:
    ER_set_type (to, (type_val_t) ((ER_node_mode_t *) ER_pack_els (vect))[index_val]);
    break;
  case ER_NM_vect:
  ...
  case ER_NM_tab:
    ...
    ER_set_tab (to, t);
    break;
  case ER_NM_thread: ER_set_thread (to, ((ER_node_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_stack: ER_set_stack (to, ((ER_node_t *) ER_pack_els (vect))[index_val]); break;
  case ER_NM_code:
    el_type_size = type_size_table[el_type];
    memcpy ((char *) to + val_displ_table[ER_NODE_MODE (to)],
            ER_pack_els (vect) + index_val * el_type_size, el_type_size);
    break;
  default: d_unreachable ();
  }
}

void do_inline load_vector_element_by_index (ER_node_t to, ER_node_t vect, ER_node_t index) {
  rint_t index_val;
  int pack_flag;
...
static void do_inline store_packed_vector_element (ER_node_t vect, rint_t index_val,
                                                   ER_node_t val) {
  ER_node_mode_t el_type;
  size_t el_type_size;
...
#endif
  case ER_NM_nil: break;
  case ER_NM_hide: ((hide_t *) ER_pack_els (vect))[index_val] = ER_hide (val); break;
  case ER_NM_char: ((ucode_t *) ER_pack_els (vect))[index_val] = ER_ch (val); break;
  case ER_NM_byte: {
  ...
  case ER_NM_int: ((rint_t *) ER_pack_els (vect))[index_val] = ER_i (val); break;
  case ER_NM_float: ((rfloat_t *) ER_pack_els (vect))[index_val] = ER_f (val); break;
  case ER_NM_long: ((ER_node_t *) ER_pack_els (vect))[index_val] = ER_l (val); break;
  case ER_NM_type:
    ((ER_node_mode_t *) ER_pack_els (vect))[index_val] = (ER_node_mode_t) ER_type (val);
    break;
  case ER_NM_vect: set_packed_vect_el (vect, index_val, ER_vect (val)); break;
  case ER_NM_tab: ((ER_node_t *) ER_pack_els (vect))[index_val] = ER_tab (val); break;
  case ER_NM_thread: ((ER_node_t *) ER_pack_els (vect))[index_val] = ER_thread (val); break;
  case ER_NM_stack: ((ER_node_t *) ER_pack_els (vect))[index_val] = ER_stack (val); break;
  case ER_NM_code:
    el_type_size = type_size_table[el_type];
    memcpy (ER_pack_els (vect) + index_val * el_type_size,
            (char *) val + val_displ_table[ER_NODE_MODE (val)], el_type_size);
    break;
  default: d_unreachable ();
  }
}
...
static void do_inline store_vector_element (ER_node_t vect, ER_node_t index, ER_node_t val) {
  rint_t index_val;
  int pack_flag;
  ...
  if (ER_immutable (vect))
    eval_error (immutable_bc_decl, get_designator_pos (), DERR_immutable_vector_modification);
  index_val = check_vector_index (vect, index);
  if (pack_flag && ER_pack_vect_el_mode (vect) != ER_NODE_MODE (val)
      && (ER_pack_vect_el_mode (vect) != ER_NM_byte || ER_NODE_MODE (val) != ER_NM_char)) {
...
void do_inline load_table_element_by_key (ER_node_t to, ER_node_t tab, ER_node_t key) {
  ER_node_t entry;
...
static void do_inline store_table_element (ER_node_t tab, ER_node_t index, ER_node_t val) {
  ER_node_t entry;

  GO_THROUGH_REDIR (tab);
  if (ER_immutable (tab))
    eval_error (immutable_bc_decl, get_designator_pos (), DERR_immutable_table_modification);
...
static void do_inline store_stack_element (ER_node_t stack, ER_node_t index, ER_node_t val) {
  if (ER_immutable (stack))
    eval_error (immutable_bc_decl, get_designator_pos (), DERR_immutable_instance_modification);
  *(val_t *) IVAL (ER_stack_vars (stack), ER_i (index)) = *(val_t *) val;
}

/* Used only for op store when check is done for lvalue value.  */
void do_inline store_designator_value (ER_node_t container, ER_node_t index, ER_node_t val) {
...
void do_inline store_vect_tab_designator_value (ER_node_t vec_tab, ER_node_t index, ER_node_t val) {
  if (ER_NODE_MODE (vec_tab) == ER_NM_vect)
    ...
    eval_error (indexop_bc_decl, get_designator_pos (), DERR_index_operation_for_non_vec_tab);
}

void do_inline store_stack_designator_value (ER_node_t stack, ER_node_t index, ER_node_t val) {
  if (ER_NODE_MODE (stack) != ER_NM_stack)
    eval_error (accessop_bc_decl, get_designator_pos (), DERR_value_is_not_class_instance_or_stack);
...
/* Return and check slice characteristics (start, bound, step and
   ...
   START_VAL.  */
static void do_inline check_and_get_slice_info (ER_node_t start_val, unsigned_rint_t vec_len,
  ...
  if (ER_NODE_MODE (IVAL (start_val, 0)) != ER_NM_int)
    eval_error (slicetype_bc_decl, get_designator_pos (), DERR_slice_start_is_not_int, depth);
  if (ER_NODE_MODE (IVAL (start_val, 1)) != ER_NM_int)
    eval_error (slicetype_bc_decl, get_designator_pos (), DERR_slice_bound_is_not_int, depth);
  if (ER_NODE_MODE (IVAL (start_val, 2)) != ER_NM_int)
    eval_error (slicetype_bc_decl, get_designator_pos (), DERR_slice_step_is_not_int, depth);
  start0 = ER_i (IVAL (start_val, 0));
  bound0 = ER_i (IVAL (start_val, 1));
  step0 = ER_i (IVAL (start_val, 2));
  if (start0 < 0)
    eval_error (sliceform_bc_decl, get_designator_pos (), DERR_slice_start_is_negative, depth);
  if (step0 == 0)
    eval_error (sliceform_bc_decl, get_designator_pos (), DERR_slice_step_is_zero, depth);
  if (bound0 < 0) {
    bound0 = vec_len + bound0 + 1;
    if (bound0 < 0) bound0 = 0;
  } else if (bound0 > vec_len)
    bound0 = vec_len;
  abs_step = (step0 > 0 ? step0 : -step0);
  niter0 = (bound0 - start0 + abs_step - 1) / abs_step;
  if (niter0 < 0) niter0 = 0;
  if (step0 > 0)
...
static ER_node_t process_slice_extract (ER_node_t container1, ER_node_t start_val1, int dim1,
                                        int depth) {
  ...
  check_and_get_slice_info (start_val1, len1, depth, &start1, &bound1, &step1, &niter1);
  if (dim1 == 1) {
    if (pack_flag1) {
      ...
      case ER_NM_type:
        for (i = 0, i1 = start1; i1 != bound1; i++, i1 += step1)
          ((ER_node_mode_t *) pack_els)[i] = ((ER_node_mode_t *) pack_els1)[i1];
        break;
      case ER_NM_long:
      case ER_NM_vect:
      case ER_NM_tab:
      ...
      case ER_NM_code: {
        ...
        for (i = 0, i1 = start1; i1 != bound1; i++, i1 += step1)
          memcpy (pack_els + i * el_type_size1, pack_els1 + i1 * el_type_size1, el_type_size1);
        break;
      }
      default: d_unreachable ();
      }
    ...
    if (!pack_flag1)
      ...
      eval_error (sliceform_bc_decl, get_designator_pos (), DERR_slice_operand_form, depth);
    else
      pack_els1 = ER_pack_els (vect1);
    dim1--;
    ...
    if (pack_flag1) {
      for (i = 0, i1 = start1; i1 != bound1; i++, i1 += step1) {
        p = process_slice_extract (((ER_node_t *) pack_els1)[i1], IVAL (start_val1, 3), dim1,
                                   depth + 1);
      ...
      for (i = 0, i1 = start1; i1 != bound1; i++, i1 += step1) {
        ...
        if (ER_NODE_MODE (v1) != ER_NM_vect)
          eval_error (sliceform_bc_decl, get_designator_pos (), DERR_slice_operand_form, depth);
        v1 = ER_vect (v1);
        p = process_slice_extract (v1, IVAL (start_val1, 3), dim1, depth + 1);
        set_packed_vect_el (vect, i, p);
      }
    }
...
/* Extract slice starting with CONTAINER and of dimension DIM.  Put
   the result into RES.  */
void do_inline slice_extract (ER_node_t res, ER_node_t container, int dim) {
  ER_node_t vect;

  if (ER_NODE_MODE (container) != ER_NM_vect)
    eval_error (sliceform_bc_decl, get_designator_pos (), DERR_slice_operand_form, 1);
  vect = process_slice_extract (ER_vect (container), IVAL (container, 1), dim, 1);
...
/* Process slice assign to CONTAINER1 of dimension DIM1 (> 0) with
   ...
   otherwise.  */
static void process_slice_assign (ER_node_t container1, ER_node_t start_val1, int dim1,
  ...
  if (dim2 != 0 && niter1 != niter2)
    eval_error (optype_bc_decl, get_designator_pos (), DERR_different_slice_operand_lengths,
  ...
  if (dim1 == 1 && dim2 == 0) {
    ...
    if (pack_flag1) {
      ...
      if (el_type1 == el_type2 || (el_type1 == ER_NM_byte && el_type2 == ER_NM_char)) {
        pack_els1 = ER_pack_els (vect1);
        switch (el_type1) {
        case ER_NM_nil: break;
        case ER_NM_hide:
        ...
        case ER_NM_type:
          for (i1 = start1; i1 != bound1; i1 += step1)
            ((ER_node_mode_t *) pack_els1)[i1] = (ER_node_mode_t) ER_type (container2);
          break;
        case ER_NM_vect: v2 = ER_vect (container2); goto node_common1;
        case ER_NM_tab: v2 = ER_tab (container2); goto node_common1;
        case ER_NM_thread: v2 = ER_thread (container2); goto node_common1;
        case ER_NM_stack:
          v2 = ER_thread (container2);
        node_common1:
          for (i1 = start1; i1 != bound1; i1 += step1) ((ER_node_t *) pack_els1)[i1] = v2;
          break;
        case ER_NM_code: {
          size_t el_type_size1 = type_size_table[el_type1];
          char *v = ((char *) container2 + val_displ_table[ER_NODE_MODE (container2)]);
    ...
    if (!done_p) {
      if (pack_flag1) vect1 = unpack_vector (vect1);
      unpack_els1 = ER_unpack_els (vect1);
      for (i1 = start1; i1 != bound1; i1 += step1)
    ...
    if (pack_flag1 && !pack_flag2) {
      ...
      if (ER_pack_vect_el_mode (vect1) == ER_NM_char && ER_pack_vect_el_mode (vect2) == ER_NM_byte)
        vect2 = bytevect_to_ucodevect (vect2);
      else if (ER_pack_vect_el_mode (vect1) == ER_NM_byte
               && ER_pack_vect_el_mode (vect2) == ER_NM_char)
        vect1 = bytevect_to_ucodevect (vect1);
    }
    if (pack_flag1 && ER_pack_vect_el_mode (vect1) != ER_pack_vect_el_mode (vect2)) {
      d_assert (pack_flag2);
    ...
    if (pack_flag1) {
      ...
      case ER_NM_float:
        for (i1 = start1, i2 = 0; i1 != bound1; i1 += step1, i2++)
          ((rfloat_t *) pack_els1)[i1] = ((rfloat_t *) pack_els2)[i2];
        break;
      case ER_NM_long:
        for (i1 = start1, i2 = 0; i1 != bound1; i1 += step1, i2++)
      ...
      case ER_NM_type:
        for (i1 = start1, i2 = 0; i1 != bound1; i1 += step1, i2++)
          ((ER_node_mode_t *) pack_els1)[i1] = ((ER_node_mode_t *) pack_els2)[i2];
        break;
      case ER_NM_vect:
      case ER_NM_tab:
      case ER_NM_thread:
      ...
      case ER_NM_code: {
        size_t el_type_size1 = type_size_table[el_type1];
        for (i1 = start1, i2 = 0; i1 != bound1; i1 += step1, i2++)
          memcpy (pack_els1 + i1 * el_type_size1, pack_els2 + i2 * el_type_size1, el_type_size1);
        break;
      }
      default: d_unreachable ();
      }
    ...
    if (pack_flag1) {
      for (i1 = start1; i1 != bound1; i1 += step1)
        process_slice_assign (((ER_node_t *) pack_els1)[i1], IVAL (start_val1, 3), dim1, container2,
                              0, depth + 1);
    } else {
      for (i1 = start1; i1 != bound1; i1 += step1) {
        if (pack_flag1)
        ...
        process_slice_assign (v1, IVAL (start_val1, 3), dim1, container2, 0, depth + 1);
      }
    }
  } else {
    ...
    if (pack_flag1 && pack_flag2) {
      for (i1 = start1, i2 = 0; i1 != bound1; i1 += step1, i2++)
        process_slice_assign (((ER_node_t *) pack_els1)[i1], IVAL (start_val1, 3), dim1,
                              ((ER_node_t *) pack_els2)[i2], dim2, depth + 1);
    } else {
      for (i1 = start1, i2 = 0; i1 != bound1; i1 += step1, i2++) {
        if (pack_flag1)
          v1 = ((ER_node_t *) pack_els1)[i1];
        ...
        process_slice_assign (v1, IVAL (start_val1, 3), dim1, v2, dim2, depth + 1);
      }
    }
  }
}
...
void do_inline slice_assign (ER_node_t container, int dim, ER_node_t val) {
  int val_dim = 0;
  ...
  process_slice_assign (ER_vect (container), IVAL (container, 1), dim, val, val_dim, 1);
}
...
static void find_catch_pc (ER_node_t except) {
  int jump_p;
  BC_node_t block;
  ER_node_t message;
  ...
  if (trace_flag) VARR_TRUNC (trace_stack_elem_t, trace_stack, 0);
  ...
  for (; cstack != uppest_stack;) {
    ...
    if (BC_NODE_MODE (block) == BC_NM_fblock && BC_fiber_p (block)) /* It can change cstack.  */
      delete_cthread_during_exception ();
    if (cstack != uppest_stack) heap_pop ();
    if (ER_c_stack_p (cstack)) jump_p = TRUE;
    /* Set up ctop as it should be for any statement begin.  */
    if (cstack != NULL)
      ctop = (ER_node_t) ((char *) cvars + BC_vars_num (ER_block_node (cstack)) * sizeof (val_t)
                          - sizeof (val_t));
  ...
  if (internal_isa_call (NULL, (ER_node_t) &v2, (ER_node_t) &v1)) {
    BC_node_t decl = get_another_block_decl (ER_block_node (except), msg_bc_decl);
    ...
    if (ER_NODE_MODE (message) == ER_NM_vect) {
      ...
      if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect
        ...
        const char *message
          = (ER_pack_vect_el_mode (vect) == ER_NM_byte
               ? encode_byte_str_varr ((byte_t *) ER_pack_els (vect), curr_byte_cd,
                                       curr_encoding_type, temp_varr, &len)
               : encode_ucode_str_varr ((ucode_t *) ER_pack_els (vect), curr_ucode_cd,
                                        curr_encoding_type, temp_varr, &len));

        /* No return after error unless REPL. */
        if (message != NULL) d_error (!repl_flag, exception_position, message);
      }
    }
  }
  if (!repl_flag) {
    /* Message is not a string or wrong message encoding.  */
    if (ER_block_node (except) != sigint_bc_decl && ER_block_node (except) != sigterm_bc_decl)
...
static int do_always_inline long_bin_op (ER_node_t op1, ER_node_t op2) {
  return ER_NODE_MODE (op1) == ER_NM_long && ER_NODE_MODE (op2) == ER_NM_long;
}

static do_always_inline ER_node_t lbop (ER_node_t a, ER_node_t b,
                                        void mpzf (mpz_t, const mpz_t, const mpz_t)) {
...
static void do_always_inline execute_ar_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p,
...
void do_inline execute_plus_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_ar_op (res, op1, op2, vect_p, DERR_plus_operands_types, i_plus, f_plus, lplus);
}

void do_inline execute_minus_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_ar_op (res, op1, op2, vect_p, DERR_minus_operands_types, i_minus, f_minus, lminus);
}

void do_inline execute_mult_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_ar_op (res, op1, op2, vect_p, DERR_mult_operands_types, i_mult, f_mult, lmult);
}

void do_inline execute_div_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_ar_op (res, op1, op2, vect_p, DERR_div_operands_types, i_div, f_div, lidiv);
}

void do_inline execute_mod_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_ar_op (res, op1, op2, vect_p, DERR_mod_operands_types, i_mod, f_mod, limod);
}

/* Do int operations using iop on OP1 and OP2.  Put result into
   RES.  Use MSG in case of error.  Check and do vector operations if
   VECT_P.  */
static void do_always_inline execute_int_op (ER_node_t res, ER_node_t op1, ER_node_t op2,
                                             int vect_p, const char *err_message,
...
void do_inline execute_lshift_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_int_op (res, op1, op2, vect_p, DERR_lshift_operands_types, i_lshift);
}

void do_inline execute_rshift_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_int_op (res, op1, op2, vect_p, DERR_rshift_operands_types, i_rshift);
}

void do_inline execute_ashift_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_int_op (res, op1, op2, vect_p, DERR_ashift_operands_types, i_ashift);
}

void do_inline execute_and_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_int_op (res, op1, op2, vect_p, DERR_and_operands_types, i_and);
}

void do_inline execute_xor_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_int_op (res, op1, op2, vect_p, DERR_xor_operands_types, i_xor);
}

void do_inline execute_or_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  execute_int_op (res, op1, op2, vect_p, DERR_or_operands_types, i_or);
}

/* The following function implements array concatenation.  */
void do_inline execute_concat_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p,
                                  int append_p) {
  size_t els_number1;
  ...
  if (!append_p) op1 = to_vect_string_conversion (op1, NULL, (ER_node_t) &tvar1);
  if (ER_NODE_MODE (op2) != ER_NM_vect || (!append_p && ER_NODE_MODE (op1) != ER_NM_vect))
  ...
  if (ER_NODE_MODE (vect2) == ER_NODE_MODE (vect1)
    ...
    if (ER_pack_vect_el_mode (vect1) == ER_NM_byte && ER_pack_vect_el_mode (vect2) == ER_NM_char)
      vect1 = bytevect_to_ucodevect (vect1);
  ...
  if (ER_NODE_MODE (vect2) != ER_NODE_MODE (vect1)
      || (ER_NODE_MODE (vect2) == ER_NM_heap_pack_vect
          && (ER_pack_vect_el_mode (vect2) != ER_pack_vect_el_mode (vect1)))) {
    if (ER_NODE_MODE (vect2) == ER_NM_heap_pack_vect) vect2 = unpack_vector (vect2);
    if (ER_NODE_MODE (vect1) == ER_NM_heap_pack_vect) vect1 = unpack_vector (vect1);
  }
  if (ER_NODE_MODE (vect2) == ER_NM_heap_pack_vect) {
    ...
    else if (ER_pack_vect_el_mode (vect2) == ER_pack_vect_el_mode (vect1))
      result_el_type = ER_pack_vect_el_mode (vect2);
    else
      d_unreachable ();
    ...
    if (append_p)
      ...
      result = create_pack_vector (els_number1 + ER_els_number (vect2), result_el_type);
      if (els_number1 != 0)
        memcpy (ER_pack_els (result), ER_pack_els (vect1), els_number1 * el_size);
    }
    if (ER_els_number (vect2) != 0) {
      memcpy (ER_pack_els (result) + els_number1 * el_size, ER_pack_els (vect2),
              ER_els_number (vect2) * el_size);
      ER_set_els_number (result, els_number1 + ER_els_number (vect2));
    }
    if (result_el_type == ER_NM_byte)
      ((byte_t *) ER_pack_els (result))[els_number1 + ER_els_number (vect2)] = '\0';
    else if (result_el_type == ER_NM_char)
      ((ucode_t *) ER_pack_els (result))[els_number1 + ER_els_number (vect2)] = '\0';
    ER_SET_MODE (res, ER_NM_vect);
    ...
    if (ER_els_number (vect2) != 0) {
      memcpy ((char *) ER_unpack_els (result) + els_number1 * sizeof (val_t), ER_unpack_els (vect2),
              ER_els_number (vect2) * sizeof (val_t));
      ER_set_els_number (result, els_number1 + ER_els_number (vect2));
    }
    ER_SET_MODE (res, ER_NM_vect);
    set_vect_dim (res, result, 0);
...
void do_inline execute_in_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  ER_node_t tab;
  ER_node_t entry;
  int cmp;
...
/* Common equality/inequality comparison CMP_OP of OP1 and OP2.
   Return the result.  */
int do_inline common_eq_ne_op (BC_node_mode_t cmp_op, ER_node_t op1, ER_node_t op2) {
  int cmp;
  ER_node_t l, r;
  ...
  if (ER_NODE_MODE (r) != ER_NODE_MODE (l)) {
    ...
    case 255: abort ();
#endif
    case ER_NM_undef: eval_error (optype_bc_decl, get_cpos (), DERR_eq_operands_types); break;
    case ER_NM_nil: cmp = 1; break;
    case ER_NM_hide: cmp = ER_hide (r) == ER_hide (l); break;
    case ER_NM_hideblock:
      if (ER_hideblock_length (r) != ER_hideblock_length (l))
        cmp = 0;
      else
        cmp = memcmp (ER_hideblock (r), ER_hideblock (l), ER_hideblock_length (r)) == 0;
      break;
    case ER_NM_char: d_unreachable (); /* ALWAYS transformed to string/int/float */
    case ER_NM_int: cmp = ER_i (r) == ER_i (l); break;
    case ER_NM_float: cmp = ER_f (r) == ER_f (l); break;
    case ER_NM_long: cmp = leq (ER_l (r), ER_l (l)); break;
    case ER_NM_type: cmp = ER_type (r) == ER_type (l); break;
    case ER_NM_vect: cmp = eq_vector (ER_vect (r), ER_vect (l)); break;
    case ER_NM_tab: cmp = eq_table (ER_tab (r), ER_tab (l)); break;
    case ER_NM_code:
      cmp = (ER_code_id (r) == ER_code_id (l) && (ER_code_context (r) == ER_code_context (l)));
      break;
    case ER_NM_efun:
      ...
      break;
    case ER_NM_thread: cmp = ER_thread (r) == ER_thread (l); break;
    case ER_NM_stack: cmp = eq_stack (ER_stack (r), ER_stack (l)); break;
    default: d_unreachable ();
    }
...
/* Implement common case of equality/inequality comparison CMP_OP of
   ...
   if VECT_P.  */
void do_inline execute_common_eq_ne_op (BC_node_mode_t cmp_op, ER_node_t res, ER_node_t op1,
                                        ER_node_t op2, int vect_p) {
  int cmp;
...
void do_inline execute_eq_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  comp_op (BC_NM_eq, res, op1, op2, vect_p, i_eq, f_eq, execute_common_eq_ne_op);
}

void do_inline execute_ne_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
...
void do_inline execute_common_cmp_op (BC_node_mode_t oper, ER_node_t res, ER_node_t op1,
                                      ER_node_t op2, int vect_p) {
  int cmp;
  ER_node_t l, r;
    ...
    cmp
      = (oper == BC_NM_lt
           ...
           : (oper == BC_NM_gt ? ER_i (l) > ER_i (r)
                               : (oper == BC_NM_le ? ER_i (l) <= ER_i (r) : ER_i (l) >= ER_i (r))));
  else if (float_bin_op (l, r))
    cmp
      = (oper == BC_NM_lt
           ? ER_f (l) < ER_f (r)
           : (oper == BC_NM_gt ? ER_f (l) > ER_f (r)
                               : (oper == BC_NM_le ? ER_f (l) <= ER_f (r) : ER_f (l) >= ER_f (r))));
  else if (long_bin_op (l, r))
    ...
    eval_error (optype_bc_decl, get_cpos (),
                (oper == BC_NM_lt
                   ...
                   : (oper == BC_NM_gt
                        ...
                        : (oper == BC_NM_le ? DERR_le_operands_types : DERR_ge_operands_types))));
  ER_SET_MODE (res, ER_NM_int);
  ER_set_i (res, cmp);
}

void do_inline execute_lt_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  comp_op (BC_NM_lt, res, op1, op2, vect_p, i_lt, f_lt, execute_common_cmp_op);
}

void do_inline execute_ge_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  comp_op (BC_NM_ge, res, op1, op2, vect_p, i_ge, f_ge, execute_common_cmp_op);
}

void do_inline execute_gt_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
  comp_op (BC_NM_gt, res, op1, op2, vect_p, i_gt, f_gt, execute_common_cmp_op);
}

void do_inline execute_le_op (ER_node_t res, ER_node_t op1, ER_node_t op2, int vect_p) {
...
/* Implement identity (if INDENTITY_P) or unidentity comparison of OP1
   ...
   VECT_P.  */
void do_inline execute_identity_op (int identity_p, ER_node_t res, ER_node_t op1, ER_node_t op2,
                                    int vect_p) {
  int cmp;
  ...
  if (ER_NODE_MODE (op1) != ER_NODE_MODE (op2)) {
    if (ER_NODE_MODE (op1) == ER_NM_undef || ER_NODE_MODE (op2) == ER_NM_undef)
      eval_error (optype_bc_decl, get_cpos (), DERR_identity_operands_types);
    cmp = FALSE;
  } else
    switch (ER_NODE_MODE (op1)) {
    case ER_NM_undef: eval_error (optype_bc_decl, get_cpos (), DERR_identity_operands_types); break;
    case ER_NM_nil: cmp = 1; break;
    case ER_NM_hide: cmp = ER_hide (op1) == ER_hide (op2); break;
    case ER_NM_hideblock: cmp = ER_hideblock (op1) == ER_hideblock (op2); break;
    case ER_NM_char: cmp = ER_ch (op1) == ER_ch (op2); break;
    case ER_NM_int: cmp = ER_i (op1) == ER_i (op2); break;
    case ER_NM_float: cmp = ER_f (op1) == ER_f (op2); break;
    case ER_NM_long: cmp = mpz_cmp (*ER_mpz_ptr (ER_l (op1)), *ER_mpz_ptr (ER_l (op2))) == 0; break;
    case ER_NM_type: cmp = ER_type (op1) == ER_type (op2); break;
    case ER_NM_vect: {
    ...
    case ER_NM_efun:
      ...
      break;
    case ER_NM_thread: cmp = ER_thread (op1) == ER_thread (op2); break;
    case ER_NM_stack: cmp = ER_stack (op1) == ER_stack (op2); break;
    default: d_unreachable ();
    }
  ER_SET_MODE (res, ER_NM_int);
...
static position_t get_left_operand_pos (void) {
  BC_node_t info = BC_info (cpc);
...
static position_t get_right_operand_pos (void) {
  BC_node_t info = BC_info (cpc);
...
static do_always_inline void int_pack_vect_op (int rev_p, size_t len, char *pack_res_els,
                                               char *pack_els1, char *pack_els2, ER_node_t op2,
                                               rint_t iop (rint_t, rint_t)) {
  size_t i;
  rint_t op_i2;
  ...
  if (pack_els2 != NULL) {
    d_assert (!rev_p);
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = iop (((rint_t *) pack_els1)[i], ((rint_t *) pack_els2)[i]);
  } else if (rev_p) {
    op_i2 = ER_i (op2);
    for (i = 0; i < len; i++) ((rint_t *) pack_res_els)[i] = iop (op_i2, ((rint_t *) pack_els1)[i]);
  } else {
    op_i2 = ER_i (op2);
    for (i = 0; i < len; i++) ((rint_t *) pack_res_els)[i] = iop (((rint_t *) pack_els1)[i], op_i2);
  }
}

/* Analogous the function above but for rfloat point operations.  */
static do_always_inline void float_pack_vect_op (int rev_p, size_t len, char *pack_res_els,
                                                 char *pack_els1, char *pack_els2, ER_node_t op2,
                                                 rfloat_t fop (rfloat_t, rfloat_t)) {
  size_t i;
  rfloat_t op_f2;
  ...
  if (pack_els2 != NULL) {
    ...
    for (i = 0; i < len; i++)
      ((rfloat_t *) pack_res_els)[i]
        = fop (((rfloat_t *) pack_els1)[i], ((rfloat_t *) pack_els2)[i]);
  } else if (rev_p) {
    op_f2 = ER_f (op2);
    for (i = 0; i < len; i++)
      ((rfloat_t *) pack_res_els)[i] = fop (op_f2, ((rfloat_t *) pack_els1)[i]);
  } else {
    op_f2 = ER_f (op2);
    for (i = 0; i < len; i++)
      ((rfloat_t *) pack_res_els)[i] = fop (((rfloat_t *) pack_els1)[i], op_f2);
  }
}

/* Analogous the function above but used for char comparison
   operations.  */
static do_always_inline void ucode_pack_vect_cmp_op (size_t len, char *pack_res_els,
                                                     char *pack_els1, char *pack_els2,
  ...
  if (pack_els2 != NULL) {
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = icmp (((ucode_t *) pack_els1)[i], ((ucode_t *) pack_els2)[i]);
  } else {
    op_i2 = ER_ch (op2);
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = icmp (((ucode_t *) pack_els1)[i], op_i2);
  }
}

/* Analogous the function above but used for byte comparison
   operations.  */
static do_always_inline void byte_pack_vect_cmp_op (size_t len, char *pack_res_els, char *pack_els1,
  ...
  if (pack_els2 != NULL) {
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = icmp (((byte_t *) pack_els1)[i], ((byte_t *) pack_els2)[i]);
  } else {
    op_i2 = ER_ch (op2);
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = icmp (((byte_t *) pack_els1)[i], op_i2);
  }
}

/* Analogous the function above but used for int comparison
   operations.  */
static do_always_inline void int_pack_vect_cmp_op (size_t len, char *pack_res_els, char *pack_els1,
                                                   char *pack_els2, ER_node_t op2,
                                                   int icmp (rint_t, rint_t)) {
  size_t i;
  rint_t op_i2;
  ...
  if (pack_els2 != NULL) {
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = icmp (((rint_t *) pack_els1)[i], ((rint_t *) pack_els2)[i]);
  } else {
    op_i2 = ER_i (op2);
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = icmp (((rint_t *) pack_els1)[i], op_i2);
  }
}

/* Analogous the function above but used for float comparison
   operations.  */
static do_always_inline void float_pack_vect_cmp_op (size_t len, char *pack_res_els,
                                                     char *pack_els1, char *pack_els2,
                                                     ER_node_t op2, int fcmp (rfloat_t, rfloat_t)) {
  size_t i;
  rfloat_t op_f2;
  ...
  if (pack_els2 != NULL) {
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i]
        = fcmp (((rfloat_t *) pack_els1)[i], ((rfloat_t *) pack_els2)[i]);
  } else {
    op_f2 = ER_f (op2);
    for (i = 0; i < len; i++)
      ((rint_t *) pack_res_els)[i] = fcmp (((rfloat_t *) pack_els1)[i], op_f2);
  }
}
...
static ER_node_t process_binary_vect_op (int rev_p, ER_node_t op1, int dim1, ER_node_t op2,
                                         int dim2, int depth) {
  ...
  if (dim2 == 0) {
    ...
    if (len1 != len2)
      eval_error (veclen_bc_decl, get_cpos (), DERR_different_vec_operand_lengths, (long) len1,
                  (long) len2, depth);
  }
  if (dim1 > 1) {
    ER_node_t p;
    ...
    if (pack_flag1 && el_type1 != ER_NM_vect)
      eval_error (vecform_bc_decl, get_left_operand_pos (), DERR_vector_form_type, depth);
    if (!scalar_p2) {
      if (pack_flag2 && el_type2 != ER_NM_vect)
        eval_error (vecform_bc_decl, get_right_operand_pos (), DERR_vector_form_type, depth);
      dim2--;
    }
  ...
  if (oper == BC_NM_madd) oper = madd_mult_p ? BC_NM_mult : BC_NM_add;
  switch (oper) {
  case BC_NM_add:
  case BC_NM_addi:
  case BC_NM_add_st:
  ...
  case BC_NM_mod_slst:
    if (pack_flag1 && pack_flag2 && el_type1 == el_type2) {
      ...
      if (el_type1 == ER_NM_int) {
        ...
        case BC_NM_add_st:
        case BC_NM_add_slst:
          int_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, i_plus);
          break;
        case BC_NM_sub:
        case BC_NM_sub_st:
        case BC_NM_sub_slst:
          int_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, i_minus);
          break;
        case BC_NM_mult:
        case BC_NM_multi:
        case BC_NM_mult_st:
        case BC_NM_mult_slst:
          int_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, i_mult);
          break;
        case BC_NM_div:
        case BC_NM_div_st:
        case BC_NM_div_slst:
          int_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, i_div);
          break;
        case BC_NM_mod:
        case BC_NM_mod_st:
        case BC_NM_mod_slst:
          int_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, i_mod);
          break;
        default: d_unreachable ();
        }
        ER_set_els_number (res, len1);
        ...
        case BC_NM_add_st:
        case BC_NM_add_slst:
          float_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, f_plus);
          break;
        case BC_NM_sub:
        case BC_NM_sub_st:
        case BC_NM_sub_slst:
          float_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, f_minus);
          break;
        case BC_NM_mult:
        case BC_NM_multi:
        case BC_NM_mult_st:
        case BC_NM_mult_slst:
          float_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, f_mult);
          break;
        case BC_NM_div:
        case BC_NM_div_st:
        case BC_NM_div_slst:
          float_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, f_div);
          break;
        case BC_NM_mod:
        case BC_NM_mod_st:
        case BC_NM_mod_slst:
          float_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, f_mod);
          break;
        default: d_unreachable ();
        }
        ER_set_els_number (res, len1);
    ...
    if (pack_flag1) op1 = unpack_vector (op1);
    unpack_els1 = ER_unpack_els (op1);
    if (!scalar_p2) {
      if (pack_flag2) op2 = unpack_vector (op2);
      unpack_els2 = ER_unpack_els (op2);
    }
    ...
    for (i = 0; i < len1; i++) {
      ...
      case BC_NM_addi:
      case BC_NM_add_st:
      case BC_NM_add_slst: execute_plus_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_sub:
      case BC_NM_sub_st:
      case BC_NM_sub_slst: execute_minus_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_mult:
      case BC_NM_multi:
      case BC_NM_mult_st:
      case BC_NM_mult_slst: execute_mult_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_div:
      case BC_NM_div_st:
      case BC_NM_div_slst: execute_div_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_mod:
      case BC_NM_mod_st:
      case BC_NM_mod_slst: execute_mod_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      default: d_unreachable ();
      }
    }
    ER_set_els_number (res, len1);
  ...
  case BC_NM_or_slst:
    if (pack_flag1 && pack_flag2 && el_type1 == el_type2) {
      ...
      if (el_type1 == ER_NM_int) {
        ...
        case BC_NM_lsh_st:
        case BC_NM_lsh_slst:
          int_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, i_lshift);
          break;
        case BC_NM_rsh:
        case BC_NM_rsh_st:
        case BC_NM_rsh_slst:
          int_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, i_rshift);
          break;
        case BC_NM_ash:
        case BC_NM_ash_st:
        case BC_NM_ash_slst:
          int_pack_vect_op (rev_p, len1, pack_res_els, pack_els1, pack_els2, op2, i_ashift);
          break;
        case BC_NM_and:
        case BC_NM_and_st:
        case BC_NM_and_slst:
          int_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, i_and);
          break;
        case BC_NM_xor:
        case BC_NM_xor_st:
        case BC_NM_xor_slst:
          int_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, i_xor);
          break;
        case BC_NM_or:
        case BC_NM_or_st:
        case BC_NM_or_slst:
          int_pack_vect_op (FALSE, len1, pack_res_els, pack_els1, pack_els2, op2, i_or);
          break;
        default: d_unreachable ();
        }
        return res;
    ...
    for (i = 0; i < len1; i++) {
      ...
      case BC_NM_lsh:
      case BC_NM_lsh_st:
      case BC_NM_lsh_slst: execute_lshift_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_rsh:
      case BC_NM_rsh_st:
      case BC_NM_rsh_slst: execute_rshift_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_ash:
      case BC_NM_ash_st:
      case BC_NM_ash_slst: execute_ashift_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_and:
      case BC_NM_and_st:
      case BC_NM_and_slst: execute_and_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_xor:
      case BC_NM_xor_st:
      case BC_NM_xor_slst: execute_xor_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      case BC_NM_or:
      case BC_NM_or_st:
      case BC_NM_or_slst: execute_or_op (el_res, (ER_node_t) &l, (ER_node_t) &r, FALSE); break;
      default: d_unreachable ();
      }
    }
    ER_set_els_number (res, len1);
  ...
  case BC_NM_concat_slst:
    ...
    for (i = 0; i < len1; i++) {
      ...
      execute_concat_op (IVAL (unpack_res_els, i), (ER_node_t) &l, (ER_node_t) &r, FALSE, FALSE);
    }
    ER_set_els_number (res, len1);
    try_full_pack (res);
    return res;
  ...
  case BC_NM_unid:
    ...
    if (pack_flag1 && pack_flag2) {
      ...
      if (el_type1 == el_type2) {
        switch (el_type1) {
        case ER_NM_char:
          ucode_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2,
                                  neg_p ? i_ne : i_eq);
          break;
        case ER_NM_byte:
          byte_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2,
                                 neg_p ? i_ne : i_eq);
          break;
        case ER_NM_int:
          int_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2, neg_p ? i_ne : i_eq);
          break;
        case ER_NM_float:
          float_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2,
                                  neg_p ? f_ne : f_eq);
          break;
        default: goto eq_common;
        }
        ER_set_els_number (res, len1);
        return res;
      }
  ...
  eq_common:
    ...
    for (i = 0; i < len1; i++) {
      ...
      if (!scalar_p2)
        ...
        r = *(val_t *) op2;
      if (oper == BC_NM_eq)
        execute_eq_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
      else if (oper == BC_NM_ne)
        execute_ne_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
      else
        execute_identity_op (!neg_p, (ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
  ...
  case BC_NM_gt:
    ...
    if (pack_flag1 && pack_flag2 && el_type1 == el_type2) {
      ...
      case ER_NM_int:
        switch (oper) {
        case BC_NM_lt:
          int_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2, rev_p ? i_ge : i_lt);
          break;
        case BC_NM_ge:
          int_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2, rev_p ? i_lt : i_ge);
          break;
        case BC_NM_le:
          int_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2, rev_p ? i_gt : i_le);
          break;
        case BC_NM_gt:
          int_pack_vect_cmp_op (len1, pack_res_els, pack_els1, pack_els2, op2, rev_p ? i_le : i_gt);
          break;
        default: d_unreachable ();
        }
        break;
  ...
  cmp_common:
    ...
    for (i = 0; i < len1; i++) {
      ...
      switch (oper) {
      case BC_NM_lt:
        execute_lt_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
        break;
      case BC_NM_ge:
        execute_ge_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
        break;
      case BC_NM_le:
        execute_le_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
        break;
      case BC_NM_gt:
        execute_gt_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
        break;
      default: d_unreachable ();
      }
      ((rint_t *) pack_res_els)[i] = ER_i ((ER_node_t) &rval);
  ...
  case BC_NM_in:
    ...
    for (i = 0; i < len1; i++) {
      ...
      execute_in_op ((ER_node_t) &rval, (ER_node_t) &l, (ER_node_t) &r, FALSE);
      ((rint_t *) pack_res_els)[i] = ER_i ((ER_node_t) &rval);
    }
    ER_set_els_number (res, len1);
    return res;
  case BC_NM_fmtvecof:
    ...
    for (i = 0; i < len1; i++) {
      ...
      execute_vectorof_op (IVAL (unpack_res_els, i), (ER_node_t) &l, (ER_node_t) &r, FALSE);
    }
    ER_set_els_number (res, len1);
    try_full_pack (res);
    return res;
...
static int do_always_inline get_dim (ER_node_t op) {
  return ER_NODE_MODE (op) == ER_NM_vect ? ER_dim (op) : 0;
}

/* Implement binary vector operation defined by CPC.  */
void binary_vect_op (ER_node_t res, ER_node_t op1, ER_node_t op2) {
  int rev_p = FALSE;
  int dim1, dim2, t;
  ER_node_t top, vect;
  ...
  if (ER_NODE_MODE (op1) == ER_NM_vect)
    ...
    eval_error (vecform_bc_decl, get_left_operand_pos (), DERR_vector_form_type, 1);
  if (dim2 != 0) {
    if (ER_NODE_MODE (op2) == ER_NM_vect)
      op2 = ER_vect (op2);
    else
      eval_error (vecform_bc_decl, get_right_operand_pos (), DERR_vector_form_type, 1);
  }
...
static do_always_inline void int_pack_vect_unary_op (size_t len, char *pack_res_els, char *pack_els,
                                                     rint_t iop (rint_t)) {
  size_t i;

  for (i = 0; i < len; i++) ((rint_t *) pack_res_els)[i] = iop (((rint_t *) pack_els)[i]);
}

/* Analogous to function above but for rfloat point operations.  */
static do_always_inline void float_pack_vect_unary_op (size_t len, char *pack_res_els,
                                                       char *pack_els, rfloat_t fop (rfloat_t)) {
  size_t i;

  for (i = 0; i < len; i++) ((rfloat_t *) pack_res_els)[i] = fop (((rfloat_t *) pack_els)[i]);
}
...
/* Recursive function implementing unary vector operations defined by
   ...
   depth is DEPTH.  */
static ER_node_t process_unary_vect_op (ER_node_t op, int dim, int depth) {
  int pack_flag;
  size_t i, len;
  BC_node_mode_t oper;
  ER_node_mode_t el_type;
  ...
  if (pack_flag) {
    if (el_type == ER_NM_float) {
      if (oper != BC_NM_plus && oper != BC_NM_minus) goto general_unary;
      ...
      if (oper == BC_NM_plus)
        ...
        float_pack_vect_unary_op (len, pack_res_els, pack_els, funary_minus);
    } else if (el_type == ER_NM_int) {
      if (oper != BC_NM_plus && oper != BC_NM_minus && oper != BC_NM_not && oper != BC_NM_bnot)
        goto general_unary;
      res = create_pack_vector (len, ER_NM_int);
      ER_set_els_number (res, 0);
      pack_res_els = ER_pack_els (res);
      switch (oper) {
      case BC_NM_not: int_pack_vect_unary_op (len, pack_res_els, pack_els, i_not); break;
      case BC_NM_bnot: int_pack_vect_unary_op (len, pack_res_els, pack_els, ibitwise_not); break;
      case BC_NM_plus: int_pack_vect_unary_op (len, pack_res_els, pack_els, iunary_plus); break;
      case BC_NM_minus: int_pack_vect_unary_op (len, pack_res_els, pack_els, iunary_minus); break;
      default: d_unreachable ();
      }
    } else
      goto general_unary;
...
general_unary:
  ...
  for (i = 0; i < len; i++) {
    ...
    el_res = IVAL (unpack_res_els, i);
    switch (oper) {
    case BC_NM_not: execute_not_op (el_res, el, FALSE); break;
    case BC_NM_bnot: execute_bitwise_not_op (el_res, el, FALSE); break;
    case BC_NM_plus:
      execute_unary_ar_op (el_res, el, FALSE, DERR_unary_plus_operand_type, iunary_plus,
                           funary_plus, lunary_plus);
      break;
    case BC_NM_minus:
      execute_unary_ar_op (el_res, el, FALSE, DERR_unary_minus_operand_type, iunary_minus,
                           funary_minus, lunary_minus);
      break;
    case BC_NM_length: execute_length_op (el_res, el, FALSE); break;
    case BC_NM_const: execute_const_op (el_res, el, FALSE); break;
    case BC_NM_new: execute_new_op (el_res, el, FALSE); break;
    case BC_NM_tpof: execute_typeof_op (el_res, el, FALSE); break;
    case BC_NM_chof: execute_charof_op (el_res, el, FALSE); break;
    case BC_NM_iof: execute_intof_op (el_res, el, FALSE); break;
    case BC_NM_lof: execute_longof_op (el_res, el, FALSE); break;
    case BC_NM_fof: execute_floatof_op (el_res, el, FALSE); break;
    case BC_NM_vecof: execute_vectorof_op (el_res, el, NULL, FALSE); break;
    case BC_NM_tabof: execute_tableof_op (el_res, el, FALSE); break;
    default: d_unreachable ();
    }
  }
  ER_set_els_number (res, len);
...
void unary_vect_op (ER_node_t res, ER_node_t op) {
  int dim;
  ER_node_t vect;
...
execute_pack_fold_op (ER_node_t res, rint_t initval, ER_node_t pack_vect, ER_node_mode_t el_type,
                      size_t len, char *pack_els, void genop (ER_node_t, ER_node_t, ER_node_t, int),
                      rint_t iop (rint_t, rint_t), rfloat_t fop (rfloat_t, rfloat_t)) {
  size_t i;
  val_t l, rval;
  ...
  if (el_type == ER_NM_int) {
    ires = initval;
    for (i = 0; i < len; i++) ires = iop (ires, ((rint_t *) pack_els)[i]);
    ER_SET_MODE (r, ER_NM_int);
    ER_set_i (r, ires);
  } else if (fop != NULL && el_type == ER_NM_float) {
    fres = initval;
    for (i = 0; i < len; i++) fres = fop (fres, ((rfloat_t *) pack_els)[i]);
...
static void process_fold_vect_op (ER_node_t res, ER_node_t op, int dim, int depth) {
  int pack_flag;
  size_t i, len;
  BC_node_mode_t oper;
  ER_node_mode_t el_type;
  ...
  if (pack_flag) {
    switch (oper) {
    case BC_NM_fold_add:
      execute_pack_fold_op (res, 0, op, el_type, len, pack_els, execute_plus_op, i_plus, f_plus);
      break;
    case BC_NM_fold_mult:
      execute_pack_fold_op (res, 1, op, el_type, len, pack_els, execute_mult_op, i_mult, f_mult);
      break;
    case BC_NM_fold_and:
      execute_pack_fold_op (res, ~(rint_t) 0, op, el_type, len, pack_els, execute_and_op, i_and,
                            NULL);
      break;
    case BC_NM_fold_xor:
      execute_pack_fold_op (res, 0, op, el_type, len, pack_els, execute_xor_op, i_xor, NULL);
      break;
    case BC_NM_fold_or:
      execute_pack_fold_op (res, 0, op, el_type, len, pack_els, execute_or_op, i_or, NULL);
      break;
    ...
    for (i = 0; i < len; i++) {
      ...
      switch (oper) {
      case BC_NM_fold_add: execute_plus_op (res, res, (ER_node_t) &l, FALSE); break;
      case BC_NM_fold_mult: execute_mult_op (res, res, (ER_node_t) &l, FALSE); break;
      case BC_NM_fold_and: execute_and_op (res, res, (ER_node_t) &l, FALSE); break;
      case BC_NM_fold_xor: execute_xor_op (res, res, (ER_node_t) &l, FALSE); break;
      case BC_NM_fold_or: execute_or_op (res, res, (ER_node_t) &l, FALSE); break;
      case BC_NM_fold_concat: execute_concat_op (res, res, (ER_node_t) &l, FALSE, TRUE); break;
      default: d_unreachable ();
      }
    }
  }
...
void fold_vect_op (ER_node_t res, ER_node_t op) {
  int dim;
  ...
  if (ER_NODE_MODE (op) == ER_NM_vect && dim > 0)
    ...
    eval_error (vecform_bc_decl, BC_pos2 (BC_info (cpc)), DERR_vector_form_type, 1);
  ER_SET_MODE (res, ER_NM_int);
  switch (BC_NODE_MODE (cpc)) {
  case BC_NM_fold_add:
  case BC_NM_fold_xor:
  case BC_NM_fold_or: ER_set_i (res, 0); break;
  case BC_NM_fold_mult: ER_set_i (res, 1); break;
  case BC_NM_fold_and: ER_set_i (res, ~(rint_t) 0); break;
  case BC_NM_fold_concat:
    ER_SET_MODE (res, ER_NM_vect);
...
void vec (ER_node_t res, ER_node_t op1, rint_t vect_parts_number) {
  ...
  if (vect_parts_number == 0)
    ...
    els_number = 0;
    for (curr_vect_part_number = 0; curr_vect_part_number < 2 * vect_parts_number;
         curr_vect_part_number += 2) {
      implicit_int_conversion (IVAL (op1, curr_vect_part_number), NULL);
      if (doubt (ER_NODE_MODE (IVAL (op1, curr_vect_part_number)) != ER_NM_int))
    ...
    pack_flag = TRUE;
    for (curr_vect_part_number = 3; curr_vect_part_number < 2 * vect_parts_number;
         curr_vect_part_number += 2)
      if (ER_NODE_MODE (IVAL (op1, 1)) != ER_NODE_MODE (IVAL (op1, curr_vect_part_number))) {
        pack_flag = FALSE;
        break;
      }
    if (pack_flag) {
      el_type_size = type_size_table[ER_NODE_MODE (IVAL (op1, 1))];
      vect = create_pack_vector (els_number, ER_NODE_MODE (IVAL (op1, 1)));
    } else
      vect = create_unpack_vector (els_number);
    ctop = saved_ctop;
    for (curr_vect_element_number = 0, curr_vect_part_number = 0;
         curr_vect_part_number < 2 * vect_parts_number; curr_vect_part_number += 2)
      if (ER_i (IVAL (op1, curr_vect_part_number)) > 0) {
        repetition = ER_i (IVAL (op1, curr_vect_part_number));
        while (repetition > 0) {
          if (pack_flag)
            memcpy (ER_pack_els (vect) + el_type_size * curr_vect_element_number,
                    (char *) IVAL (op1, curr_vect_part_number + 1)
                      + val_displ_table[ER_NODE_MODE (IVAL (op1, curr_vect_part_number + 1))],
                    el_type_size);
          else
            memcpy (IVAL (ER_unpack_els (vect), curr_vect_element_number),
                    IVAL (op1, curr_vect_part_number + 1), sizeof (val_t));
          repetition--;
          curr_vect_element_number++;
        }
      }
    d_assert ((unsigned_rint_t) curr_vect_element_number == els_number);
  }
  ER_SET_MODE (res, ER_NM_vect);
  set_vect_dim (res, vect, 0);
}

void tab (ER_node_t res, ER_node_t op1, rint_t tab_els_number) {
  ...
  for (curr_tab_el_number = 0; curr_tab_el_number < 2 * tab_els_number; curr_tab_el_number += 2) {
    entry = find_tab_el (tab, IVAL (op1, curr_tab_el_number), TRUE);
    d_assert (entry != NULL);
    if (ER_NODE_MODE (entry) != ER_NM_empty_el)
      eval_error (keyvalue_bc_decl, get_cpos (), DERR_repeated_key, (long long) curr_tab_el_number);
    *(val_t *) entry = *(val_t *) IVAL (op1, curr_tab_el_number);
    make_immutable (entry);
    *((val_t *) entry + 1) = *(val_t *) IVAL (op1, curr_tab_el_number + 1);
    d_assert (ER_IS_OF_TYPE (INDEXED_EL_VAL (entry, 0), ER_NM_val));
  }
  ctop = saved_ctop;
...
static void out (void) {
  ...
  for (i = 0; i < BC_op1 (cpc); i++) heap_pop ();
}

int throw (ER_node_t op1) {
  const char *message;
  ER_node_t op2;
  val_t tvar1;
  ...
  if (!ER_IS_OF_TYPE (op1, ER_NM_stack) || !internal_isa_call (&message, op2, op1))
    eval_error (optype_bc_decl, get_cpos (), DERR_no_exception_after_throw);
...
static int except (void) {
  ...
  if (ER_IS_OF_TYPE (op2, ER_NM_code) && internal_isa_call (&message, op2, op1)) {
    INCREMENT_PC ();
    d_assert (cpc != NULL && BC_IS_OF_TYPE (cpc, BC_NM_block));
    TOP_DOWN; /* exception */
...
void do_compile (void) {
  if (!compile_c_code (cpc, store_handle))
    eval_error (compile_bc_decl, get_cpos (), DERR_C_code_compilation_failure);
}

void evaluate_code (void) {
...
#define BREAK break
  ...
  for (;;) {
	...
	case 255:
      ...
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_sts) : sts (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_stsu)
      ...
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ste) : ste (get_op (BC_op1 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_steu) : steu (get_op (BC_op1 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ldnil) : ldnil (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldthis) : ldthis (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldch) : ldch (get_op (BC_op1 (cpc)), BC_op2 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldi) : ldi (get_op (BC_op1 (cpc)), BC_op2 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldbi) : ldbi (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldl) : ldl (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldf) : ldf (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lds) : lds (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldus) : ldus (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ldtp) : ldtp (get_op (BC_op1 (cpc)), BC_op2 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_flat) : flat (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fld) : fld (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lfld) : lfld (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lfldv) : lfldv (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ovfld) : ovfld (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lovfld) : lovfld (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lovfldv) : lovfldv (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_brts)
        : if (brts (get_op (BC_op1 (cpc)), get_op (BC_res (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_brfs)
        : if (brfs (get_op (BC_op1 (cpc)), get_op (BC_res (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lconv) : lconv (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_in) : in (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_not) : not(get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_inot) : inot (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_bnot) : bnot (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ibnot) : ibnot (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fold_add)
        : CASE (BC_NM_fold_mult)
        ...
        : CASE (BC_NM_fold_xor)
        : CASE (BC_NM_fold_or)
        : CASE (BC_NM_fold_concat) : foldop (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_eq) : eq (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ieq) : ieq (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_eqi) : eqi (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ieqi) : ieqi (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ne) : ne (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ine) : ine (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_nei) : nei (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_inei) : inei (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_id) : id (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_unid)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lt) : lt (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ilt) : ilt (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lti) : lti (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ilti) : ilti (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ge) : ge (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ige) : ige (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_gei) : gei (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_igei) : igei (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_gt) : gt (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_igt) : igt (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_gti) : gti (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_igti) : igti (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_le) : le (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ile) : ile (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lei) : lei (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ilei) : ilei (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_plus) : plus (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_iplus) : iplus (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fplus) : fplus (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_minus) : minus (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_iminus) : iminus (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fminus) : fminus (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_length) : length (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_const) : constop (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_new) : new (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_tpof) : tpof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chof) : chof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_iof) : iof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lof) : lof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fof) : fof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fmtvecof)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_vecof) : vecof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_tabof) : tabof (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_vec) : vec (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_tab) : tab (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ind) : ind (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ind2)
        : ind2 (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_sl) : sl (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lslv) : lslv (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_call) : call (get_op (BC_op1 (cpc)), BC_op2 (cpc), FALSE);
      BREAK;
      CASE (BC_NM_tcall) : tcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), FALSE);
      BREAK;
      CASE (BC_NM_icall)
        : icall (get_op (BC_op1 (cpc)), BC_op2 (cpc), BC_vars_num (BC_cfblock (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_itcall)
        : itcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), BC_vars_num (BC_cfblock (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_cicall)
        : cicall (get_op (BC_op1 (cpc)), BC_op2 (cpc), BC_vars_num (BC_cfblock (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_citcall)
        : citcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), BC_vars_num (BC_cfblock (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_ticall)
        : ticall (get_op (BC_op1 (cpc)), BC_op2 (cpc), BC_vars_num (BC_cfblock (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_titcall)
        : titcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), BC_vars_num (BC_cfblock (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_ibcall) : ibcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), FALSE);
      BREAK;
      CASE (BC_NM_mcall)
        : mcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), get_op (BC_op3 (cpc)), BC_op4 (cpc), FALSE);
      BREAK;
      CASE (BC_NM_omcall)
        : omcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), get_op (BC_op3 (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_omicall)
        : omicall (get_op (BC_op1 (cpc)), BC_op2 (cpc), get_op (BC_op3 (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_omitcall)
        : omitcall (get_op (BC_op1 (cpc)), BC_op2 (cpc), get_op (BC_op3 (cpc)), FALSE);
      BREAK;
      CASE (BC_NM_add) : add (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_addi) : addi (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_sub) : sub (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_mult)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_multi) : multi (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_div)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_mod) : mod (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_madd)
        : madd (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_concat)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lsh) : lsh (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_rsh) : rsh (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ash) : ash (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_and) : and(get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_xor) : xor(get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_or) : or (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_iadd)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_iaddi) : iaddi (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_faddi) : faddi (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_isub)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_imulti) : imulti (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fmulti) : fmulti (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_idiv)
      ...
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ior) : ior (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_add_st)
        : add_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_sub_st)
        : sub_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_mult_st)
        : mult_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                   get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_div_st)
        : div_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_mod_st)
        : mod_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_concat_st)
        : concat_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                     get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_lsh_st)
        : lsh_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_rsh_st)
        : rsh_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ash_st)
        : ash_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_and_st)
        : and_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_xor_st)
        : xor_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                  get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_or_st)
        : or_st (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)),
                 get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_mult_slst)
        ...
        : CASE (BC_NM_xor_slst)
        : CASE (BC_NM_or_slst)
        : op_slst (get_op (BC_op1 (cpc)), BC_op2 (cpc), get_op (BC_op3 (cpc)),
                   get_op (BC_op4 (cpc)));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_slst) : slst (get_op (BC_op1 (cpc)), BC_op2 (cpc), get_op (BC_op3 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_b) : b ();
      INCREMENT_PC ();
      ...
      CASE (BC_NM_btdef)
        : /* Branch if defined  */
          if (btdef (get_op (BC_op1 (cpc)))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_bf) : if (bf (get_op (BC_op1 (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibf) : if (ibf (get_op (BC_op1 (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_bfni) : if (bfni (get_op (BC_op1 (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ibfni) : if (ibfni (get_op (BC_op1 (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_bteqinc)
      ...
      BREAK;
      CASE (BC_NM_bteq)
        : if (bteq (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btne)
        : if (btne (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btge)
        : if (btge (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btlt)
        : if (btlt (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btle)
        : if (btle (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btgt)
        : if (btgt (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibteq)
        : if (ibteq (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtne)
        : if (ibtne (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtge)
        : if (ibtge (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtlt)
        : if (ibtlt (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtle)
        : if (ibtle (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtgt)
        : if (ibtgt (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_fbteq)
        : if (fbteq (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_fbtne)
        : if (fbtne (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_fbtge)
        : if (fbtge (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_fbtlt)
        : if (fbtlt (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_fbtle)
        : if (fbtle (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_fbtgt)
        : if (fbtgt (get_op (BC_op1 (cpc)), get_op (BC_bcmp_op2 (cpc)), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_bteqi)
        : if (bteqi (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btnei)
        : if (btnei (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btlti)
        : if (btlti (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btlei)
        : if (btlei (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btgti)
        : if (btgti (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_btgei)
        : if (btgei (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibteqi)
        : if (ibteqi (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtnei)
        : if (ibtnei (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtlti)
        : if (ibtlti (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtlei)
        : if (ibtlei (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtgti)
        : if (ibtgti (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibtgei)
        : if (ibtgei (get_op (BC_op1 (cpc)), BC_bcmp_op2 (cpc), BC_bcmp_res (cpc))) cpc
          = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_bt) : if (bt (get_op (BC_op1 (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_ibt) : if (ibt (get_op (BC_op1 (cpc)))) cpc = BC_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_foreach)
        : if (foreach (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), get_op (BC_op3 (cpc)))) cpc
          = BC_body_pc (cpc);
      else INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      ...
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_bend) : if (bend ()) return;
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_stpop) : stpop (BC_op1 (cpc), BC_op2 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_leave) : if (leave ()) return;
      BREAK;
      CASE (BC_NM_fbend) : if (fbend ()) return;
      BREAK;
      CASE (BC_NM_ret) : if (ret (get_op (BC_op1 (cpc)))) return;
      BREAK;
      CASE (BC_NM_waitcond) : waitcond (get_op (BC_op1 (cpc)));
      BREAK;
      CASE (BC_NM_waitend) : waitend ();
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_stinc) : stinc (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)), BC_op3 (cpc));
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_stdecm) : stdecm (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_stdecu) : stdecu (get_op (BC_op1 (cpc)), BC_op2 (cpc));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_block) : block ();
      INCREMENT_PC ();
      INTERRUPT_CHECK;
      BREAK;
      CASE (BC_NM_throw) : if (throw (get_op (BC_op1 (cpc)))) return;
      BREAK;
      CASE (BC_NM_except) : if (except ()) return;
      BREAK;
      CASE (BC_NM_chvec)
        : if (chvec (get_op (BC_op1 (cpc)), BC_fail_pc (cpc))) cpc = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chvend)
        : if (chvend (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chvlen)
        : if (chvlen (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), get_op (BC_ch_op3 (cpc)),
                      BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chvel)
        : if (chvel (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), get_op (BC_ch_op3 (cpc)),
                     get_op (BC_ch_op4 (cpc)), BC_ch_op5 (cpc), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chtab)
        : if (chtab (get_op (BC_op1 (cpc)), BC_fail_pc (cpc))) cpc = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chtend)
        : if (chtend (get_op (BC_op1 (cpc)), BC_ch_op2 (cpc), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chtel)
        : if (chtel (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), get_op (BC_ch_op3 (cpc)),
                     BC_ch_op4 (cpc), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chst)
        : if (chst (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chstend)
        : if (chstend (get_op (BC_op1 (cpc)), BC_ch_op2 (cpc), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_chstel)
        : if (chstel (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), get_op (BC_ch_op3 (cpc)),
                      BC_ch_op4 (cpc), BC_ch_op5 (cpc), BC_fail_pc (cpc))) cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_rmatch)
        : if (rmatch (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)), get_op (BC_ch_op3 (cpc))))
            cpc
          = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_rmatchs)
        : if (rmatchs (get_op (BC_op1 (cpc)), get_op (BC_ch_op2 (cpc)))) cpc = BC_fail_pc (cpc);
      else INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_move) : move (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_imove) : imove (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fmove) : fmove (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_var) : var (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lvar) : lvar (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_lvarv) : lvarv (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_evar) : evar (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_levar) : levar (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_levarv) : levarv (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_efun) : efun (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_fun) : CASE (BC_NM_class) : funclass (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_ofun) : ofun (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_oclass) : oclass (get_op (BC_op1 (cpc)), get_op (BC_op2 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_rpr) : rpr (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_rpr_def) : rpr_def (get_op (BC_op1 (cpc)));
      INCREMENT_PC ();
      BREAK;
      CASE (BC_NM_compile) : compile ();
      if (!compile_c_code (cpc, store_handle))
        eval_error (compile_bc_decl, get_cpos (), DERR_C_code_compilation_failure);
      INCREMENT_PC ();
      BREAK;
...
void eval_error (BC_node_t except_class_block, position_t position, const char *format, ...) {
...
void call_fun_class (BC_node_t code, ER_node_t context, int pars_number, int from_c_code_p) {
  pc_t saved_cpc;
  pc_t saved_next_pc;
  ...
  if (BC_implementation_fun (code) != NULL)
    ...
    process_imm_fun_call ((val_t *) IVAL (ctop, 1), code, context, pars_number, BC_vars_num (code),
                          FALSE, from_c_code_p);
  }
  for (;;) {
    if (cpc != NULL) evaluate_code ();
...
void evaluate_program (pc_t start_pc, int init_p, int last_p) {
  ...
  if (!init_p)
    ...
    if (profile_flag) {
      ...
      if (setitimer (ITIMER_VIRTUAL, &itimer, NULL)) abort ();
#endif
    }
#endif
  }
...
static int profile_compare_function (const void *el1, const void *el2) {
...
void print_profile (BC_node_t block) {
  ...
  VARR_CREATE (BC_node_t, profile_funcs, 0);
  collect_profile (block);
  qsort (VARR_ADDR (BC_node_t, profile_funcs), VARR_LENGTH (BC_node_t, profile_funcs),
         sizeof (BC_node_t), profile_compare_function);
  fprintf (stderr,
#if !HAVE_SETITIMER
           "\n** Calls ** In Time ** Name **************************************\n"
#else
  ...
  for (size_t i = 0; i < VARR_LENGTH (BC_node_t, profile_funcs); i++) {
    BC_node_t fun = VARR_GET (BC_node_t, profile_funcs, i), fdecl = BC_fdecl (fun);

#if !HAVE_SETITIMER
    fun_time = active_time (BC_exec_time (fun));
    ...
    fun_time = (all_interrupts_number == 0
                  ? 0.0
                  : (all_time * BC_interrupts_number (fun) / all_interrupts_number));
    if (gc_interrupts_number != 0 && gc_interrupts_number > BC_interrupts_number (fun)) {
      fprintf (stderr, "%8u %8.2f  --  * garbage collection *\n", gc_number,
...
#endif
    fprintf (stderr, "%8d %8.2f  --  %s: \"%s\": %d\n", BC_calls_number (fun), fun_time,
             BC_ident (fdecl), BC_pos (fdecl).file_name, BC_pos (fdecl).line_number);
  }
#if !HAVE_SETITIMER
  ...
  VARR_DESTROY (BC_node_t, profile_funcs);
}
#endif
