...
VARR (ER_node_t) * heap_temp_refs;

/* Temporary variable length objects. */
VARR (char) * temp_varr, *temp_varr2;

typedef void *void_ptr_t;
DEF_VARR (void_ptr_t);

VARR (void_ptr_t) * dl_handle_varr;
...
void initiate_int_tables (void) {
  int i;
  ER_node_t v = NULL;

  for (i = 0; i <= ER_NM__error; i++) type_size_table[i] = 0;
  ...
  type_size_table[ER_NM_tab] = type_size_table[ER_NM_thread] = type_size_table[ER_NM_stack]
    = sizeof (ER_node_t);
  ...
  for (i = 0; i <= ER_NM__error; i++) val_displ_table[i] = 0;
  val_displ_table[ER_NM_undef] = 0;
  val_displ_table[ER_NM_nil] = 0;
  val_displ_table[ER_NM_hide] = (char *) &((_ER_hide *) v)->hide - (char *) v;
  val_displ_table[ER_NM_hideblock] = (char *) &((_ER_hideblock *) v)->hideblock - (char *) v;
  ...
  val_displ_table[ER_NM_thread] = (char *) &((_ER_thread *) v)->thread - (char *) v;
  val_displ_table[ER_NM_code] = 0;
  val_displ_table[ER_NM_efun] = (char *) &((_ER_efun *) v)->efdecl - (char *) v;
  val_displ_table[ER_NM_stack] = (char *) &((_ER_stack *) v)->stack - (char *) v;
}

/* See definitions of the corresponding nodes. */
static size_t do_always_inline val_displ (ER_node_t var) {
  d_assert (ER_NODE_MODE (var) != ER_NM_byte);
  return val_displ_table[ER_NODE_MODE (var)];
}

static int do_inline in_heap_temp_refs (ER_node_t obj) {
  int i;

  for (i = 0; i < TEMP_REFS_LENGTH (); i++)
    if (GET_TEMP_REF (i) == obj) return TRUE;
  return FALSE;
}
...
char *_pack_els (ER_node_t vect) {
  ...
  res = ((char *) vect + ALLOC_SIZE (sizeof (_ER_heap_pack_vect)) + ER_disp (vect));
  return res;
}
...
ER_node_t _unpack_els (ER_node_t vect) {
  ...
  res = (ER_node_t) ((char *) vect + ALLOC_SIZE (sizeof (_ER_heap_unpack_vect)) + ER_disp (vect));
  return res;
}
...
entry_ptr_t _tab_entries (ER_node_t tab) {
  entry_ptr_t res;
...
ER_node_t _stack_vars (ER_node_t stack) {
  ...
  res = (ER_node_t) ((char *) stack + ALLOC_SIZE (sizeof (_ER_heap_stack)));
  return res;
}

BC_node_t _stack_block (ER_node_t stack) {
  d_assert (ER_NODE_MODE (stack) == ER_NM_heap_stack);
  return ER_block_node (stack);
}

stack_ptr_t _stacks_table (ER_node_t thread) {
  stack_ptr_t res;

  d_assert (ER_NODE_MODE (thread) == ER_NM_heap_thread);
  res = (stack_ptr_t) ((char *) thread + ALLOC_SIZE (sizeof (_ER_heap_thread)));
  return res;
}
...
ER_node_t _indexed_el_key (ER_node_t first_el, int index) {
  ER_node_t res;
...
ER_node_t _indexed_el_val (ER_node_t first_el, int index) {
  ER_node_t res;

  res = (ER_node_t) ((char *) first_el + (2 * index + 1) * (int) sizeof (val_t));
  return res;
}
...
int eq_val (val_t *val1_ptr, val_t *val2_ptr, size_t number) {
  ...
  for (i = 0; i < number; i++)
    if (ER_NODE_MODE ((ER_node_t) &val1_ptr[i]) != ER_NODE_MODE ((ER_node_t) &val2_ptr[i]))
      return FALSE;
    else {
      displ = val_displ ((ER_node_t) &val1_ptr[i]);
      if (memcmp ((char *) &val1_ptr[i] + displ, (char *) &val2_ptr[i] + displ,
                  type_size_table[ER_NODE_MODE ((ER_node_t) &val1_ptr[i])])
          != 0)
        return FALSE;
    }
  return TRUE;
...
static size_t do_always_inline vars_stack_size (int nvars) {
  return nvars * sizeof (val_t) + ALLOC_SIZE (sizeof (_ER_heap_stack));
}

/* Minimal stack size of BLOCK_NODE.  */
static size_t do_always_inline shrink_block_stack_size (BC_node_t block_node) {
  d_assert (block_node != NULL && BC_IS_OF_TYPE (block_node, BC_NM_block));
  return vars_stack_size (BC_vars_num (block_node));
}

/* The func returns size (in bytes) of the stack of the block node given
   as BLOCK_NODE. */
static size_t do_always_inline new_block_stack_size (BC_node_t block_node) {
  d_assert (block_node != NULL && BC_IS_OF_TYPE (block_node, BC_NM_block));
  return (vars_stack_size (BC_vars_num (block_node)) + BC_tvars_num (block_node) * sizeof (val_t));
}

/* The func returns size (in bytes) of the STACK. */
size_t do_always_inline stack_size (ER_node_t stack) {
  d_assert (ER_NODE_MODE (stack) == ER_NM_heap_stack);
  return (ER_all_block_vars_num (stack) * sizeof (val_t) + ALLOC_SIZE (sizeof (_ER_heap_stack)));
}

ER_node_t create_gmp (void) {
  ER_node_t gmp;
...
ER_node_t copy_gmp (ER_node_t gmp) {
...
DEF_VARR (heap_chunk_t);

/* Array of heap descriptors. */
static VARR (heap_chunk_t) * heap_chunks;

static struct heap_chunk *curr_heap_chunk;

/* The following VARR contains registered external variable
   addresses. */
static VARR (void_ptr_t) * external_vars;
...
static void new_heap_chunk (size_t size) {
  char *str;
  size_t old_size;
  static const size_t max_default_chunk_size = 8 * 1024 * 1024;

  VARR_GROW (heap_chunk_t, heap_chunks, 1);
  curr_heap_chunk
    = &VARR_ADDR (heap_chunk_t, heap_chunks)[VARR_LENGTH (heap_chunk_t, heap_chunks) - 1];
  old_size = size;
  ...
  heap_chunks_number++;
  if (max_heap_chunks_number < heap_chunks_number) max_heap_chunks_number = heap_chunks_number;
}

static void del_heap_chunks (int number_saved_chunks) {
  int i;

  for (i = number_saved_chunks; i < VARR_LENGTH (heap_chunk_t, heap_chunks); i++) {
    curr_heap_chunk = &VARR_ADDR (heap_chunk_t, heap_chunks)[i];
    FREE (curr_heap_chunk->chunk_start);
    heap_chunks_number--;
    heap_size -= curr_heap_chunk->chunk_bound - curr_heap_chunk->chunk_start;
  }
  VARR_TRUNC (heap_chunk_t, heap_chunks, number_saved_chunks);
  curr_heap_chunk = &VARR_ADDR (heap_chunk_t, heap_chunks)[number_saved_chunks - 1];
}
...
void initiate_heap () {
  VARR_CREATE (heap_chunk_t, heap_chunks, 0);
  VARR_CREATE (void_ptr_t, external_vars, 0);
  ...
  uppest_stack = NULL;
  CREATE_TEMP_REF ();
  VARR_CREATE (char, temp_varr, 256);
  VARR_CREATE (char, temp_varr2, 256);
  VARR_CREATE (void_ptr_t, dl_handle_varr, 0);
  gc_number = 0;
...
void final_call_destroy_functions (void) {
  BC_node_t block_node;

  if (uppest_stack == NULL) /* Heap is not initialized.  */
    return;
  ...
  if (mark_stacks_need_destroying (FALSE)) {
    ...
    if (cstack == uppest_stack) {
      ...
      ctop
        = (ER_node_t) ((char *) cvars + BC_vars_num (block_node) * sizeof (val_t) - sizeof (val_t));
      cpc = BC_next (ER_call_pc (cstack));
      if (cthread != NULL) ER_set_saved_cstack (cthread, cstack);
    }
    destroy_stacks ();
  }
}

void finish_heap (void) {
  struct heap_chunk *start;

  for (curr_heap_chunk = start = VARR_ADDR (heap_chunk_t, heap_chunks);
       curr_heap_chunk - start < VARR_LENGTH (heap_chunk_t, heap_chunks); curr_heap_chunk++)
    FREE (curr_heap_chunk->chunk_start);
  VARR_DESTROY (heap_chunk_t, heap_chunks);
  VARR_DESTROY (void_ptr_t, external_vars);
  FINISH_TEMP_REF ();
  VARR_DESTROY (void_ptr_t, dl_handle_varr);
  VARR_DESTROY (char, temp_varr2);
  VARR_DESTROY (char, temp_varr);
}
...
void *heap_allocate (size_t size, int stack_p) {
  void *result;
...
static size_t do_inline heap_object_size (ER_node_t obj) {
  size_t size = 1;
  ...
  case ER_NM_heap_tab:
  case ER_NM_heap_redir: size = ER_allocated_length (obj); break;
  case ER_NM_heap_stack: size = stack_size (obj); break;
  case ER_NM_heap_thread: size = ALLOC_SIZE (sizeof (_ER_heap_thread)); break;
  case ER_NM_heap_hideblock:
    size = (ALLOC_SIZE (sizeof (_ER_heap_hideblock)) + ER_hideblock_length (obj));
    break;
  case ER_NM_heap_gmp: size = ALLOC_SIZE (sizeof (_ER_heap_gmp)); break;
  default: d_unreachable ();
  }
  return ALLOC_SIZE (size);
}

static void do_always_inline try_heap_stack_free (void *from, size_t size) {
...
static void do_always_inline shrink_stack (ER_node_t stack) {
  size_t size;
  BC_node_t block = ER_block_node (stack);

  d_assert (BC_ext_life_p (block));
  size = heap_object_size (stack);
  if (curr_heap_chunk->chunk_free != (char *) stack + size) /* Mark it for shrinkage during GC.  */
  ...
  ER_set_ctop (stack, (char *) ((val_t *) ER_stack_vars (stack) + BC_vars_num (block) - 1));
}

static int do_always_inline stack_with_destroy (ER_node_t obj) {
  BC_node_t decl;

  if (ER_NODE_MODE (obj) != ER_NM_heap_stack) return FALSE;
...
static int do_always_inline stack_for_shrink_p (ER_node_t stack) {
  return (ER_NODE_MODE (stack) == ER_NM_heap_stack && ER_prev_stack (stack) == NULL
          && BC_ext_life_p (ER_block_node (stack)));
}

static size_t tailored_heap_object_size (ER_node_t obj) {
  ...
  if (node_mode == ER_NM_heap_pack_vect || node_mode == ER_NM_heap_unpack_vect) {
    ...
    el_size = (node_mode == ER_NM_heap_pack_vect ? type_size_table[ER_pack_vect_el_mode (obj)]
                                                 : sizeof (val_t));
    ...
    if (size > optimal_size) size = optimal_size;
...
static ER_node_t do_always_inline next_heap_object (ER_node_t obj) {
...
static void clean_heap_object_process_flag (void) {
  ER_node_t curr_obj;
  struct heap_chunk *curr_descr, *start;

  for (curr_descr = start = VARR_ADDR (heap_chunk_t, heap_chunks);
       curr_descr - start < VARR_LENGTH (heap_chunk_t, heap_chunks); curr_descr++) {
    for (curr_obj = (ER_node_t) curr_descr->chunk_start; (char *) curr_obj < curr_descr->chunk_free;
    ...
    for (curr_obj = (ER_node_t) curr_descr->chunk_stack_top;
         (char *) curr_obj < curr_descr->chunk_bound; curr_obj = next_heap_object (curr_obj))
      ER_set_it_was_processed (curr_obj, FALSE);
  }
}
...
static void traverse_used_var (ER_node_t var) {
  ...
  case ER_NM_int:
  case ER_NM_float:
  case ER_NM_type: return;
  case ER_NM_long: traverse_used_heap_object (ER_l (var)); return;
  case ER_NM_hideblock: traverse_used_heap_object (ER_hideblock (var)); return;
  case ER_NM_vect: traverse_used_heap_object (ER_vect (var)); return;
  case ER_NM_tab: traverse_used_heap_object (ER_tab (var)); return;
  case ER_NM_code: traverse_used_heap_object (ER_code_context (var)); return;
  case ER_NM_efun: return;
  case ER_NM_thread: traverse_used_heap_object (ER_thread (var)); return;
  case ER_NM_stack: traverse_used_heap_object (ER_stack (var)); return;
  case ER_NM_external_var_ref: return;
  default: d_unreachable ();
  }
}
...
static void traverse_used_heap_object (ER_node_t obj) {
  size_t i;

  if (obj == NULL || ER_it_was_processed (obj)) return;
  ER_set_it_was_processed (obj, TRUE);
  switch (ER_NODE_MODE (obj)) {
  case ER_NM_heap_hideblock:
  case ER_NM_heap_gmp: return;
  case ER_NM_heap_pack_vect: {
    ER_node_mode_t el_type = ER_pack_vect_el_mode (obj);
    size_t el_size;

    if (el_type == ER_NM_undef || el_type == ER_NM_nil || el_type == ER_NM_hide
        || el_type == ER_NM_char || el_type == ER_NM_byte || el_type == ER_NM_int
        || el_type == ER_NM_float || el_type == ER_NM_type)
      return;
    el_size = type_size_table[el_type];
    if (el_type == ER_NM_hideblock || el_type == ER_NM_long || el_type == ER_NM_vect
        || el_type == ER_NM_tab || el_type == ER_NM_thread || el_type == ER_NM_stack)
      for (i = 0; i < ER_els_number (obj); i++)
        traverse_used_heap_object (*(ER_node_t *) (ER_pack_els (obj) + i * el_size));
    else if (el_type == ER_NM_code)
      for (i = 0; i < ER_els_number (obj); i++)
        traverse_used_heap_object (((_ER_code *) (ER_pack_els (obj) + i * el_size))->code_context);
  ...
  case ER_NM_heap_unpack_vect:
    for (i = 0; i < ER_els_number (obj); i++) traverse_used_var (IVAL (ER_unpack_els (obj), i));
    return;
  case ER_NM_heap_tab: {
    ER_node_mode_t mode;
    ...
    for (i = 0; i < ER_els_bound (obj); i++) {
      ...
      if (mode == ER_NM_empty_el) continue;
      traverse_used_var (INDEXED_EL_KEY (ER_tab_els (obj), i));
      traverse_used_var (INDEXED_EL_VAL (ER_tab_els (obj), i));
    }
  }
    return;
  case ER_NM_heap_redir: traverse_used_heap_object (ER_redir (obj)); return;
  case ER_NM_heap_stack: {
    ...
    for (var = ER_stack_vars (obj); /* !!! */
         (char *) var <= ER_ctop (obj); var = IVAL (var, 1))
  ...
  if (mark_dependent_p && result)
    for (curr_descr = start = VARR_ADDR (heap_chunk_t, heap_chunks);
      ...
      for (curr_obj = (ER_node_t) curr_descr->chunk_start;
           (char *) curr_obj < curr_descr->chunk_free; curr_obj = next_heap_object (curr_obj))
        if (ER_state (curr_obj) == IS_to_be_destroyed) traverse_used_heap_object (curr_obj);
    }
  return result;
}

static int mark_used_heap_objects (void) {
  size_t i;

  clean_heap_object_process_flag ();
  traverse_used_heap_object (cstack);
  traverse_used_heap_object (uppest_stack);
  for (i = 0; i < VARR_LENGTH (BC_node_t, block_tab); i++)
    BC_set_free_stacks (VARR_GET (BC_node_t, block_tab, i), NULL);
  for (i = 0; i < TEMP_REFS_LENGTH (); i++) traverse_used_heap_object (GET_TEMP_REF (i));
  for (i = 0; i < VARR_LENGTH (void_ptr_t, external_vars); i++)
    traverse_used_var ((ER_node_t) VARR_GET (void_ptr_t, external_vars, i));
...
static do_inline char *define_new_heap_object (ER_node_t obj, struct heap_chunk **descr,
                                               char *place) {
  size_t size;
  ...
  if (ER_NODE_MODE (obj) == ER_NM_heap_redir) {
    ...
    if (!temp_refs_p) try_full_pack (obj);
    size = (!temp_refs_p ? tailored_heap_object_size (obj) : heap_object_size (obj));
...
static int define_new_heap_object_places (void) {
  ER_node_t curr_obj;
  struct heap_chunk *curr_descr, *start;
  ...
  curr_place_descr = VARR_ADDR (heap_chunk_t, heap_chunks);
  new_place = curr_place_descr->chunk_start;
  for (curr_descr = start = VARR_ADDR (heap_chunk_t, heap_chunks);
       curr_descr - start < VARR_LENGTH (heap_chunk_t, heap_chunks); curr_descr++) {
    for (curr_obj = (ER_node_t) curr_descr->chunk_start; (char *) curr_obj < curr_descr->chunk_free;
         curr_obj = next_heap_object (curr_obj))
      new_place = define_new_heap_object (curr_obj, &curr_place_descr, new_place);
    for (curr_obj = (ER_node_t) curr_descr->chunk_stack_top;
         (char *) curr_obj < curr_descr->chunk_bound; curr_obj = next_heap_object (curr_obj))
      new_place = define_new_heap_object (curr_obj, &curr_place_descr, new_place);
  }
...
static void do_inline change_val (ER_node_mode_t mode, ER_node_t *val_addr) {
  switch (mode) {
  case ER_NM_undef:
  case ER_NM_nil:
  case ER_NM_hide:
  ...
  case ER_NM_efun: return;
  case ER_NM_vect:
  case ER_NM_tab: CHANGE_VECT_TAB_REF (*val_addr); return;
  case ER_NM_long:
  case ER_NM_hideblock:
  case ER_NM_thread:
  case ER_NM_stack:
  case ER_NM_code: CHANGE_REF (*val_addr); return;
  case ER_NM_external_var_ref: return;
  default: d_unreachable ();
  }
}
...
/* The function changes variable VAR value if it refers to a heap
   ...
   compaction. */
static void change_var (ER_node_t var) {
  switch (ER_NODE_MODE (var)) {
  case ER_NM_undef:
  case ER_NM_nil:
  case ER_NM_hide:
  ...
  case ER_NM_type:
  case ER_NM_efun: return;
  case ER_NM_vect: CHANGE_VECT_TAB_REF (((_ER_vect *) var)->vect); return;
  case ER_NM_tab: CHANGE_VECT_TAB_REF (((_ER_tab *) var)->tab); return;
  case ER_NM_long: CHANGE_REF (((_ER_long *) var)->l); return;
  case ER_NM_hideblock: CHANGE_REF (((_ER_hideblock *) var)->hideblock); return;
  case ER_NM_code: CHANGE_REF (((_ER_code *) var)->code_context); return;
  case ER_NM_thread: CHANGE_REF (((_ER_thread *) var)->thread); return;
  case ER_NM_stack: CHANGE_REF (((_ER_stack *) var)->stack); return;
  case ER_NM_external_var_ref: return;
  default: d_unreachable ();
  }
}

static void do_inline change_obj_refs (ER_node_t obj) {
  size_t i;

  if (ER_it_was_processed (obj)) switch (ER_NODE_MODE (obj)) {
    case ER_NM_heap_pack_vect: {
      ...
      if (el_type == ER_NM_hideblock || el_type == ER_NM_long || el_type == ER_NM_vect
          || el_type == ER_NM_tab || el_type == ER_NM_thread || el_type == ER_NM_stack)
        for (i = 0; i < ER_els_number (obj); i++)
          change_val (el_type, (ER_node_t *) (ER_pack_els (obj) + i * el_size));
      else if (el_type == ER_NM_code)
        for (i = 0; i < ER_els_number (obj); i++)
          change_val (el_type, &((_ER_code *) (ER_pack_els (obj) + i * el_size))->code_context);
      else if (el_type == ER_NM_efun)
        ;
      else
        d_unreachable ();
    ...
    case ER_NM_heap_unpack_vect:
      for (i = 0; i < ER_els_number (obj); i++) change_var (IVAL (ER_unpack_els (obj), i));
      break;
    case ER_NM_heap_tab: {
      ER_node_mode_t el_type;
      ...
      for (i = 0; i < ER_els_bound (obj); i++) {
        ...
        if (el_type == ER_NM_empty_el) continue;
    ...
    case ER_NM_heap_stack: {
      ...
      CHANGE_REF (((_ER_heap_stack *) obj)->_ER_M_context_heap_obj.context);
      CHANGE_REF (((_ER_heap_stack *) obj)->prev_stack);
      for (var = ER_stack_vars (obj); (char *) var <= ER_ctop (obj); var = IVAL (var, 1))
        change_var (var);
      diff = ((char *) ER_ctop (obj) - (char *) ER_stack_vars (obj));
      ((_ER_heap_stack *) obj)->ctop
        = (ER_new_place (obj) + ALLOC_SIZE (sizeof (_ER_heap_stack)) + diff);
      break;
    }
    case ER_NM_heap_thread:
      ...
      break;
    case ER_NM_heap_gmp:
    case ER_NM_heap_hideblock: break;
    default:
      /* Redirections do not survive GC. */
      d_unreachable ();
    }
...
static void change_refs (void) {
  ...
  for (i = 0; i < VARR_LENGTH (BC_node_t, block_tab); i++) {
    block = VARR_GET (BC_node_t, block_tab, i);
    curr_obj = BC_free_stacks (block);
    CHANGE_REF (curr_obj);
    BC_set_free_stacks (block, curr_obj);
  }
  /* `heap_temp_refs' may refer for a vector. */
  for (i = 0; i < TEMP_REFS_LENGTH (); i++) CHANGE_VECT_TAB_REF (GET_TEMP_REF (i));
  for (i = 0; i < VARR_LENGTH (void_ptr_t, external_vars); i++)
    change_var ((ER_node_t) VARR_GET (void_ptr_t, external_vars, i));
  CHANGE_REF (cthread);
  CHANGE_REF (first_thread_not_started);
}

static do_inline char *move_or_destroy_object (ER_node_t obj, struct heap_chunk **descr,
                                               size_t *curr_heap_size, char *place) {
  size_t tailored_size;

  if (!ER_it_was_processed (obj)) {
    if (ER_NODE_MODE (obj) == ER_NM_heap_gmp) mpz_clear (*ER_mpz_ptr (obj));
  } else {
    ...
    tailored_size = (!in_heap_temp_refs ((ER_node_t) place) ? tailored_heap_object_size (obj)
    ...
    if (place + tailored_size > (*descr)->chunk_bound) {
      ...
      if ((*descr)->chunk_free > place) memset (place, 0, (*descr)->chunk_free - place);
      if ((*descr)->chunk_stack_top > place)
        memset ((*descr)->chunk_stack_top, 0, (*descr)->chunk_bound - (*descr)->chunk_stack_top);
      else
    ...
    if (ER_NODE_MODE (obj) == ER_NM_heap_pack_vect || ER_NODE_MODE (obj) == ER_NM_heap_unpack_vect)
    ...
    if (place != (char *) obj) memmove (place, obj, tailored_size);
    place += tailored_size;
    d_assert (place <= (*descr)->chunk_bound);
  }
  return place;
...
static void compact_heap (void) {
  ...
  new_place = curr_place_descr->chunk_start;
  free_heap_memory = 0;
  curr_heap_size = (curr_place_descr->chunk_bound - curr_place_descr->chunk_start);
  start = VARR_ADDR (heap_chunk_t, heap_chunks);
  for (curr_descr = start; curr_descr - start < VARR_LENGTH (heap_chunk_t, heap_chunks);
       curr_descr++) {
    for (curr_obj = (ER_node_t) curr_descr->chunk_start; (char *) curr_obj < curr_descr->chunk_free;
         curr_obj = next_obj) {
      next_obj = next_heap_object (curr_obj);
      new_place = move_or_destroy_object (curr_obj, &curr_place_descr, &curr_heap_size, new_place);
    }
    /* Put stacks into another part of the heap.  Such cases should
       be rare.  */
    for (curr_obj = (ER_node_t) curr_descr->chunk_stack_top;
         (char *) curr_obj < curr_descr->chunk_bound; curr_obj = next_obj) {
      next_obj = next_heap_object (curr_obj);
      new_place = move_or_destroy_object (curr_obj, &curr_place_descr, &curr_heap_size, new_place);
    }
  }
  curr_heap_chunk = curr_place_descr;
  d_assert (curr_heap_chunk + 1 < start + VARR_LENGTH (heap_chunk_t, heap_chunks)
            || curr_heap_chunk->chunk_free
  ...
  for (;;) {
    if (free_heap_memory >= curr_heap_size / 4) break;
    curr_heap_chunk++;
    if (curr_heap_chunk >= start + VARR_LENGTH (heap_chunk_t, heap_chunks)) {
      curr_heap_chunk--;
      break;
    }
    free_heap_memory += (curr_heap_chunk->chunk_bound - curr_heap_chunk->chunk_start);
    curr_heap_size += (curr_heap_chunk->chunk_bound - curr_heap_chunk->chunk_start);
  ...
  del_heap_chunks (curr_heap_chunk - (struct heap_chunk *) VARR_ADDR (heap_chunk_t, heap_chunks)
                   + 1);
}
...
static int compare_chunks (const void *chunk1, const void *chunk2) {
  ...
  if (c1->chunk_bound - c1->chunk_start > c2->chunk_bound - c2->chunk_start)
    return -1;
  else if (c1->chunk_bound - c1->chunk_start < c2->chunk_bound - c2->chunk_start)
    return 1;
  else
    return 0;
}
  ...
  for (curr_descr = start = VARR_ADDR (heap_chunk_t, heap_chunks);
    ...
    for (curr_obj = (ER_node_t) curr_descr->chunk_start; (char *) curr_obj < curr_descr->chunk_free;
      ...
      if (ER_NODE_MODE (curr_obj) == ER_NM_heap_stack
        ...
        decl = LV_BLOCK_IDENT_DECL (ER_block_node (curr_obj), DESTROY_FLDID_NUM);
        ...
        if (BC_next (fblock) != NULL)
          ...
          if (!flag)
            eval_error (abstrcall_bc_decl, BC_pos (decl), DERR_unfinished_fun_class_call,
                        BC_ident (decl));
        }
      }
  }
...
void GC (void) {
  ...
  if (profile_flag) ticker_on (&gc_ticker);
#endif
  in_gc_p = TRUE;
...
#endif
  qsort (VARR_ADDR (heap_chunk_t, heap_chunks), VARR_LENGTH (heap_chunk_t, heap_chunks),
  ...
  free_gc_memory_percent
    = (free_gc_memory_percent * (gc_number - 1) + (100 * free_heap_memory) / heap_size) / gc_number;
  d_assert (free_gc_memory_percent < 100);
  in_gc_p = FALSE;
#if !defined(NO_PROFILE) && !HAVE_SETITIMER
  if (profile_flag) ticker_on (&gc_ticker);
#endif
  if (flag) destroy_stacks ();
  /* Switch to byte code execution as stacks might move.  */
  if (leave_c_stacks_p) switch_to_bcode ();
}
...
void profile_interrupt (void) {
  ...
  for (block = ER_block_node (cstack); block != NULL && BC_NODE_MODE (block) == BC_NM_block;
       block = BC_scope (block))
    ;
  if (block != NULL) BC_set_interrupts_number (block, BC_interrupts_number (block) + 1);
}
#endif

#ifndef NO_PROFILE
/* Update profile info when we use profiling.  */
void do_always_inline update_profile (BC_node_t block_node) {
...
ER_node_t do_always_inline alloc_new_stack (BC_node_t block_node, int vars_num) {
  ER_node_t stack;

  /* Remember about possible GC. */
  stack = ((ER_node_t) heap_allocate (new_block_stack_size (block_node), FALSE));
...
void heap_push (BC_node_t block_node, ER_node_t context, int offset) {
#ifndef NO_PROFILE
  if (profile_flag) update_profile (block_node);
#endif
  heap_push_without_profile_update (block_node, context, BC_vars_num (block_node), offset);
}
...
void heap_pop (void) {
...
#if !defined(NO_PROFILE) && !HAVE_SETITIMER
    if (profile_flag) ticker_off (&BC_exec_time (block_node));
#endif
  }
  ER_set_ctop (cstack, (char *) ctop);
  ...
  if (BC_ext_life_p (block_node))
    ...
    if (BC_NODE_MODE (block_node) == BC_NM_fblock && BC_pure_fun_p (block_node)) {
      /* Saving the result.  */
      ER_node_t res = IVAL (ER_stack_vars (stack), BC_pars_num (block_node));
...
void create_uppest_stack (BC_node_t block_node) {
...
void expand_uppest_stack (void) {
  ...
  stack = ((ER_node_t) heap_allocate (vars_stack_size (new_all_block_vars),
...
void clear_c_stack_flags (void) {
  ER_node_t thread, stack;

  if (generated_c_function_calls_num == 0) return;
  for (thread = cthread;;) {
    stack = thread == cthread ? cstack : ER_saved_cstack (thread);
    for (; stack != NULL; stack = ER_prev_stack (stack)) ER_set_c_stack_p (stack, FALSE);
    thread = ER_next (thread);
    if (thread == cthread) break;
  }
}
...
ER_node_t create_unpack_vector (size_t els_number) {
  ...
  allocated_length
    = (ALLOC_SIZE (sizeof (_ER_heap_unpack_vect)) + OPTIMAL_ELS_NUM (els_number) * sizeof (val_t));
...
ER_node_t create_pack_vector (size_t els_number, ER_node_mode_t eltype) {
  ...
  allocated_length = (ALLOC_SIZE (sizeof (_ER_heap_pack_vect))
                      + OPTIMAL_ELS_NUM (els_number) * type_size_table[eltype]);
...
ER_node_t expand_vector (ER_node_t vect, size_t els_number) {
  ...
  if (ER_NODE_MODE (vect) == ER_NM_heap_pack_vect
      && (ER_pack_vect_el_mode (vect) == ER_NM_char || ER_pack_vect_el_mode (vect) == ER_NM_byte)) {
  ...
  if (allocated_length < header_length + els_number * el_length)
    allocated_length = (header_length + OPTIMAL_ELS_NUM (els_number) * el_length);
  if (allocated_length != prev_vect_allocated_length) {
    ...
    if (disp == 0)
      ...
      ER_set_disp (vect, 0);
      memcpy (ER_NODE_MODE (vect) == ER_NM_heap_unpack_vect ? (char *) ER_unpack_els (vect)
                                                            : (char *) ER_pack_els (vect),
    ...
    /* Set it before getting els.  */
    ER_set_disp (vect, 0);
    memmove (ER_NODE_MODE (vect) == ER_NM_heap_unpack_vect ? (char *) ER_unpack_els (vect)
                                                           : (char *) ER_pack_els (vect),
...
ER_node_t unpack_vector (ER_node_t vect) {
  ...
  el_size = type_size_table[el_type];
  prev_vect = vect;
  if (allocated_length - disp
      < (ALLOC_SIZE (sizeof (_ER_heap_unpack_vect)) + els_number * sizeof (val_t))) {
  ...
  if (els_number != 0) {
    ...
    for (i = els_number - 1;; i--) {
      if (el_type == ER_NM_byte) {
        ...
        memcpy ((char *) IVAL (els, i) + displ, pack_els + i * el_size, el_size);
        ER_SET_MODE (IVAL (els, i), el_type);
        if (el_type == ER_NM_vect) ER_set_dim (IVAL (els, i), 0);
      }
      if (i == 0) break;
    }
  }
...
static void pack_vector_if_possible (ER_node_t unpack_vect) {
  ER_node_t pack_vect;
  size_t i;
  size_t allocated_length;
  int immutable;
  ...
  el_type = (els_number != 0 ? ER_NODE_MODE (IVAL (ER_unpack_els (unpack_vect), 0)) : ER_NM_nil);
  for (i = 0; i < els_number; i++)
    if (ER_NODE_MODE (IVAL (ER_unpack_els (unpack_vect), i)) != el_type) break;
  if (i >= els_number) {
    /* Pack it */
    el_size = type_size_table[el_type];
    ...
    for (i = 0; i < els_number; i++)
      memcpy (els + i * el_size, (char *) IVAL (ER_unpack_els (unpack_vect), i) + displ, el_size);
    if (el_type == ER_NM_char) ((ucode_t *) els)[els_number] = '\0';
    ER_SET_MODE (pack_vect, ER_NM_heap_pack_vect);
...
int eq_vector (ER_node_t v1, ER_node_t v2) {
  ...
  if (ER_els_number (v1) != ER_els_number (v2)) return FALSE;
  try_full_pack (v1);
  try_full_pack (v2);
  if (ER_NODE_MODE (v1) != ER_NODE_MODE (v2))
    ...
    return (ER_pack_vect_el_mode (v1) == ER_pack_vect_el_mode (v2)
            && memcmp (ER_pack_els (v1), ER_pack_els (v2),
                       ER_els_number (v1) * type_size_table[ER_pack_vect_el_mode (v1)])
                 == 0);
  else
    return eq_val ((val_t *) ER_unpack_els (v1), (val_t *) ER_unpack_els (v2), ER_els_number (v1));
}
...
ER_node_t create_empty_string (size_t min_length) {
  ER_node_t vect;
...
ER_node_t create_string (const char *string) {
  ER_node_t vect;
  size_t chars_number;
...
/* Create and return packed ucode vector from ucode string STR with
   trailing zero.  */
ER_node_t create_ucodestr (const ucode_t *str) {
  ER_node_t vect;
  size_t chars_number;
...
ER_node_t bytevect_to_ucodevect (ER_node_t vect) {
  size_t allocated_length;
  int immutable;
  size_t els_number;
  ER_node_t prev_vect;
  ...
  prev_vect = vect;
  if (allocated_length - disp
      < (ALLOC_SIZE (sizeof (_ER_heap_pack_vect)) + (els_number + 1) * sizeof (ucode_t))) {
    allocated_length = (ALLOC_SIZE (sizeof (_ER_heap_pack_vect))
                        + OPTIMAL_ELS_NUM (els_number) * sizeof (ucode_t));
    vect = heap_allocate (allocated_length, FALSE);
    ER_SET_MODE (vect, ER_NM_heap_pack_vect);
  }
  ER_set_disp (vect, 0);
...
/* Transform ucode packed vector VECT into byte packed vector if
   possible.  */
static void ucodevect_to_bytevect_if_possible (ER_node_t vect) {
  size_t i, els_num;
  ...
  for (i = 0; i < els_num; i++)
    if (!in_byte_range_p (char_els[i])) return;
  els = (byte_t *) ER_pack_els (vect) - ER_disp (vect);
  for (i = 0; i < els_num; i++) els[i] = char_els[i];
  els[els_num] = '\0';
...
/* Transform OBJ to packed byte vector if possible, otherwise to
   packed vector.  */
void try_full_pack (ER_node_t obj) {
  if (ER_NODE_MODE (obj) == ER_NM_heap_unpack_vect) pack_vector_if_possible (obj);
  if (ER_NODE_MODE (obj) == ER_NM_heap_pack_vect && ER_pack_vect_el_mode (obj) == ER_NM_char)
    ucodevect_to_bytevect_if_possible (obj);
}

int do_always_inline eq_stack (ER_node_t i1, ER_node_t i2) {
  return (i1 == i2
          || (ER_block_node (i1) == ER_block_node (i2)
              && ER_context (i1) == ER_context (i2) /* !!! */
              && eq_val ((val_t *) ER_stack_vars (i1), (val_t *) ER_stack_vars (i2),
...
int eq_table (ER_node_t t1, ER_node_t t2) {
  ...
  if (ER_els_number (t1) != ER_els_number (t2)) return FALSE;
...
static size_t do_always_inline murmur_init (size_t len) {
  return MURMUR_SEED ^ (len * MURMUR_MAGIC);
}

static size_t do_always_inline murmur_step (size_t h, size_t val) {
  val *= MURMUR_MAGIC;
...
static size_t do_always_inline murmur_finish (size_t h) {
  h ^= h >> MURMUR_SHIFT;
  h *= MURMUR_MAGIC;
  h ^= h >> MURMUR_SHIFT;
  return h;
...
static size_t murmur (const void *key, size_t len) {
  ...
  switch (len) {
#if SIZEOF_SIZE_T == 8
  case 7: h ^= ((size_t) data[6]) << 48;
  case 6: h ^= ((size_t) data[5]) << 40;
  case 5: h ^= ((size_t) data[4]) << 32;
  case 4: h ^= ((size_t) data[3]) << 24;
#endif
  case 3: h ^= ((size_t) data[2]) << 16;
  case 2: h ^= ((size_t) data[1]) << 8;
  case 1: h ^= ((size_t) data[0]); h *= MURMUR_MAGIC;
  }
...
static size_t do_always_inline hash_ref (ER_node_t ref) {
  if (ref == NULL) return 0;
  return ER_unique_number (ref);
}

/* Return hash value of integer I.  */
static size_t do_always_inline int_hash_val (rint_t i) { return (size_t) i; }

/* Return hash value of VAL.  */
static size_t hash_val (ER_node_t val) {
  size_t hash;
  size_t length;
  ...
  case ER_NM_nil: return 0;
  case ER_NM_char:
    /* See also special case (string) in eq_key.  */
    return (size_t) ER_ch (val);
  case ER_NM_int: return int_hash_val (ER_i (val));
  case ER_NM_float: {
    rfloat_t f;
  ...
  case ER_NM_type: return (size_t) ER_type (val);
  case ER_NM_hide: return (size_t) ER_hide (val);
  case ER_NM_long: return hash_mpz (*ER_mpz_ptr (ER_l (val)));
  case ER_NM_hideblock: return (size_t) ER_unique_number (ER_hideblock (val));
  case ER_NM_vect: return (size_t) ER_unique_number (ER_vect (val));
  case ER_NM_tab: return (size_t) ER_unique_number (ER_tab (val));
  case ER_NM_code:
    return murmur_finish (murmur_step (murmur_step (murmur_init (2), (size_t) ER_code_id (val)),
                                       (size_t) ER_unique_number (ER_code_context (val))));
  case ER_NM_efun: return (size_t) ER_efdecl (val);
  case ER_NM_thread: return (size_t) ER_unique_number (ER_thread (val));
  case ER_NM_stack: return (size_t) ER_unique_number (ER_stack (val));
  default: d_unreachable ();
  }
}

static size_t do_always_inline hash_key (ER_node_t key) {
  size_t hash, el_hash;
  size_t i;
  size_t length;
  unsigned char *string;
  ...
  case ER_NM_code:
  case ER_NM_efun:
  case ER_NM_thread: hash = hash_val (key); break;
  case ER_NM_vect: {
    ER_node_t vect;
    ...
    if (ER_NODE_MODE (ER_vect (key)) == ER_NM_heap_pack_vect) {
      ...
      hash = murmur (ER_pack_els (pv),
                     ER_els_number (pv) * type_size_table[ER_pack_vect_el_mode (pv)]);
    } else {
  ...
  case ER_NM_tab: {
    ...
    for (i = 0; i < ER_els_bound (tab); i++) {
      ...
      if (ER_NODE_MODE (el_key) == ER_NM_empty_el) continue;
  ...
  case ER_NM_stack: {
    ...
    hash
      = murmur_init ((char *) ER_ctop (ER_stack (key)) - (char *) ER_stack_vars (ER_stack (key)));
    for (i = 0, var = ER_stack_vars (ER_stack (key)); (char *) var <= ER_ctop (ER_stack (key));
         var = IVAL (var, 1), i++) {
...
static int do_inline eq_key (ER_node_t entry_key, ER_node_t key) {
  if (ER_NODE_MODE (entry_key) != ER_NODE_MODE (key)) return FALSE;
  switch (ER_NODE_MODE (key)) {
  case ER_NM_nil: return TRUE;
  case ER_NM_char: return ER_ch (key) == ER_ch (entry_key);
  case ER_NM_int: return ER_i (key) == ER_i (entry_key);
  case ER_NM_long: return mpz_cmp (*ER_mpz_ptr (ER_l (key)), *ER_mpz_ptr (ER_l (entry_key))) == 0;
  case ER_NM_float: return ER_f (key) == ER_f (entry_key);
  case ER_NM_type: return ER_type (key) == ER_type (entry_key);
  case ER_NM_vect: return eq_vector (ER_vect (key), ER_vect (entry_key));
  case ER_NM_tab: return eq_table (ER_tab (key), ER_tab (entry_key));
  case ER_NM_code:
    return (ER_code_context (key) == ER_code_context (entry_key)
            && ER_code_id (key) == ER_code_id (entry_key));
  case ER_NM_efun: return ER_efdecl (key) == ER_efdecl (entry_key);
  case ER_NM_thread: return ER_thread (key) == ER_thread (entry_key);
  case ER_NM_stack: return eq_stack (ER_stack (key), ER_stack (entry_key));
  default: d_unreachable ();
  }
...
#define START_ENTRIES_NUMBER(n) (OCCUPANCY_FACTOR * (n) < 300 ? 300 : OCCUPANCY_FACTOR * (n))

/* Return smallest such 2^n > SIZE.  */
static int get_power2 (size_t size) {
  unsigned int n;

  for (n = 0; size != 0; n++) size >>= 1;
...
ER_node_t create_tab (size_t size) {
  ...
  allocated_length
    = (ALLOC_SIZE (sizeof (_ER_heap_tab)) + els_size + entries_number * sizeof (entry_t));
  ...
  for (i = 0; i < ER_entries_number (result); i++) entries[i] = EMPTY_ENTRY;
  return result;
}
...
/* Return index of the first empty elem or EMPTY_ENTRY if there is no
   such element.  */
static entry_t do_always_inline first_empty_elem (ER_node_t tab) {
  entry_t els_bound = ER_els_bound (tab);

  if (els_bound >= ER_els_space_bound (tab)) return EMPTY_ENTRY;
...
static entry_t do_always_inline get_empty_elem_index (ER_node_t tab) {
  entry_t result = first_empty_elem (tab);

  if (result != EMPTY_ENTRY) ER_set_els_bound (tab, ER_els_bound (tab) + 1);
  return result;
}

/* Return the index of table TAB bin corresponding to
   HASH_VALUE.  */
static inline size_t hash_entry (size_t hash_value, ER_node_t tab) {
...
static inline size_t secondary_hash (size_t ind, ER_node_t tab, size_t *perterb) {
  *perterb >>= 11;
...
static entry_t find_tab_entry_num (ER_node_t tab, ER_node_t key, int reserve) {
  ER_node_t el_key;
  entry_ptr_t entries;
  ...
  if (reserve && ER_els_bound (tab) >= ER_els_space_bound (tab)) tab = tailor_tab (tab);
  d_assert (!reserve || first_empty_elem (tab) != EMPTY_ENTRY);
  peterb = hash_value
    = (ER_NODE_MODE (key) == ER_NM_int ? int_hash_val (ER_i (key)) : hash_key (key));
  ...
  for (;;) {
    ...
    if (el_index == EMPTY_ENTRY) {
      ...
      if (int_p && ER_NODE_MODE (el_key) == ER_NM_int ? ER_i (el_key) == ER_i (key)
                                                      : eq_key (el_key, key))
        break;
...
/* This function searches for table entry which contains key equal to
   ...
   reservation. */
ER_node_t find_tab_el (ER_node_t tab, ER_node_t key, int reserve) {
  ER_node_t el_key;
  ...
  if (el_index == EMPTY_ENTRY) return NULL;
  el_key = INDEXED_EL_KEY (ER_tab_els (tab), el_index);
  if (ER_NODE_MODE (el_key) != ER_NM_empty_el) ER_set_last_accessed_el_index (tab, el_index);
  return el_key;
}
...
ER_node_t copy_tab (ER_node_t tab) {
  ...
  for (i = 0; i < ER_els_bound (tab); i++)
    if (ER_NODE_MODE (INDEXED_EL_KEY (tab_els, i)) != ER_NM_empty_el) {
      ...
      d_assert (ER_NODE_MODE (new_tab) != ER_NM_heap_redir && new_el != NULL
                && ER_NODE_MODE (new_el) == ER_NM_empty_el);
      *(val_t *) new_el = *(val_t *) INDEXED_EL_KEY (tab_els, i);
      ...
      d_assert (ER_IS_OF_TYPE ((ER_node_t) & ((val_t *) new_el)[1], ER_NM_val));
    }
  return new_tab;
}
...
int remove_tab_el (ER_node_t tab, ER_node_t key) {
  ...
  if (el_index == EMPTY_ENTRY) return FALSE;
...
ER_node_t find_next_key (ER_node_t tab, int start) {
  size_t i;
  ER_node_t key;
    ...
    if (ER_NODE_MODE (vect) == ER_NM_heap_unpack_vect)
      *((val_t *) el + 1) = *(val_t *) IVAL (ER_unpack_els (vect), i);
      ...
      if (el_type == ER_NM_byte) {
        ...
        memcpy ((char *) v + val_displ (v), ER_pack_els (vect) + i * el_type_size, el_type_size);
        if (el_type == ER_NM_vect) ER_set_dim (v, 0);
      }
    }
  }
  return tab;
...
ER_node_t create_thread (pc_t start_thread_pc, BC_node_t block, ER_node_t fun_context) {
  ER_node_t thread;

  thread = (ER_node_t) heap_allocate (ALLOC_SIZE (sizeof (_ER_heap_thread)), FALSE);
...
static void activate_given_thread (ER_node_t thread, int first_p) {
  ER_node_t var;
  ...
  if (cthread != thread && profile_flag) {
    ...
    for (block = ER_block_node (ER_saved_cstack (cthread)); BC_ident (block) == NULL;
         block = BC_block_scope (block))
      ;
    ticker_off (&BC_exec_time (block));
    for (block = ER_block_node (ER_saved_cstack (thread)); BC_ident (block) == NULL;
         block = BC_block_scope (block))
      ;
    if (BC_calls_number (block) == 0) abort ();
    ticker_on (&BC_exec_time (block));
  }
#endif
  cthread = thread;
  cpc = ER_saved_pc (cthread);
  if (cstack != NULL) ER_set_ctop (cstack, (char *) ctop);
  ...
  if (!first_p) {
    var = IVAL (ER_stack_vars (get_obj_stack (lang_bc_decl)), BC_var_num (curr_thread_bc_decl));
...
static void activate_thread (int first_p) {
...
void block_cthread (pc_t first_resume_pc, int wait_stmt_flag) {
  ...
  ER_set_saved_cstack (cthread, cstack);
  if (executed_stmts_count != -thread_quantum) ER_set_thread_status (cthread, PS_STARTED);
  if (!wait_stmt_flag) {
    ...
    if (ER_thread_status (cthread) == PS_READY) {
      ...
      if (first_thread_not_started == NULL) first_thread_not_started = cthread;
    } else {
      d_assert (ER_thread_status (cthread) == PS_STARTED);
...
int delete_cthread_during_exception (void) {
...
void initiate_threads (pc_t start_pc) {
  cthread = NULL;
  thread_number = 0;
  thread_quantum = 700;
...
void interrupt (pc_t first_resume_pc) {
...
void store_handle (void *handle) { VARR_PUSH (void_ptr_t, dl_handle_varr, handle); }

void *external_address (BC_node_t decl) {
  void *address = NULL;
  const char *name;
  ...
  if (BC_address (decl) != NULL)
    ...
    for (i = 0; i < VARR_LENGTH (void_ptr_t, dl_handle_varr); i++) {
      address = dlsym (VARR_GET (void_ptr_t, dl_handle_varr, i), name);
      if (dlerror () == NULL) break;
    }
    if (i >= VARR_LENGTH (void_ptr_t, dl_handle_varr)) {
      (void) dlopen (NULL, RTLD_NOW);
      for (curr_libname_ptr = libraries; *curr_libname_ptr != NULL; curr_libname_ptr++) {
        void *handle;
        FILE *f;
  ...
  if (BC_IS_OF_TYPE (decl, BC_NM_evdecl)) {
    for (i = 0; i < VARR_LENGTH (void_ptr_t, external_vars); i++)
      ...
      if (VARR_GET (void_ptr_t, external_vars, i) == address) break;
    if (i >= VARR_LENGTH (void_ptr_t, external_vars))
      VARR_PUSH (void_ptr_t, external_vars, address);
  }
  return address;
}
